...
def findmatch(caps, MIMEtype, key="view", filename="/dev/null", plist=[]):
    ...
    if _find_unsafe(filename):
        msg = (
            "Refusing to use mailcap with filename %r. Use a safe temporary filename."
            % (filename,)
        )
        ...
        if c != "%":
            ...
            if c == "%":
                ...
                if _find_unsafe(MIMEtype):
                    msg = (
                        "Refusing to substitute MIME type %r into a shell command."
                        % (MIMEtype,)
                    )
                    warnings.warn(msg, UnsafeMailcapInput)
                    return None
                ...
                if _find_unsafe(param):
                    msg = (
                        "Refusing to substitute parameter %r (%s) into a shell command"
                        % (param, name)
                    )
                    warnings.warn(msg, UnsafeMailcapInput)
                    return None
