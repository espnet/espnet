...
def make_pat():
    ...
    match_softkw = (
        r"^[ \t]*"
        + r"(?P<MATCH_SOFTKW>match)\b"  # at beginning of line + possible indentation
        + r"(?![ \t]*(?:"
        + "|".join(
            [  # not followed by ...
    ...
    case_default = (
        r"^[ \t]*"
        + r"(?P<CASE_SOFTKW>case)"  # at beginning of line + possible indentation
        + r"[ \t]+(?P<CASE_DEFAULT_UNDERSCORE>_\b)"
    )
    case_softkw_and_pattern = (
        r"^[ \t]*"
        + r"(?P<CASE_SOFTKW2>case)\b"  # at beginning of line + possible indentation
        + r"(?![ \t]*(?:"
        + "|".join(
            [  # not followed by ...
