...
python subword-nmt/subword_nmt/learn_bpe.py -s 16000 < ${dumpdir}/${train_set}/train.tkn.tc.clean.${src_lang} > ${SRC_BPE_CODE}
python subword-nmt/subword_nmt/learn_bpe.py -s 16000 < ${dumpdir}/${train_set}/train.tkn.tc.clean.${tgt_lang} > ${TRG_BPE_CODE}

# apply bpe operation
# train
python subword-nmt/subword_nmt/apply_bpe.py -c ${SRC_BPE_CODE} < ${dumpdir}/${train_set}/train.tkn.tc.clean.${src_lang} > ${dumpdir}/${train_set}/train.tkn.tc.clean.${src_lang}_bpe16000
...
# valid
python subword-nmt/subword_nmt/apply_bpe.py -c ${SRC_BPE_CODE} < ${dumpdir}/${train_dev}/tst2012.tkn.tc.${src_lang} > ${dumpdir}/${train_dev}/tst2012.tkn.tc.${src_lang}_bpe16000
...
for ts in $(echo ${trans_set} | tr '_' ' '); do
    ...
    python subword-nmt/subword_nmt/apply_bpe.py -c ${SRC_BPE_CODE} < ${dumpdir}/${ts}/${name}.tkn.tc.${src_lang} > ${dumpdir}/${ts}/${name}.tkn.tc.${src_lang}_bpe16000
