...
num_spkrs=2     # number speakers in the mixed speech
# decoder related
dlayers=1
dunits=300
# attention related
...
wsj_2mix_wav=/export/b09/xkc09/data/wsj_mix/2speakers/wav16k
wsj_2mix_script=/export/b09/xkc09/data/wsj_mix/2speakers
...
if [ ${stage} -le 0 ]; then
    ...
    local/wsj_2mix_data_prep.sh $wsj_2mix_wav $wsj_2mix_script || exit 1;
fi
...
if [ ${stage} -le 1 ]; then
    ...
    # by xkc09 : I am not sure how the following JHU dump is used.
...
if [ ${stage} -le 3 ]; then
    ...
    if [ $use_wordlm = true ]; then
        ...
        mkdir -p ${lmdatadir}
        cat data/${wsj_train_set}/text | cut -f 2- -d" " > ${lmdatadir}/train_trans.txt
        zcat ${wsj1}/13-32.1/wsj1/doc/lng_modl/lm_train/np_data/{87,88,89}/*.z \
                | grep -v "<" | tr [a-z] [A-Z] > ${lmdatadir}/train_others.txt
        cat data/${wsj_train_dev}/text | cut -f 2- -d" " > ${lmdatadir}/valid.txt
        cat data/${wsj_train_test}/text | cut -f 2- -d" " > ${lmdatadir}/test.txt
...
if [ -z ${tag} ]; then
    expdir=exp/${train_set}_${backend}_${etype}_sde${elayers_sd}_rece${elayers_rec}_subsample${subsample}_unit${eunits}_proj${eprojs}_d${dlayers}_unit${dunits}_${atype}_spa${spa}_aconvc${aconv_chans}_aconvf${aconv_filts}_mtlalpha${mtlalpha}_${opt}_sampprob${samp_prob}_bs${batchsize}_mli${maxlen_in}_mlo${maxlen_out}
    if [ "${lsm_type}" != "" ]; then
