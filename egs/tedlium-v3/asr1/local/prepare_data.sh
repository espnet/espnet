...
;;'
    # Process the STMs
    cat db/TEDLIUM_release-3/speaker-adaptation/$set/stm/*.stm | sort -k1,1 -k2,2 -k4,4n | \
  ...
  cat $dir/spk2utt | awk -v set=$set -v pwd=$PWD '{ printf("%s '$sph2pipe' -f wav -p %s/db/TEDLIUM_release-3/speaker-adaptation/%s/sph/%s.sph |\n", $1, pwd, set, $1); }' > $dir/wav.scp
