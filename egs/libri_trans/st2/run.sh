...
train_config=conf/train_tcen.yaml
decode_config=conf/decode_tcen.yaml
...
feat_asr_dir=${dumpdir}/train_sp.en/delta${do_delta};
feat_mt_dir=${dumpdir}/train.fr
...
expname=${train_set}_${case}_tcen
expdir=exp/${expname}
mkdir -p ${expdir}
...
if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
    ...
    ${cuda_cmd} --gpu ${ngpu} ${expdir}/train.log \
        ...
        --verbose ${verbose} \
        --resume ${resume} \
        --st-train-json ${feat_tr_dir}/data_2ref.${case}.json \
        --asr-train-json ${feat_asr_dir}/data.lc.rm.json \
        --mt-train-json ${feat_mt_dir}/data_2ref.lc.rm_lc.json \
