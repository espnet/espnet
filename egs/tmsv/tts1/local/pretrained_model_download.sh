#!/bin/bash -e

# Copyright 2019 Nagoya University (Tomoki Hayashi)
#  Apache 2.0  (http://www.apache.org/licenses/LICENSE-2.0)

download_dir=$1
pretrained_model=$2

# check arguments
if [ $# != 2 ]; then
    echo "Usage: $0 <download_dir> <pretrained_model>"
    echo ""
    echo "Available pretrained models:"
    echo "    - CH_TG.tacotron2+spkemb"
    echo "    - CH_TG.transformer+spkemb"
    exit 1
fi

case "${pretrained_model}" in
    "CH_TG.tacotron2+spkemb") share_url="https://drive.google.com/open?id=1HwGjeu7IMtNWZEE-PT1A8-lSeT9QilBT" ;;
    "CH_TG.transformer+spkemb") share_url="https://drive.google.com/open?id=" ;;
    *) echo "No such pretrained model: ${pretrained_model}"; exit 1 ;;
esac

dir=${download_dir}/${pretrained_model}
mkdir -p ${dir}
if [ ! -e ${dir}/.complete ]; then
    download_from_google_drive.sh ${share_url} ${dir} ".tar.gz"
    touch ${dir}/.complete
fi
echo "Successfully finished donwload of pretrained model."
