...
if [ ${stage} -le -1 ] && [ ${stop_stage} -ge -1 ]; then
    ...
    # Please download the M-AILABS datasets first.
fi
...
if [ ${stage} -le 0 ] && [ ${stop_stage} -ge 0 ]; then
    ...
    for spk in angela rebecca ramona eva karlsson; do
        echo "Processing ${spk}..."
        local/data_prep.sh ${db_root} de_DE ${spk} data/${spk} ${trans_type}
    ...
    for spk in judy elliot; do
        echo "Processing ${spk}..."
        local/data_prep.sh ${db_root} en_US ${spk} data/${spk} ${trans_type}
        utils/fix_data_dir.sh data/${spk}
        utils/validate_data_dir.sh --no-feats data/${spk}
    done
