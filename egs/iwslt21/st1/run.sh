...
# WebRTC VAD related
frame_duration=10       # milliseconds of each frames
padding_duration=150    # milliseconds of the window size
aggressive_mode=3
...
iwslt_test_set_vad="et_stted_dev2010_vad.de et_stted_tst2010_vad.de et_stted_tst2013_vad.de et_stted_tst2014_vad.de \
                et_stted_tst2015_vad.de et_stted_tst2018_vad.de et_stted_tst2019_vad.de"
...
if [ ${stage} -le 0 ] && [ ${stop_stage} -ge 0 ]; then
    ...
    # prepare En-De test set with WebRTC VAD tool
    for x in dev2010 tst2010 tst2010 tst2013 tst2014 tst2015 tst2018 tst2019; do
        cp -rf ${stted_dir}/st1/data/${x}.de data/et_${data_code}_${x}_vad.de
        rm -rf data/et_${data_code}_${x}_vad.de/segments
        local/generate_segments_with_vad.py \
            --data-dir data/et_${data_code}_${x}_vad.de \
...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
    ...
    for x in ${iwslt_test_set} ${iwslt_test_set_vad}; do
...
if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ]; then
    ...
    for x in ${trans_subset} ${iwslt_test_set} ${iwslt_test_set_vad}; do
    (
        if [[ ${x} = *tst20* ]] || [[ ${x} = *dev20* ]]; then
