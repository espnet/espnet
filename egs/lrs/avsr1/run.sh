...
num=0  			# this variable is related with next variable. Only applies when ifdebug=true
...
if [ ${stage} -le 0 ] && [ ${stop_stage} -ge 0 ]; then
    ### Task dependent. You have to make data the following preparation part by yourself.
    ### But you can utilize Kaldi recipes in most cases
    echo "stage 0: Audio augmentation"
    for part in Test Val Train; do 
    ...
    echo "stage 0: Audio augmentation finished"
fi

mp3files=Dataset_processing/Audioaugments
feat_tr_dir=${dumpdir}/audio_org/${train_set}/delta${do_delta}; mkdir -p ${feat_tr_dir}
# Stage 1: Feature Generation for audio features
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
    echo "stage 1: Feature Generation"
    echo "stage 1.1: Make augmented mp3 files"
    mkdir -p $mp3files
    ...
    echo "stage 1.1: Make augmented mp3 files finished"

    ### Task dependent. You have to design training and dev sets by yourself.
    ### But you can utilize Kaldi recipes in most cases
    echo "stage 1.2: Feature Generation"
    ...
    echo "stage 1.2: Audio Feature Generation finished"
    echo "stage 1: Feature Generation finished"
fi


dict=data/lang_char/train_${bpemode}${nbpe}_units.txt
bpemodel=data/lang_char/train_${bpemode}${nbpe}
...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
    ...
    if [ "$train_lm" = true ] ; then
    	...
    	sed -r 's/([^ \t]+\s)(.*)/\1\L\2/' local/input.txt > data/lang_char/input.txt  || exit 1;
	...
	mv avsrlrs2_3/exp/train_rnnlm_pytorch_lm_unigram500 exp/train_rnnlm_pytorch_lm_unigram500
	...
	mv avsrlrs2_3/pretrainedvideomodel pretrainedvideomodel
	rm -rf avsrlrs2_3
    ...
    textfilenames=./data/audio/augment/LRS2_decode/*/text
    for textfilename in $textfilenames
    do
	sed -r 's/([^ \t]+\s)(.*)/\1\L\2/' $textfilename > ${textfilename}1  || exit 1;
	rm -rf $textfilename  || exit 1;
    ...
    textfilenames=./data/audio/clean/*/*/text
    for textfilename in $textfilenames
    do
	sed -r 's/([^ \t]+\s)(.*)/\1\L\2/' $textfilename > ${textfilename}1  || exit 1;
	rm -rf $textfilename  || exit 1;
    ...
    echo "stage 2: Dictionary and Json Data Preparation finished"
fi
...
if [ ${stage} -le 3 ] && [ ${stop_stage} -ge 3 ]; then
    echo "stage 3: Extract reliability measures"
	...
	if [ ${dataprocessingstage} -le 2 ] && [ ${stop_dataprocessingstage} -ge 2 ]; then
		#Stage 3.2: Video stream processing, using OpenFace for face recognition
    		echo "stage 3.2: OpenFace face recognition"
		mkdir -p $facerecog
    		for part in Test Val Train; do  #
	...
	if [ ${dataprocessingstage} -le 3 ] && [ ${stop_dataprocessingstage} -ge 3 ]; then	
	    # Stage 3.3: Extract Video frames from the MP4 File by using OpenFace results
	    echo "stage 3.3: Extract Frames"
	    mkdir -p $videoframe
	...
	if [ ${dataprocessingstage} -le 4 ] && [ ${stop_dataprocessingstage} -ge 4 ]; then
            # Stage 3.4: Use DeepXi to estimate SNR
            echo "stage 3.4: Estimate SNRs using DeepXi framework"
	    if [ -d DeepXi ]; then
	...
	if [ ${dataprocessingstage} -le 5 ] && [ ${stop_dataprocessingstage} -ge 5 ]; then
	    ...
	    echo "stage 3.5: Extract video features"
	...
	if [ ${dataprocessingstage} -le 6 ] && [ ${stop_dataprocessingstage} -ge 6 ]; then	
	    ...
	    echo "stage 3.6: Make video ark files"
	...
	if [ ${dataprocessingstage} -le 7 ] && [ ${stop_dataprocessingstage} -ge 7 ]; then
	    # Remake dump files
	    echo "stage 3.7: Remake audio and video dump files"

	    for dset in pretrain_Train Val Test Test_decode_music Test_decode_noise; do 
     		rm -rf dump/audio/$dset
	        python3 local/audiodump.py dump/audio dump/audio_org $dset $ifmulticore || exit 1;
            done

	    for dset in pretrain Val Test; do 
 		rm -rf dump/avpretrain/$dset
	        python3 local/avpretraindump.py dump/avpretrain dump/audio_org dump/video \
		...
		rm -rf dump/avtrain/$dset
	        python3 local/avtraindump.py dump/avtrain dump/audio_org $videofeature \
						$SNRptdir $videoframe dump/mfcc \
						$dset $ifmulticore || exit 1;
            done
		...
		rm -rf dump/videopretrain/$dset
	        python3 local/videodump.py dump/avpretrain dump/videopretrain $dset || exit 1;
            done

	    for dset in Train Val Test; do 
		rm -rf dump/videotrain/$dset
	        python3 local/videodump.py dump/avtrain dump/videotrain $dset || exit 1;
            done
	    ...
	    for noisecombination in 'noise_None' 'music_None' 'noise_blur' 'noise_saltandpepper'; do 
	        python3 local/avpretraindecodedump.py dump/avpretraindecode dump/audio_org dump/video \
				$SNRptdir $videoframe dump/mfcc \
				$dset $noisecombination $ifmulticore || exit 1;
		python3 local/avtraindecodedump.py dump/avtraindecode dump/audio_org dump/video \
	...
	if [ ${dataprocessingstage} -le 8 ] && [ ${stop_dataprocessingstage} -ge 8 ]; then
	    echo "stage 3.8: Split Test decode dump files"
    ...
    echo "stage 3: Reliability measures generation finished"
fi
...
# Stage 4: Language Model (LM) preparation
if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
    if [ "$train_lm" = false ] ; then
        echo "stage 4: Use pretrained LM"
    else
...
if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ]; then
	...
	noisetype=noise 	# Which noise type data is used for decoding, possible noisetype: noise music blur and saltandpepper
        (local/train_audio.sh --backend $backend \
				--ngpu $ngpu \
				--debugmode $debugmode \
				--N $N \
 				--verbose $verbose \
	...
	noisetype=blur 	# Which noise type data is used for decoding, possible noisetype: noise music blur and saltandpepper
        (. local/pretrain_video.sh --backend $backend \
				--ngpu $ngpu \
				--debugmode $debugmode \
				--N $N \
 				--verbose $verbose \
	...
	noisetype=blur 	# Which noise type data is used for decoding, possible noisetype: noise music blur and saltandpepper
        (. local/finetune_video.sh --backend $backend \
				--ngpu $ngpu \
				--debugmode $debugmode \
				--N $N \
 				--verbose $verbose \
	...
	noisetype=noise 	# Which noise type data is used for decoding, possible noisetype: noise music blur and saltandpepper
        (. local/pretrain_av.sh --backend $backend \
				--ngpu $ngpu \
				--debugmode $debugmode \
				--N $N \
 				--verbose $verbose \
	...
	noisetype=noise 	# Which noise type data is used for decoding, possible noisetype: noise music blur and saltandpepper
        (. local/finetune_av.sh --backend $backend \
				--ngpu $ngpu \
				--debugmode $debugmode \
				--N $N \
 				--verbose $verbose \
...
if [ ${stage} -le 6 ] && [ ${stop_stage} -ge 6 ]; then
    ...
    modeldir=exp/pretrain/A 	# Which model
    noisetype=noise		# Which noise type data is used for decoding, possible noisetype: noise music blur and saltandpepper
    ./local/show_result.sh $modeldir/$noisetype $modeldir/${noisetype}.txt
fi
exit 0
