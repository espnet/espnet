...
def main(sourcedir, filelistdir, savedir, dset, savewavdir, ifmulticore, ifsegment):
    ...
    if ifmulticore is True:
        pool = mp.Pool()
        job_args = [
            (i, dset, savedir, sourcedir, savewavdir, ifsegment) for i in filelist
        ]
        pool.map(product_helper, job_args)
    else:
        for i in filelist:
            set(i, dset, savedir, sourcedir, savewavdir, ifsegment)
...
def segment(textfiledir, file, ifsegment=True):
...
def set(file, s, savedir, sourcedir, savewavdir, ifsegment):
    ...
    Args:
        ...
        savedir (str): The dir save the Kaldi files.
        savewavdir (str): The dir save mp3 files

    """
    # segdir = os.path.join(savedir, 'segments')
    ...
    if ifsegment is False:
        segmentinfo = segment(textfile, file, ifsegment)
        command1 = "ffmpeg -y -i"
        command2 = (
            "-vn -ac 2 -ar 16000 -ab 320k -f wav /tmp/tmp.$$; cat /tmp/tmp.$$ |\n"
        )
        ...
        segmentinfo = segment(textfile, file)
        if len(segmentinfo[file]) == 1:
            ...
            utttxt = [
                Title
                + " "
                + "_".join(["LRS2", splitname[0], splitname[1] + "p"])
                + "\n"
            ]
            ...
            for i in range(len(segmentinfo[file])):
                ...
                if endtime - starttime <= 1.0:
                    ...
                    finalsave = os.path.join(savewavdir, Title + ".wav")
                    segmentinfos.append(
                        " ".join([finalsave, mp4dir, str(starttime), str(endtime)])
                        + "\n"
                    )
                    ...
                    wavtxt.append(" ".join([Title, finalsave]) + "\n")
                    utttxt.append(
                        Title
                        + " "
                        + "_".join(["LRS2", splitname[0], splitname[1] + "p"])
...
# sys.argv[7] = ifsegment (str): If do segmentation.
