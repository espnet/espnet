...
def main(sourcedir, filelistdir, savedir, dset, savewavdir, ifmulticore, ifsegment):
    ...
    if ifmulticore is True:
        pool = mp.Pool()
        job_args = [(i, dset, savedir, sourcedir, savewavdir, ifsegment) for i in filelist]
        ...
        if len(segmentinfo[file]) == 1:
            ...
            for i in range(len(segmentinfo[file])):
                ...
                if endtime - starttime <= 1.0:
                    ...
                    segmentinfos.append(" ".join([finalsave, mp4dir, str(starttime), str(endtime)]) + "\n")
                    ...
                    utttxt.append(Title + ' ' + '_'.join(['LRS2', splitname[0], splitname[1] + 'p']) + '\n')
