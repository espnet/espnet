...
def batchfy_by_seq(
    ...
    while True:
        ...
        ilen = int(info[ikey][iaxis]["ashape"][0])
        olen = (
...
def batchfy_by_bin(
    ...
    """Make variably sized batch set, which maximizes the number of bins
        up to `batch_bins`.
    ...
    while True:
        ...
        while next_size < batch_bins and (start + b) < length:
            ilen = int(sorted_data[start + b][1][ikey][0]["sshape"][0]) * idim
...
def batchfy_by_frame(
    ...
    if max_frames_in <= 0 and max_frames_out <= 0 and max_frames_inout <= 0:
        raise ValueError(
            f"At least, one of `--batch-frames-in`, `--batch-frames-out` \
            or `--batch-frames-inout` should be > 0"
        )
    length = len(sorted_data)
    minibatches = []
    start = 0
...
def make_batchset(
    ...
    :param bool swap_io: if True, use "input" as output and "output" as input
                        in `data` dict
    :param bool mt: if True, use 0-axis of "output" as output and 1-axis of "output" as
                    input in `data` dict
    :param int iaxis: dimension to access input (for ASR, TTS iaxis=0,
                    for MT iaxis="1".)
    :param int oaxis: dimension to access output (for ASR, TTS, MT oaxis=0,
    ...
    if batch_sort_key not in BATCH_SORT_KEY_CHOICES:
        raise ValueError(
            f"arg 'batch_sort_key' ({batch_sort_key}) should be one \
            of {BATCH_SORT_KEY_CHOICES}"
        )
