    ...
    def __call__(self, batch, device):
        ...
        if axs[0].dtype.kind == "c":
            axs_pad_real = pad_list(
                [torch.from_numpy(ax.real).float() for ax in axs], 0
            ).to(device, dtype=self.dtype)
            axs_pad_imag = pad_list(
                [torch.from_numpy(ax.imag).float() for ax in axs], 0
            ).to(device, dtype=self.dtype)
            # Note(kamo):
            ...
            axs_pad = pad_list([torch.from_numpy(ax).float() for ax in axs], 0).to(
        ...
        if rms[0].dtype.kind == "c":
            rms_pad_real = pad_list(
                [torch.from_numpy(rm.real).float() for rm in rms], 0
            ).to(device, dtype=self.dtype)
            xs_pad_imag = pad_list(
                [torch.from_numpy(rm.imag).float() for rm in rms], 0
            ).to(device, dtype=self.dtype)
            # Note(kamo):
            # {'real': ..., 'imag': ...} will be changed to ComplexTensor in E2E.
            # Don't create ComplexTensor and give it E2E here
            ...
            rms_pad = pad_list(
                [torch.from_numpy(rm.astype(np.float32)).float() for rm in rms], 0
            ).to(device, dtype=self.dtype)
...
def train(args):
    ...
    if args.n_iter_processes > 0:
        ...
        train_iter = ToggleableShufflingSerialIterator(
            TransformDataset(train, load_tr), batch_size=1, shuffle=not use_sortagrad
        )
    ...
    trainer.extend(
        CustomEvaluator(model, valid_iter, reporter, converter, device, args.ngpu)
    )
    ...
    if args.num_save_attention > 0 and args.mtlalpha != 1.0:
        data = sorted(
            ...
            key=lambda x: int(x[1]["input"][0]["vshape"][1]),
            reverse=True,
        )
            ...
            for idx, name in enumerate(js.keys(), 1):
                ...
                if args.streaming_mode == "window" and args.num_encs == 1:
                    ...
                    nbest_hyps = model.recognize(
                        afeat, vfeat, rms, args, train_args.char_list, rnnlm
                    )
