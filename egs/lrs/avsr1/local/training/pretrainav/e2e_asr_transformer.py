...
from espnet.pretrainav.ctc import CTC
from espnet.nets.pytorch_backend.e2e_asr import CTC_LOSS_THRESHOLD
from espnet.nets.pytorch_backend.e2e_asr import Reporter
from espnet.nets.pytorch_backend.nets_utils import make_pad_mask
from espnet.pretrainav.nets_utils import th_accuracy
from espnet.nets.pytorch_backend.transformer.attention import MultiHeadedAttention
from espnet.pretrainav.weighttransfn import transformerNet
from espnet.pretrainav.decoder import Decoder
from espnet.nets.pytorch_backend.transformer.encoder import Encoder
from espnet.pretrainav.rmencoder import Encoder as rmEncoder
from espnet.pretrainav.ctcencoder import Encoder as ctcEncoder
from espnet.nets.pytorch_backend.transformer.initializer import initialize
from espnet.pretrainav.label_smoothing_loss import LabelSmoothingLoss
from espnet.nets.pytorch_backend.transformer.mask import subsequent_mask
from espnet.pretrainav.plot import PlotAttentionReport
...
class E2E(ASRInterface, torch.nn.Module):
    ...
    def forward(self, axs_pad, vxs_pad, rms_pad, ilens, ys_pad):
        ...
        vhs_pad, vhs_mask = self.vencoder(vxs_pad, vsrc_mask)
        ...
        apred_pad, apred_mask, armored = self.adecoder(
            ys_in_pad, ys_mask, ahs_pad, ahs_mask, armhs_pad
        )
        vpred_pad, vpred_mask, vrmpred = self.vdecoder(
            ys_in_pad, ys_mask, vhs_pad, vhs_mask, vrmhs_pad
        )
        ...
        cattransfeats = torch.cat(
            (
                torch.softmax(apred_pad, dim=-1),
                torch.softmax(vpred_pad, dim=-1),
                transinfo,
            ),
            dim=-1,
        )
    ...
    def vencode(self, vfeat):
        self.eval()
        vfeat = torch.as_tensor(vfeat).unsqueeze(0)
        venc_output, _ = self.vencoder(vfeat, None)
    ...
    def recognize(
        self, afeat, vfeat, rms, recog_args, char_list=None, rnnlm=None, use_jit=False
    ):
        """recognize feat
        ...
        venc_output = self.vencode(vfeat).unsqueeze(0)
        ...
        for i in six.moves.range(maxlen):
            ...
            for hyp in hyps:
                ...
                if use_jit:
                    ...
                    a_att_scores, armpred = self.adecoder.recognize(
                        ys, ys_mask, aenc_output, arm_output
                    )
                    v_att_scores, vrmpred = self.vdecoder.recognize(
                        ys, ys_mask, venc_output, vrm_output
                    )
