...
ln -rsf  local/training/pretrainav ${MAIN_ROOT}/espnet/pretrainav  || exit 1;
rm -rf $MAIN_ROOT/espnet/bin/asr_train_pretrain_av.py   || exit 1;
ln -rsf  local/training/pretrainav/asr_train_pretrain_av.py $MAIN_ROOT/espnet/bin/asr_train_pretrain_av.py   || exit 1;
rm -rf $MAIN_ROOT/espnet/bin/asr_recog_pretrain_av.py  || exit 1;
ln -rsf  local/training/pretrainav/asr_recog_pretrain_av.py $MAIN_ROOT/espnet/bin/asr_recog_pretrain_av.py  || exit 1;


if [ ${stage} -le 0 ] && [ ${stop_stage} -ge 0 ]; then
    echo "stage 0: Pretrained AV Network Training"
    ${cuda_cmd} --gpu ${ngpu} ${expdir}/train.log \
        asr_train_pretrain_av.py \
        --ngpu ${ngpu} \
        --preprocess-conf ${preprocess_config} \
        --config $(change_yaml.py ${train_config} -o conf/pretrainav.yaml -a model-module=espnet.pretrainav.e2e_asr_transformer:E2E -a batch-size=16 -a epochs=65)  \
        --backend ${backend} \
        ...
        --pretrain-audio-model $pretrainedaudio/results/model.last10.avg.best \
        --pretrain-video-model $pretrainedvideo/results/model.last10.avg.best 
