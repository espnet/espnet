...
from espnet.finetuneav.decoder_layer import DecoderLayer
...
class Decoder(ScorerInterface, torch.nn.Module):
    ...
    def forward(self, tgt, tgt_mask, memory, memory_mask, rmmemory):
        ...
        :param torch.Tensor tgt: input token ids, int64 (batch, maxlen_out)
                                    if input_layer == "embed"
                                 input tensor (batch, maxlen_out, #mels) in
                                 the other cases
        ...
        :return x: decoded token score before softmax (batch, maxlen_out, token)
                   ...
                   final block outputs (batch, maxlen_out, attention_dim) in
                   the other cases
        :rtype: torch.Tensor
        ...
        x, tgt_mask, memory, memory_mask, rmmemory, rmx = self.decoders(
            x, tgt_mask, memory, memory_mask, rmmemory, rmx
        )
        if self.normalize_before:
            x = self.after_norm(x)
    ...
    def recognize(self, tgt, tgt_mask, memory, rmmemory):
        ...
        rmx = torch.zeros(x.size())
        x, tgt_mask, memory, memory_mask, rmmemory, rmx = self.decoders(
            x, tgt_mask, memory, None, rmmemory, rmx
        )
        if self.normalize_before:
