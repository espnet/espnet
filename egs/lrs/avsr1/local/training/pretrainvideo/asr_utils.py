...
def torch_load_pretrained_audio(snapshot_path, trainer):
    ...
    loaded_state = {
        f"a{k}": v for k, v in loaded_state.items() if f"a{k}" in self_state
    }
...
def torch_load_pretrained_video(snapshot_path, trainer):
    ...
    self_state = trainer.updater.model.state_dict()
    loaded_state = {f"{k}": v for k, v in loaded_state.items() if f"{k}" in self_state}
    state_key = loaded_state.keys()
    self_state.update(loaded_state)
    trainer.updater.model.load_state_dict(self_state)
...
def add_gradient_noise(model, iteration, duration=100, eta=1.0, scale_factor=0.55):
    ...
    Args:
        ...
        duration (int) {100, 1000}: Number of durations to control the
                                    interval of the `sigma` change.
