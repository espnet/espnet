...
from espnet.finetunevideo.asr_utils import torch_load_pretrained_video
...
from espnet.finetunevideo.io_utils import LoadInputsAndTargets
from espnet.finetunevideo.batchfy import make_batchset
...
def train(args):
    ...
    idim_list = [int(valid_json[utts[0]]['input'][i]['ashape'][-1]) for i in range(args.num_encs)]
    ...
    if args.num_save_attention > 0 and args.mtlalpha != 1.0:
        data = sorted(list(valid_json.items())[:args.num_save_attention],
                      key=lambda x: int(x[1]['input'][0]['ashape'][1]), reverse=True)
...
def recog(args):
    ...
    if args.batchsize == 0:
        ...
        keys = list(js.keys())
        if args.batchsize > 1:
            feat_lens = [js[key]['input'][0]['ashape'][0] for key in keys]
