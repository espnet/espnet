...
def torch_load_pretrained_av(snapshot_path, trainer):
    ...
    # load snapshot
    loaded_state = torch.load(snapshot_path, map_location=lambda storage, loc: storage)

    #self_state = trainer.updater.model.state_dict();
    ...
    trainer.updater.model.load_state_dict(self_state)

    print('load for video model')
    print(torch.__version__) 

    del loaded_state

def torch_load_pretrained_audio(snapshot_path, trainer):
    """Resume from snapshot for pytorch.
    ...
    # load snapshot
    saveloaded_state = torch.load(snapshot_path, map_location=lambda storage, loc: storage)

    #self_state = trainer.updater.model.state_dict();
    #trainer.updater.model.load_state_dict(snapshot_dict)
    self_state = trainer.updater.model.state_dict()
    loaded_state = {f'a{k}': v for k, v in saveloaded_state.items() if f'a{k}' in self_state}
    saveloaded_state['ctc.actc_lo.weight'] = saveloaded_state.pop('ctc.ctc_lo.weight')
    saveloaded_state['ctc.actc_lo.bias'] = saveloaded_state.pop('ctc.ctc_lo.bias')
    loaded_state.update({'ctc.actc_lo.weight': saveloaded_state['ctc.actc_lo.weight']})
    loaded_state.update({'ctc.actc_lo.bias': saveloaded_state['ctc.actc_lo.bias']})
    #loaded_state = {k: v for k, v in loaded_state.items() if k in self_state}
    state_key = loaded_state.keys()
    self_state.update(loaded_state)
    trainer.updater.model.load_state_dict(self_state)
    for name, p in trainer.updater.model.named_parameters():
        if "a" in name[0]:
            if name in state_key:
                p.requires_grad = False
        elif name == 'ctc.actc_lo.weight':
            p.requires_grad = False
        elif name == 'ctc.actc_lo.bias':
            p.requires_grad = False

    print('load for audio model, frozen')

    del loaded_state

def torch_load_pretrained(snapshot_path, trainer):
    """Resume from snapshot for pytorch.
    ...
    #trainer.updater.model.load_state_dict(snapshot_dict)
    self_state = trainer.updater.model.state_dict()
    loaded_state = {f'{k}': v for k, v in saveloaded_state.items() if f'{k}' in self_state}
    state_key = loaded_state.keys()
    self_state.update(loaded_state)
    trainer.updater.model.load_state_dict(self_state)

    print('load for pretrained model')

    del loaded_state    



def torch_load_pretrained_video(snapshot_path, trainer):
    """Resume from snapshot for pytorch.
    ...
    #trainer.updater.model.load_state_dict(snapshot_dict)
    self_state = trainer.updater.model.state_dict()
    loaded_state = {f'v{k}': v for k, v in saveloaded_state.items() if f'v{k}' in self_state}
    saveloaded_state['ctc.vctc_lo.weight'] = saveloaded_state.pop('ctc.ctc_lo.weight')
    saveloaded_state['ctc.vctc_lo.bias'] = saveloaded_state.pop('ctc.ctc_lo.bias')
    loaded_state.update({'ctc.vctc_lo.weight': saveloaded_state['ctc.vctc_lo.weight']})
    loaded_state.update({'ctc.vctc_lo.bias': saveloaded_state['ctc.vctc_lo.bias']})
    #del loaded_state['vencoder.embed.out.0.weight']
    #del loaded_state['vencoder.embed.out.0.bias']
    state_key = loaded_state.keys()
    self_state.update(loaded_state)
    trainer.updater.model.load_state_dict(self_state)
    for name, p in trainer.updater.model.named_parameters():
        if "v" in name[0]:
            if name in state_key:
                p.requires_grad = False
        elif name == 'ctc.vctc_lo.weight':
            p.requires_grad = False
        elif name == 'ctc.vctc_lo.bias':
            p.requires_grad = False

    print('load for video model, frozen')
...
def get_model_conf(model_path, conf_path=None):
    ...
    if isinstance(confs, dict):
        ...
        # for asr, tts, mt
        aidim, vidim, odim, args = confs
        return aidim, vidim, odim, argparse.Namespace(**args)
