...
def get_preprocessing_pipelines():
    ...
    # -- LRW config
    crop_size = (88, 88)
    preprocessing['pretrain'] = Compose([
...
def pad_packed_collate(batch):
    if len(batch) == 1:
        data, lengths, namelist, = zip(*[(a, a.shape[0], b) for (a, b) in sorted(batch, key=lambda x: x[0].shape[0], reverse=True)])
        data = torch.FloatTensor(data)
        lengths = [data.size(1)]

    if len(batch) > 1:
        data_list, lengths, namelist = zip(*[(a, a.shape[0], b) for (a, b) in sorted(batch, key=lambda x: x[0].shape[0], reverse=True)])
...
class Lipreading(nn.Module):
    ...
    def forward(self, x):
        '''if training is True:
            ...
            x = HorizontalFlip(x)
            #cv2.imshow('window_name', x[0,:,:].cpu().numpy())
            #cv2.waitKey(0)
        else:
...
def main(filedir, savedir, pretrainedmodel, dset, ifcuda, debug=False):
    ...
    data_loader = torch.utils.data.DataLoader(dataset, batch_size=1, shuffle=False, collate_fn=pad_packed_collate, num_workers=NWORK)
    for count, batch in enumerate(data_loader, 0):
        ...
        try:
            ...
            for i in range(len(datalength)):
                ...
                torch.save(savedata, savedir + '/' + name[i], _use_new_zipfile_serialization=False)
                if debug == True:
