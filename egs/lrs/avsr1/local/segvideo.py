...
def extract_pretrain_opencv(mp4filedir, csvname, segmentslist, corpus):
    ...
    cropframe = cropframe / 255.
    cropframe = [cv2.resize(cropframe[i, :, :], (96, 96)) for i in range(len(cropframe))]
    cropframe = np.array(cropframe)
    mean = 0.4161
    std = 0.1688
...
def segpretrainvideo(segdict, savedir, csvdir, corpus):
    ...
    filename = "/".join([mp4filedir.split("/")[-2], mp4filedir.split("/")[-1].split(".")[0]])
    ...
    for file in filekeys:
        ...
        torch.save(segmented[file]['conf'], confdir, _use_new_zipfile_serialization=False)
        torch.save(segmented[file]['AU'], AUdir, _use_new_zipfile_serialization=False)
        torch.save(segmented[file]['frames'], Picdir, _use_new_zipfile_serialization=False)
...
def segvideo(sourcedir, filelist, savedir, csvdir, dset):
    ...
    if dset == "pretrain":
        confdir = os.path.join(savedir, "Conf", "LRS2_" + filelist[0] + "_" + filelist[1] + "p.pt")
        AUdir = os.path.join(savedir, "AUs", "LRS2_" + filelist[0] + "_" + filelist[1] + "p.pt")
        Picdir = os.path.join(savedir, "Pics", "LRS2_" + filelist[0] + "_" + filelist[1] + "p.pt")
    else:
        confdir = os.path.join(savedir, "Conf", "LRS2_" + filelist[0] + "_" + filelist[1] + "m.pt")
        AUdir = os.path.join(savedir, "AUs", "LRS2_" + filelist[0] + "_" + filelist[1] + "m.pt")
        Picdir = os.path.join(savedir, "Pics", "LRS2_" + filelist[0] + "_" + filelist[1] + "m.pt")
...
def main(sourcedir, savedir, csvdir, audiorefdir, dset, corpus, ifsegment, ifmulticore):
    ...
    if ifsegment is False:
        ...
        for i in range(len(filelist)):
            ...
            filelist[i] = "/".join([filelist[i].split("_")[1], filelist[i].split("_")[2][: -1]])
        ...
        if dset == "pretrain":
            ...
            if corpus == 'LRS2':
                if ifmulticore is True:
                    pool = mp.Pool()
                    job_args = [({i: segdict[i]}, savedir, csvdir, corpus) for i in segmentdictkey]
                ...
                if ifmulticore is True:
                    pool = mp.Pool()
                    job_args = [(sourcedir, i, savedir, csvdir, dset) for i in nosegmentlist]
                    pool.map(product_helper, job_args)
                else:
