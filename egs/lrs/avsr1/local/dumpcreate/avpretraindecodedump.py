...
def processing(i, audiodata, mfccdata, vdir, snrdir, vrmdir, dset, videonoise):
    ...
    if dset == "pretrain":
        ...
        if videonoise == "None":
            ...
            videofeatdir = os.path.join(
                vrmdir, "LRS2" + dset + "_" + videonoise, "Pics"
            )
    audiodata["utts"][i]["input"][0]["afeat"] = audiodata["utts"][i]["input"][0].pop(
        "feat"
    )
    audiodata["utts"][i]["input"][0]["ashape"] = audiodata["utts"][i]["input"][0].pop(
        "shape"
    )
    ...
    audiodata["utts"][i]["input"][0]["aRMshape"] = [
        audiodata["utts"][i]["input"][0]["ashape"][0],
        1,
    ]
    audiodata["utts"][i]["input"][0]["vRMshape"] = [
        audiodata["utts"][i]["input"][0]["vshape"][0],
        1,
    ]
    audiodata["utts"][i]["input"][0]["mfcc"] = mfccdata["utts"][i]["input"][0]["feat"]
    audiodata["utts"][i]["input"][0]["mfccshape"] = mfccdata["utts"][i]["input"][0][
        "shape"
    ]
    audiodata["utts"][i]["input"][0]["AUs"] = os.path.join(AUdir, videoi + ".pt")
    audiodata["utts"][i]["input"][0]["AUshape"] = [
        audiodata["utts"][i]["input"][0]["vshape"][0],
        6,
    ]
...
def avpretraindecodedump(
    ...
    for root, dirs, files in os.walk(
        os.path.join(dumpaudiofile, dset + "_decode_" + audionoise)
    ):
        for file in files:
            if ".json" in file:
                jsonname = file
    ...
    for root, dirs, files in os.walk(
        os.path.join(mfccdumpdir, dset + "_decode_" + audionoise)
    ):
        for file in files:
            if ".json" in file:
    ...
    with open(videodumpfile, encoding="UTF-8") as videojson_file:
    ...
    savefilename = filename.replace(
        os.path.join(dumpaudiofile, dset + "_decode_" + audionoise),
        os.path.join(dumpfile, savename),
    )
...
# sys.argv[8] = noisecombination (str), Augumented audio and video noise type
# sys.argv[9] = ifmulticore (boolean), If multi cpu processing should be used


avpretraindecodedump(
    sys.argv[1],
    sys.argv[2],
    sys.argv[3],
    sys.argv[4],
