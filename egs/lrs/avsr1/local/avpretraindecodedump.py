...
def avpretraindecodedump(dumpfile, dumpaudiofile, dumpvideofile, snrdir, vrmdir, mfccdumpdir, dset, noisecombination, ifmulticore):
    ...
    if ifmulticore is True:
        ...
        pool = mp.Pool()
        job_args = [(i, audiodata, mfccdata, videodata['utts'][i.split('-')[0]], snrdir, vrmdir, dset, videonoise) for i in keylist]
        results.extend(pool.map(product_helper, job_args))
    else:
        keylist = list(audiodata['utts'].keys())
        results = []
        for i in keylist:
            results.append(processing(i, audiodata, mfccdata, videodata['utts'][i.split('-')[0]], snrdir, vrmdir, dset, videonoise))
...
avpretraindecodedump(sys.argv[1],sys.argv[2],sys.argv[3], sys.argv[4], sys.argv[5],sys.argv[6],sys.argv[7], sys.argv[8], sys.argv[9])
