...
class LoadInputsAndTargets(object):
    ...
    def __call__(self, batch):
        ...
        rms_feats_dict = OrderedDict()  # OrderedDict[str, List[np.ndarray]]
        target_dict = OrderedDict()  # OrderedDict[str, List[np.ndarray]]
        ...
        for uttid, info in batch:
            ...
            if self.load_input:
                ...
                for idx, inp in enumerate(info['input']):
                    ...
                    mfcc = self._get_from_loader(
                        filepath=inp['mfcc'],
                        filetype=inp.get('filetype', 'mat'))
                    vx = self._get_from_loader(
                        filepath=inp['vfeat'],
                        filetype=inp.get('filetype', 'mat'))
                    arms = self._get_from_loader(
                        filepath=inp['aRMs'],
                        filetype=inp.get('filetype', 'pt'))

                    vrms = self._get_from_loader(
                        filepath=inp['vRMs'],
                        filetype=inp.get('filetype', 'pt'))
                    AUs = self._get_from_loader(
                        filepath=inp['AUs'],
                        filetype=inp.get('filetype', 'pt'))
                    pitch = ax[:, -3:]
                    ...
                    vrms = dda(vrms, alen)
                    AUs = dda(AUs, alen)
                    arms = np.expand_dims(arms, axis=1)
                    vrms = np.expand_dims(vrms, axis=1)
                    rms = np.concatenate((mfcc[:, : 7], pitch), axis=1)

                    rms = np.concatenate((rms, arms), axis=1)
                    rms = np.concatenate((rms, vrms), axis=1)
                    rms = np.concatenate((rms, AUs), axis=1)
            ...
            if self.load_output:
                ...
                for idx, inp in enumerate(info['output']):
                    #if int(info['input'][idx]['ashape'][0] / 4.0 ) == 0:
                        #pass
                    #else:
                    if 'tokenid' in inp:
        ...
        if self.mode == 'asr':
            return_batch, uttid_list = self._create_batch_asr(
                ax_feats_dict, vx_feats_dict, rms_feats_dict, y_feats_dict, uttid_list)
        elif self.mode == 'tts':
            _, info = batch[0]
    ...
    def _create_batch_asr(self, ax_feats_dict, vx_feats_dict, rms_feats_dict, y_feats_dict, uttid_list):
        ...
        rms = list(rms_feats_dict.values())
        if self.load_output:
            if len(y_feats_dict) == 1:
                ys = list(y_feats_dict.values())[0]
                assert len(axs[0]) == len(ys), (len(axs[0]), len(ys))
        ...
        rms = [[x[i] for i in nonzero_sorted_idx] for x in rms]
        ...
        if self.load_output:
            ...
            return_batch = OrderedDict([*[('a' + x_name, ax) for x_name, ax in zip(x_names, axs)], *[('v' + x_name, vx) for x_name, vx in zip(x_names, vxs)], *[('r' + x_name, rm) for x_name, rm in zip(x_names, rms)], (y_name, ys)])
        else:
            return_batch = OrderedDict([*[('a' + x_name, ax) for x_name, ax in zip(x_names, axs)], *[('v' + x_name, vx) for x_name, vx in zip(x_names, vxs)], *[('r' + x_name, rm) for x_name, rm in zip(x_names, rms)]])
...
def downsample(data, c2):
    ...
    for i in range(len(idxdict)):
        id = idxdict[i]
        dataexp.append(data[id])

    return np.asarray(dataexp)
