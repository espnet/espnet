...
from espnet.pretrainav.attention import MultiHeadedAttention as transfMultiHeadedAttention
from espnet.nets.pytorch_backend.transformer.attention import MultiHeadedAttention
from espnet.pretrainav.decoder_layer import DecoderLayer
...
class Decoder(ScorerInterface, torch.nn.Module):
    ...
    def __init__(self, odim,
        ...
        self.decoders = repeat(
            ...
            lambda lnum: DecoderLayer(
                ...
                transfMultiHeadedAttention(attention_heads, attention_dim, src_attention_dropout_rate),
        ...
        #self.transformerweightnet = Net(attention_dim)
    ...
    def forward(self, tgt, tgt_mask, memory, memory_mask, rmmemory):
        ...
        """
        x = self.embed(tgt)
        rmx = torch.zeros(x.size())
        x, tgt_mask, memory, memory_mask, rmmemory, rmx = self.decoders(x, tgt_mask, memory, memory_mask, rmmemory, rmx)
        if self.normalize_before:
            x = self.after_norm(x)
        if self.output_layer is not None:
            x = self.output_layer(x)
        return x, tgt_mask, rmx 

    def recognize(self, tgt, tgt_mask, memory, rmmemory):
        """recognize one step
        ...
        x = self.embed(tgt)
        rmx = torch.zeros(x.size())
        x, tgt_mask, memory, memory_mask, rmmemory, rmx = self.decoders(x, tgt_mask, memory, None, rmmemory, rmx)
        if self.normalize_before:
            x_ = self.after_norm(x[:, -1])
        else:
            x_ = x[:, -1]
        if self.output_layer is not None:
            return torch.softmax(self.output_layer(x_), dim=-1), rmx[:, -1]
        else:
            return x_
