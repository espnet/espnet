...
#  Apache 2.0  (http://www.apache.rorg/licenses/LICENSE-2.0)
...
from espnet.pretrainav.asr_utils import torch_resume
from espnet.asr.asr_utils import torch_snapshot
from espnet.pretrainav.asr_init import load_trained_model
from espnet.pretrainav.asr_utils import torch_load_pretrained_audio
from espnet.pretrainav.asr_utils import torch_load_pretrained_video
from espnet.pretrainav.asr_utils import torch_load_pretrained
...
from espnet.pretrainav.io_utils import LoadInputsAndTargets
from espnet.pretrainav.batchfy import make_batchset
...
class CustomConverter(object):
    ...
    def __call__(self, batch, device):
        ...
        ilens = np.array([x.shape[0] for x in axs])

        # perform padding and convert to tensor
        # currently only support real number
        if axs[0].dtype.kind == 'c':
            axs_pad_real = pad_list(
                [torch.from_numpy(ax.real).float() for ax in axs], 0).to(device, dtype=self.dtype)
            axs_pad_imag = pad_list(
                [torch.from_numpy(ax.imag).float() for ax in axs], 0).to(device, dtype=self.dtype)
            # Note(kamo):
            ...
            axs_pad = pad_list([torch.from_numpy(ax).float() for ax in axs], 0).to(device, dtype=self.dtype)

        if vxs[0].dtype.kind == 'c':
            vxs_pad_real = pad_list(
                [torch.from_numpy(vx.real).float() for vx in vxs], 0).to(device, dtype=self.dtype)
            xs_pad_imag = pad_list(
                [torch.from_numpy(vx.imag).float() for vx in vxs], 0).to(device, dtype=self.dtype)
            # Note(kamo):
            # {'real': ..., 'imag': ...} will be changed to ComplexTensor in E2E.
            # Don't create ComplexTensor and give it E2E here
            # because torch.nn.DataParellel can't handle it.
            ...
            vxs_pad = pad_list([torch.from_numpy(vx).float() for vx in vxs], 0).to(device, dtype=self.dtype)

        if rms[0].dtype.kind == 'c':
            rms_pad_real = pad_list(
                [torch.from_numpy(rm.real).float() for rm in rms], 0).to(device, dtype=self.dtype)
            xs_pad_imag = pad_list(
                [torch.from_numpy(rm.imag).float() for rm in rms], 0).to(device, dtype=self.dtype)
            # Note(kamo):
            # {'real': ..., 'imag': ...} will be changed to ComplexTensor in E2E.
            # Don't create ComplexTensor and give it E2E here
            # because torch.nn.DataParellel can't handle it.
            rms_pad = {'real': rms_pad_real, 'imag': rms_pad_imag}
        else:
            rms_pad = pad_list([torch.from_numpy(rm.astype(np.float32)).float() for rm in rms], 0).to(device, dtype=self.dtype)
        ...
        return axs_pad, vxs_pad, rms_pad, ilens, ys_pad
...
def train(args):
    ...
    audio_idim_list = [int(valid_json[utts[0]]['input'][i]['ashape'][-1]) for i in range(args.num_encs)]
    video_idim_list = [int(valid_json[utts[0]]['input'][i]['vshape'][-1]) for i in range(args.num_encs)]
    ...
    if (args.enc_init is not None or args.dec_init is not None) and args.num_encs == 1:
        ...
        model = model_class(audio_idim_list[0] if args.num_encs == 1 else audio_idim_list, video_idim_list[0] if args.num_encs == 1 else video_idim_list, odim, args)
    ...
    with open(model_conf, 'wb') as f:
        ...
        f.write(json.dumps((audio_idim_list[0] if args.num_encs == 1 else audio_idim_list, video_idim_list[0] if args.num_encs == 1 else video_idim_list, odim, vars(args)),
    ...
    if args.opt == 'adadelta':
        ...
        optimizer = get_std_opt(model.parameters(), args.adim, args.transformer_warmup_steps, args.transformer_lr)
    else:
    ...
    if args.pretrain_audio_model:
        logging.info('load pretrained model from %s' % args.pretrain_audio_model)
        torch_load_pretrained_audio(args.pretrain_audio_model, trainer)
        # torch_resume_pretrain(args.pretrain_audio_model, trainer)

    if args.pretrain_video_model:
        logging.info('load pretrained model from %s' % args.pretrain_video_model)
        torch_load_pretrained_video(args.pretrain_video_model, trainer)
        # torch_resume_pretrain(args.pretrain_audio_model, trainer)      
    ...
    if args.num_save_attention > 0 and args.mtlalpha != 1.0:
        data = sorted(list(valid_json.items())[:args.num_save_attention],
                      key=lambda x: int(x[1]['input'][0]['vshape'][1]), reverse=True)
    ...
    if args.batchsize == 0:
        with torch.no_grad():
            for idx, name in enumerate(js.keys(), 1):
                ...
                afeat = feat[0][0] if args.num_encs == 1 else [feat[idx][0] for idx in range(model.num_encs)]
                vfeat = feat[1][0] if args.num_encs == 1 else [feat[idx][0] for idx in range(model.num_encs)]
                rms = feat[2][0] if args.num_encs == 1 else [feat[idx][0] for idx in range(model.num_encs)]
                if args.streaming_mode == 'window' and args.num_encs == 1:
                    ...
                    nbest_hyps = model.recognize(afeat, vfeat, rms, args, train_args.char_list, rnnlm)
