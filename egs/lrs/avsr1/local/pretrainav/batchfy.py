...
def make_batchset(data, batch_size=0, max_length_in=float("inf"), max_length_out=float("inf"),
    ...
    for d in category2data.values():
        ...
        sorted_data = sorted(d.items(), key=lambda data: int(
            data[1][batch_sort_key][batch_sort_axis]['vshape'][0]), reverse=not shortest_first)
        datanew = []
        for i in sorted_data:
            if i[1]['input'][0]['ashape'][0] > 10:
                datanew.append(i)
        #[datanew.append(i) if int(i[1]['input'][0]['ashape'][0] / 4) != 0 else pass for i in sorted_data]
