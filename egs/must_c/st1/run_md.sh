...
#trans_set="dev_org.en-${tgt_lang}.${tgt_lang} tst-COMMON.en-${tgt_lang}.${tgt_lang}"
trans_set=dev.en-${tgt_lang}.${tgt_lang}
...
if [[ ${tag} == "" ]]; then
    expname=${train_set}_${tgt_case}_${backend}_$(basename ${train_config%.*})_${bpemode}${nbpe}_gpu${ngpu}
    if ${do_delta}; then
