...
if [ ${stage} -le 0 ] && [ ${stop_stage} -ge 0 ]; then
    ...
    # data cleaning
    ### local/forced_align.sh ${datadir} data/train
...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
    ...
    utils/copy_data_dir.sh data/dev.en data/train_dev.en
    utils/copy_data_dir.sh data/dev.de data/train_dev.de
...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
    ...
    for x in ${train_set} ${train_dev}; do
        ...
        local/update_json.sh --nlsyms ${nlsyms} ${feat_dir}/data.json \
