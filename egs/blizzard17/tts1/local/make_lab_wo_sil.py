...
def ignore_sil(voice_part_ana, sec=0.1):
    ...
    # check short silent
    check_list = np.ones( avn+1, np.int64 ) # 1:time to use, 0:time to ignore (=short silence)
...
def separete_vp(voice_part_old, voice_part_tmp):
    ...
    for i in range( ovn ):
        for j in range( tvn ):
                if((voice_part_tmp[j][0] < voice_part_old[i][0]) and (voice_part_old[i][0] < voice_part_tmp[j][1])):
                    voice_part_add.append(voice_part_old[i][0])
                    voice_part_add.append(voice_part_tmp[j][1])
                    if not np.any(del_list == j):
                        del_list.append(j)
                        if((voice_part_tmp[j][0] < voice_part_old[i][1]) and (voice_part_old[i][1] < voice_part_tmp[j][1])):
                            voice_part_add.append(voice_part_old[i][0])
...
def create_vp(voice_part_old, voice_part_tmp):
    ...
    for i in range( ovn ):
        for j in range( tvn ):
            if ((voice_part_old[i][0] <= voice_part_tmp[j][0]) and (voice_part_tmp[j][1] <= voice_part_old[i][1])):
    ...
    with open(out_lab_file, mode='w') as f:
        for i in range( nvn ):
            if i == 0:
                ...
                # Body of silence
                f.write('%.6f\t%.6f\t%s\n' % (np.float(voice_part_new[i-1][1]), np.float(voice_part_new[i][0]), '#'))
            #Body of voice
            f.write('%.6f\t%.6f\t%s\n' % (np.float(voice_part_new[i][0]), np.float(voice_part_new[i][1]), utt_array[i]))
        #Tail
...
def compare_vp(voice_part_old, voice_part_new):
    ...
    for i in range(vn):
        ...
        if (1.0 < diff_st):
            sys.stderr.write('Warning: StDiff[%4f][%d] is bigger than 1.\n' % (diff_st, i) )
        if (1.0 < diff_ed):
            sys.stderr.write('Warning: EdDiff[%4f][%d] is bigger than 1.\n' % (diff_ed, i) )
        if ( dur_new < 0.5 ):
            sys.stderr.write('Warning: NewDur(%4f)[%d] is smaller than 0.5.\n' % (dur_new, i) )
        if ( (diff_dur < 0) or (base_dur < diff_dur)):
            sys.stderr.write('Warning: Diff(%4f)[%d] is out of 0<x<%.2f.\n' % (diff_dur, i, base_dur) )
...
def main():
    ...
    voice_part_ana = librosa.effects.split(wav_form, top_db=40, frame_length=2048, hop_length=512)/fs
