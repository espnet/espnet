...
for l in `cat ${langs} | tr "\n" " "`; do
  ...
  # Prepare training set subsets
  for num_heldout in 100 200 400 800 1600 3200; do
      ./utils/subset_data_dir.sh ${langdata} $num_heldout ${langdata}_train-$num_heldout

    ./utils/fix_data_dir.sh ${langdata}_train-${num_heldout}
    cat ${langdata}_train-${num_heldout}/text |\
      ./utils/apply_map.pl -f 2- ${dict_l}/lexicon.txt > ${langdata}_train-${num_heldout}/text.phn
  done
