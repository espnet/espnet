...
if [ -z ${tag} ]; then
    expdir=exp/${train_set}_${etype}_e${elayers}_unit${eunits}_proj${eprojs}_d${dlayers}_unit${dunits}_mtlalpha${mtlalpha}_phoneme-weight${phoneme_objective_weight}_${opt}_sampprob${samp_prob}_bs${batchsize}_mli${maxlen_in}_mlo${maxlen_out}
    if ${do_delta}; then
