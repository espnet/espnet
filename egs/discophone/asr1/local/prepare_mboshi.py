...
def main():
    parser = ArgumentParser(description='Prepare the Mboshi data directory from Mboshi-French parallel corpus'
...
def create_kaldi_data_dir(wavs: List[Path], dest: Path):
    ...
    with open(dest / 'wav.scp', 'w') as wavscp, open(dest / 'text', 'w') as text, open(dest / 'utt2spk',
                                                                                       'w') as utt2spk:
