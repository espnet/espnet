...
if [ $# -ne 3 ]; then
  printf "\nUSAGE: %s <WSJ0 corpus-directory> <WSJ0 contaminated-folder>\n\n" `basename $0`
  exit 1;
fi
...
cat $wsj0/doc/indices/train/tr_s_wv1.ndx \
  | $local/cstr_ndx2flist.pl $wsj0 \
  | sed s/si_tr_s/$wsj0_contaminated/ \
  | sed s/.wv1/.wav/ | awk '{gsub("/wsj0","");print $0}' | sort -u > tr05_cont_$mic.flist
...
#  $local/cstr_ndx2flist.pl $wsj0 | sort > et05_orig.flist
...
#find $wsj0/wsj0/si_dt_05 -print | grep -i ".wv1" | sort > dt05_orig.flist
