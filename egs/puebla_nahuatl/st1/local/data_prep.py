...
def ELANProcess(afile, spk_info, spk_details):
    ...
    correction_channels = ({}, {})
    for tier in tiers:
        if tier.getAttribute("LINGUISTIC_TYPE_REF") not in ["UtteranceType", "Transcripción", "Traducción", "Transcripcion", "Traduccíón"] :
        ...
        if code not in spk_info:
            ...
            continue
        if tier.getAttribute("LINGUISTIC_TYPE_REF") in ["UtteranceType", "Transcripción", "Transcripcion"]:
...
def TraverseData(sound_dir, annotation_dir, target_dir, mode, speaker_info, new_data_dir, speaker_details):
    ...
    for afile in annotation_files.keys():
        ...
        segment_info, correction_segment_info = ELANProcess(afile_path, spk_info, spk_details)
        ...
        left_channel_segments_corr, right_channel_segments_corr = correction_segment_info
        for segment_number, segment in left_channel_segments.items():
            ...
            print("%s %s" % (segment_id, left_channel_segments_corr[segment_number]), file=correction_text)
        ...
        if len(right_channel_segments) > 0:
            ...
            for segment_number, segment in right_channel_segments.items():
                ...
                print("%s %s" % (segment_id, segment[2]), file=text)
                print("%s %s" % (segment_id, right_channel_segments_corr[segment_number]), file=correction_text)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-w', dest="wav_path", type=str, help="wav path", default="/export/c04/jiatong/data/Puebla-Nahuatl/Sound-files-Puebla-Nahuatl")
    ...
    parser.add_argument('-a', dest="ann_path", type=str, help="annotation path", default="/export/c04/jiatong/data/Puebla-Nahuatl/SpeechTranslation210217")
    parser.add_argument('-t', dest="target_dir", type=str, help='target_dir', default="data/nahuatl_init")
    parser.add_argument('-i', dest='speaker_info', type=str, help='speaker info file dir', default='local/speaker_wav_mapping.csv')
    ...
    TraverseData(args.wav_path, args.ann_path, args.target_dir, mode=args.mode, speaker_info=args.speaker_info,
                 new_data_dir=args.new_data_dir, speaker_details=args.speaker_details)
