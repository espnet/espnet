...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
    ...
    for x in ${train_set} ${train_dev}; do
        ...
        local/update_json.sh --text ${data_dir}/text.${case} --bpecode ${bpemodel}.model ${feat_dir}/data_${bpemode}${nbpe}.${case}.json ${data_dir} ${dict}
    done
    ...
    for rtask in fisher_dev.en fisher_dev2.en fisher_test.en; do
        feat_recog_dir=${dumpdir}/${rtask}/delta${do_delta}
        for no in 1 2 3; do
          local/data2json.sh --text data/${rtask}/text.${case}.${no} --feat ${feat_recog_dir}/feats.scp --bpecode ${bpemodel}.model \
              data/${rtask} ${dict} > ${feat_recog_dir}/data_${bpemode}${nbpe}_${no}.${case}.json
        done
    done
fi
...
if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ]; then
    ...
    (
        ...
        if [ ${rtask} = "fisher_dev.en" ] || [ ${rtask} = "fisher_dev2.en" ] || [ ${rtask} = "fisher_test.en" ]; then
            for no in 1 2 3; do
              cp ${feat_recog_dir}/data_${bpemode}${nbpe}_${no}.${case}.json ${expdir}/${decode_dir}/data_ref${no}.json
            done
        fi

        local/score_bleu.sh --case ${case} --set ${rtask} --bpe ${nbpe} --bpemodel ${bpemodel}.model ${expdir}/${decode_dir} ${dict}
