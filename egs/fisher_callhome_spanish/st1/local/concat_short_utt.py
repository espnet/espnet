...
def main():
    ...
    parser.add_argument('mapping', type=str,
                        help='mapping file to concatenate multiple utterances')
    ...
    with codecs.open(os.path.join(os.path.dirname(args.segments), 'segments'), 'w', encoding='utf-8') as f:
        for line in codecs.open(args.mapping, 'r', encoding='utf-8'):
            ...
            if len(ids.split('_')) == 1:
                ...
                segment_start, spk_start, start_t, _ = segments[(session, start_id)].split()
                segment_end, spk_end, _, end_t = segments[(session, end_id)].split()
                line_new = ' '.join(['-'.join(segment_start.split('-')[:3]) + '-' + segment_end.split('-')[-1],
                                     spk_start, start_t, end_t])
            f.write(line_new + '\n')
            ...
            if len(ids.split('_')) == 1:
                ...
                for i, id in enumerate(map(int, ids.split('_')[1:])):
                    ...
                    if i == len(ids.split('_')[1:]) - 1:
                        line_new = ' '.join(['-'.join(segment.split('-')[:3]) +
                                             '-' + segment_next.split('-')[-1], trans])
            f.write(line_new + '\n')
