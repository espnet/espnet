...
decode_config=conf/tuning/decode_ngram.yaml
...
if [ ${stage} -le 3 ] && [ ${stop_stage} -ge 3 ]; then
    ...
    lmplz --discount_fallback -o ${n_gram} <${lmdatadir}/train.txt > ${lmexpdir}/${n_gram}gram.arpa
    build_binary -s ${lmexpdir}/${n_gram}gram.arpa ${lmexpdir}/${n_gram}gram.bin
fi
...
if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ]; then
    ...
    for rtask in ${recog_set}; do
    (
        decode_dir=decode_${rtask}_$(basename ${decode_config%.*})_${ngram_weight}
        feat_recog_dir=${dumpdir}/${rtask}/delta${do_delta}
        ...
        ${decode_cmd} JOB=1:${nj} ${expdir}/${decode_dir}/log/decode.JOB.log \
            asr_recog.py \
            --config $(change_yaml.py ${decode_config} -a ngram-weight=${ngram_weight}) \
            --ngpu ${ngpu} \
            --backend ${backend} \
            --batchsize 0 \
            ...
            --api v2 \
            --ngram-model ${lmexpdir}/${n_gram}gram.bin\
