...
if [ ${stage} -le 3 ] && [ ${stop_stage} -ge 3 ]; then
    ...
    # use only 1 gpu for train.
    if [ ${ngpu} -gt 1 ]; then
	echo "LM training does not support multi-gpu. Singlu gpu will be used."
    fi
    lmgpu=1
    ${cuda_cmd} --gpu ${lmgpu} ${lmexpdir}/train.log \
	    lm_train.py \
	    	--config ${lm_config} \
		--ngpu ${lmgpu} \
		--backend ${backend} \
