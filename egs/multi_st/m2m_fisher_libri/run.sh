...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
    ...
    utils/combine_data.sh --extra_files "text.tc text.lc text.lc.rm" data/${train_set} data/tr_fisher_callhome.en data/tr_librispeech.fr
    utils/combine_data.sh --extra_files "text.tc text.lc text.lc.rm" data/${train_dev} data/dt_fisher_callhome.en data/dt_librispeech.fr
    ...
    for rtask in ${recog_set_fisher} ${recog_set_libri}; do
...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
    ...
    grep sp1.0 data/tr_fisher_callhome*/text.${case} data/tr_librispeech*/text.${case} | cut -f 2- -d " " | grep -o -P '&[^;]*;|<[^>]*>' | sort | uniq > ${nlsyms}
    cat ${nlsyms}

    echo "make a dictionary"
    echo "<unk> 1" > ${dict} # <unk> must be 1, 0 will be used for "blank" in CTC
    grep sp1.0 data/tr_fisher_callhome*/text.${case} data/tr_librispeech*/text.${case} | text2token.py -s 1 -n 1 -l ${nlsyms} | cut -f 2- -d" " | tr " " "\n" \
    ...
    # concatenate fisher and libri
    local/concat_json_multiref.py \
        ${feat_tr_dir}/data_fisher.${case}.json \
        ${feat_tr_dir}/data_libri.${case}.json \
        ${feat_tr_dir}/data_libri_gtranslate.${case}.json > ${feat_tr_dir}/data.${case}.json
    ...
    local/concat_json_multiref.py \
        ...
        ${feat_dt_dir}/data_libri.${case}.json > ${feat_dt_dir}/data.${case}.json
