...
if [ ${stage} -le 3 ]; then
    ...
    mkdir -p ${lmdatadir}
    utils_espnet/text2token.py -s 1 -n 1 data/${train_set}/text | cut -f 2- -d" " \
        > ${lmdatadir}/train.txt
    utils_espnet/text2token.py -s 1 -n 1 data/${train_dev}/text | cut -f 2- -d" " \
        > ${lmdatadir}/valid.txt
    # use only 1 gpu
    if [ ${ngpu} -gt 1 ]; then
...
if [ ${stage} -le 5 ]; then
    ...
    (
        ...
        utils_espnet/score_sclite.sh ${expdir}/${decode_dir} ${dict}
