...
if [ ${stage} -le 1 ]; then
    ...
    utils_espnet/remove_longshortdata.sh --maxframes 3000 --maxchars 400 data/train_all data/${train_set}
    utils_espnet/remove_longshortdata.sh --maxframes 3000 --maxchars 400 data/rt03 data/${train_dev}
...
if [ ${stage} -le 3 ]; then
    ...
    mkdir -p ${lmdatadir}
    utils_espnet/text2token.py -s 1 -n 1 -l ${nlsyms} data/train_all/text | cut -f 2- -d" " \
        > ${lmdatadir}/train.txt
