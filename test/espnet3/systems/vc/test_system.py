...
def test_prepare_features_end_to_end(tmp_path):
    system = VCSystem(training_config=_training_config(tmp_path))
...
def test_prepare_features_requires_config(tmp_path, missing):
    ...
    system = VCSystem(training_config=config)
    with pytest.raises(RuntimeError, match=missing):
        system.prepare_features()

    system = VCSystem(training_config=OmegaConf.create({"exp_dir": str(tmp_path)}))
...
def test_indices_are_grouped_by_pool(tmp_path, monkeypatch):
    ...
    alone is not enough.
    """
    from espnet3.systems.vc import system as system_module
    ...
    VCSystem(training_config=config).prepare_features()
