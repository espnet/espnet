...
def test_from_demo_loads_local_pack_via_from_packed(tmp_path, monkeypatch):
    ...
    def fake_from_packed(pack_dir, trust_user_code=False, config_overrides=None):
        captured["pack_dir"] = Path(pack_dir)
        captured["trust_user_code"] = trust_user_code
        captured["config_overrides"] = config_overrides
        return "packed"

    monkeypatch.setattr(InferenceModel, "from_packed", staticmethod(fake_from_packed))

    result = InferenceModel.from_demo(demo_dir)

    assert result == "packed"
    assert captured["pack_dir"] == bundle_root.resolve()
    assert captured["trust_user_code"] is False
    assert captured["config_overrides"] is None


def test_from_demo_loads_pretrained_tag_via_from_pretrained(tmp_path, monkeypatch):
    demo_dir = tmp_path / "demo"
    demo_dir.mkdir()
    (demo_dir / "demo.yaml").write_text(
        "model:\n"
    ...
    result = InferenceModel.from_demo(
        demo_dir,
        trust_user_code=False,
    ...
    assert captured["config_overrides"] == {"device": "cuda:0"}
