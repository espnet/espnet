...
def test_resolve_recipe_rejects_path_outside_egs3(fake_egs3):
    outside = fake_egs3.parent / "espnet3"
    outside.mkdir()
...
def test_inject_corpus_system_updates_existing_yaml_keys(tmp_path):
    """Updating pre-existing keys must not produce duplicate YAML keys."""
    from omegaconf import OmegaConf

    conf = tmp_path / "conf"
    conf.mkdir()
    (conf / "publication.yaml").write_text(
        "upload_model:\n  hf_repo: existing/model\n  private: false\n"
    )
    (conf / "demo.yaml").write_text(
    ...
    # OmegaConf rejects the duplicate keys generated by the former append-based
    # implementation, so successfully loading both files is the regression test.
    OmegaConf.load(conf / "publication.yaml")
    OmegaConf.load(conf / "demo.yaml")

    publication = (conf / "publication.yaml").read_text(encoding="utf-8")
    demo = (conf / "demo.yaml").read_text(encoding="utf-8")
    assert publication.count("upload_model:") == 1
    assert "hf_repo: espnet/mini_an4_asr_${exp_tag}" in publication
    assert "private: false" in publication
    assert demo.count("ui:") == 1
    assert demo.count("upload_demo:") == 1
    assert "title: mini_an4_asr demo" in demo
    assert "hf_repo: espnet/mini_an4_asr_${exp_tag}" in demo
    assert "app_script: src/app.py" in demo
    assert "update: false" in demo
