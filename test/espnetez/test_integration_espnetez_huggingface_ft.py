...
def get_data_info(task, dataset_name, model_name):
    if task in ["svs", "gan_svs"] and dataset_name.startswith("espnet"):
        data_info = {
                ...
                "score": lambda d: (
                    d["tempo"],
                    list(zip(d["note_start_times"], d["note_end_times"], d["note_lyrics"], d["note_midi"], d["note_phns"])),
                ),
                "text": lambda d: d["transcription"],
                "label": lambda d: (np.array(list(zip(d["phn_start_times"], d["phn_end_times"]))), d["phns"]),
            }
        if dataset_name == "espnet/ace-kising-segments" and model_name == "espnet/aceopencpop_svs_visinger2_40singer_pretrain":
            # Speaker-to-ID mapping (specific to ACE-KiSing for espnet/aceopencpop_svs_visinger2_40singer_pretrain)
            singer2sid = {
                "barber": 3, "blanca": 30, "changge": 5, "chuci": 19, "chuming": 4, "crimson": 1, "david": 28,
                "ghost": 27, "growl": 25, "hiragi-yuki": 22, "huolian": 13, "kuro": 2, "lien": 29, "liyuan": 9,
                "luanming": 21, "luotianyi": 31, "namine": 8, "orange": 12, "original": 32, "qifu": 16,
                "qili": 15, "qixuan": 7, "quehe": 6, "ranhuhu": 11, "steel": 26, "tangerine": 23,
                "tarara": 20, "tuyuan": 24, "wenli": 10, "xiaomo": 17, "xiaoye": 14, "yanhe": 33,
                "yuezhengling": 34, "yunhao": 18,
            }
            data_info["sids"] = lambda d: np.array([singer2sid[d["singer"]]]) if "singer" in d else None
        return data_info
    else:
        raise NotImplementedError(f"Data mapping not implemented for task '{task}' and dataset '{dataset_name}'.")

if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--task", type=str, required=True, choices=TASK_CLASSES)
    parser.add_argument("--model_name", type=str, required=True, help="Model name from ESPnet Model Zoo")
    parser.add_argument("--dataset_name", type=str, required=True, help="Dataset name from HuggingFace Dataset Hub")
    parser.add_argument("--use_ez_preprocessor", action="store_true", default=False, help="Use ESPnetEZ preprocessor")
    parser.add_argument("--output_dir", type=Path, required=True, help="Directory for outputs")
    parser.add_argument("--cache_dir", type=Path, default=None, help="Directory for cached files")
    parser.add_argument("--run_finetune", action="store_true", default=False, help="Flag to run fine-tuning")
    ...
    dataset = load_dataset(args.dataset_name, cache_dir=args.cache_dir, split="train[:100]").cast_column("audio", Audio(sampling_rate=pretrain_config["fs"]))
