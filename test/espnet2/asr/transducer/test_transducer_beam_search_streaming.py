...
def test_transducer_beam_search_streaming(rnn_type, search_params):
    token_list = ["<blank>", "a", "b", "c", "<sos>"]
    vocab_size = len(token_list)
    beam_size = 1 if search_params["search_type"] in ["greedy"] else 2
    ...
    with torch.no_grad():
        _ = beam(enc_out, is_final=True, incremental_decode=True)
