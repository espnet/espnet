"""Frozen copy of the pre-batching `BatchBeamSearch`, for regression testing.

Byte for byte `espnet2/legacy/nets/batch_beam_search.py` as it stood at
`ec5632b16`, before it learned to decode a batch of utterances. Only this
docstring differs; the class is still called `BatchBeamSearch`, and the test
imports it under an alias.

It exists so that `test_batch_beam_search_reference.py` can assert that the
current implementation still reproduces the old one token for token and score
for score when a single utterance is decoded. Comparing against the non-batch
`BeamSearch` only gives that guarantee transitively, over the narrow set of
configurations `test_batch_beam_search.py` covers.

DO NOT "fix", reformat or refactor this file to track changes made to the real
implementation -- its whole value is that it does not move. If a change to
`BatchBeamSearch` is deliberately meant to alter single-utterance results,
update the test to say so rather than editing this reference.
"""
