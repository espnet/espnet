...
def _state_tensors(hyp):
    """Yield every tensor held by a hypothesis's scorer states."""
    stack = [hyp.states, hyp.yseq, hyp.hs]
    while stack:
        obj = stack.pop()
...
@pytest.mark.parametrize("ctc_weight", [0.0, 0.5])
def test_finished_hypotheses_do_not_pin_the_running_batch(ctc_weight):
    ...
    encs = encs * 3  # 6 utterances, so the amplification would be obvious
    batched = BatchBeamSearch(beam_size=3, **common)
    batched.eval()
    padded, lengths = _pad(encs, device, dtype)
    with torch.no_grad():
        results = batched(x=padded, x_lengths=lengths, maxlenratio=0.0, minlenratio=0.0)

    checked = 0
    for nbest in results:
        for hyp in nbest:
            for tensor in _state_tensors(hyp):
                needed = tensor.numel() * tensor.element_size()
                alive = tensor.untyped_storage().nbytes()
                assert alive == needed, (
                    f"a finished hypothesis keeps {alive} bytes alive for a "
                    f"{needed}-byte tensor ({alive / max(needed, 1):.0f}x)"
                )
                checked += 1
    assert checked > 0, "no state tensors were inspected"
