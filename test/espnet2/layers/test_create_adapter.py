...
@pytest.mark.parametrize(
    "model, adapter, adapter_conf", [(init_S3prl_model(), "houlsby", {"bottleneck":64, "target_layers":[]})]
)
def test_create_adapter_houslby(
    ...
    create_adapter(
        model=model, adapter=adapter, adapter_conf=adapter_conf
    )
    assert isinstance(
...
@pytest.mark.parametrize(
    "model, adapter, adapter_conf", [(init_decoder_model(), "lora", {"rank":2, "alpha":4, "target_modules" : ["linear_q"]})]
)
...
if __name__ == '__main__':
    test_create_adapter_houslby(init_S3prl_model(), "houlsby", {"bottleneck":64, "target_layers":[]})
    print("Houlsby test passed")
    test_create_adapter_lora(init_decoder_model(), "lora", {"rank":2, "alpha":4, "target_modules" : ["linear_q"]})
