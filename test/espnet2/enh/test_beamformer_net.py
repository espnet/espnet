...
def test_beamformer_net_consistency(
    ...
    # random_input_torch=torch.from_numpy(random_input_numpy-1.0).float() + 1.0 # torch.float32
    random_input_torch=torch.from_numpy(random_input_numpy).float() # torch.float32
    random_input_numpy=torch.from_numpy(random_input_numpy.astype('float32')) #np.float64-->np.float32-->torch.float32
    est_speech_numpy, *_ = model(
        random_input_numpy, ilens=torch.LongTensor([16, 12])
    )

    est_speech_torch, *_ = model(
        random_input_torch, ilens=torch.LongTensor([16, 12])
    )
    assert (est_speech_torch[0]-est_speech_numpy[0]).abs().mean()<1e-3
    assert np.abs((est_speech_torch[-1]-est_speech_numpy[-1]).detach().numpy()).mean()<1e-3
