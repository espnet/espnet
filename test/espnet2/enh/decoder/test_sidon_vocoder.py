...
from espnet2.enh.decoder.sidon_vocoder import (
    VOCODERS,
    SidonHiFiGANVocoder,
    SidonVocoder,
    build_vocoder,
...
def test_dac_decoder_remove_weight_norm_keeps_output():
    vocoder = SidonVocoder(input_dim=8, channels=64, rates=[2, 2]).eval()
    x = torch.randn(1, 8, 6)
    with torch.no_grad():
        before = vocoder(x)
...
def test_build_vocoder_registry():
    ...
    assert isinstance(dac, SidonVocoder)
    ...
    assert isinstance(hifigan, SidonHiFiGANVocoder)
