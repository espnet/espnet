...
def test_complex_norm_keeps_epsilon_for_legacy_input_only():
    torch_complex = pytest.importorskip("torch_complex")
    from espnet2.enh.layers.complex_utils import EPS, complex_norm

    zero = torch.zeros(2, 3, dtype=torch.complex64)
    legacy = torch_complex.tensor.ComplexTensor(zero.real, zero.imag)
    # torch_complex callers always got sqrt(sum|c|^2 + EPS); native callers get
    # the plain norm
