...
from egs3.voxlingua107.esp2_lid.src import (
    babel_prepare,
    fleurs_prepare,
    ml_superb2_prepare,
)
from egs3.voxlingua107.esp2_lid.src.prepared_dataset import Dataset
from egs3.voxlingua107.esp2_lid.src.voxpopuli_prepare import (
    annotations,
    download_recordings,
)
...
def test_voxpopuli_selection_and_archive_download(tmp_path, monkeypatch):
    ...
    from egs3.voxlingua107.esp2_lid.src import voxpopuli_prepare

    annotation_dir = destination / "annotations"
    annotation_dir.mkdir()
    # Official annotations can reuse an utterance ID in different recordings.
    second_audio = destination / "raw_audios/original/2013/2013-second_original.ogg"
    sf.write(second_audio, np.full(16000, 0.25), 16000, format="WAV")
    for language in ("en", "fr"):
        ...
        with gzip.open(annotation_dir / f"asr_{language}.tsv.gz", "wt") as output:
            ...
            writer.writerow(row)
            if language == "en":
                writer.writerow({**row, "session_id": "2013-second"})
    calls = []
    monkeypatch.setattr(
        voxpopuli_prepare,
        "download_recordings",
    ...
    assert calls == [{"2013-session", "2013-second"}]
    ds = Dataset(tmp_path / "prepared/test/manifest.tsv")
    assert [ds[i]["lid_labels"] for i in range(len(ds))] == ["eng", "eng", "fra"]
    assert len({example.utt_id for example in ds.examples}) == 3
