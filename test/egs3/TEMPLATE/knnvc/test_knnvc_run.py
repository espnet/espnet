from egs3.TEMPLATE.knnvc.run import DEFAULT_STAGES
...
def test_load_default_config_train_contains_expected_defaults() -> None:
    cfg = load_default_config("training.yaml", "egs3.TEMPLATE.knnvc")
    assert (
...
def test_load_default_config_infer_contains_expected_targets() -> None:
    cfg = load_default_config("inference.yaml", "egs3.TEMPLATE.knnvc")
    assert cfg.output_fn == "egs3.TEMPLATE.knnvc.src.inference.build_output"
...
def test_build_output_wraps_waveform() -> None:
    from egs3.TEMPLATE.knnvc.src.inference import build_output
