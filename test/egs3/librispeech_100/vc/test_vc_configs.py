...
def test_recipe_training_config_merges_over_template(tmp_path: Path) -> None:
    recipe_config = _REPO_ROOT / "egs3/librispeech_100/vc/conf/training.yaml"
    cfg = load_and_merge_config(
        recipe_config, "training.yaml", default_package="egs3.TEMPLATE.vc"
    )
    assert (
        cfg.model._target_
        == "espnet3.systems.vc.models.knnvc.vocoder.KNNVCVocoderModel"
    )
...
def test_recipe_publication_config_keeps_features_out_of_the_bundle() -> None:
    cfg = load_and_merge_config(
        _REPO_ROOT / "egs3/librispeech_100/vc/conf/publication.yaml",
        "publication.yaml",
        default_package="egs3.TEMPLATE.vc",
    )
    ...
    inference = load_and_merge_config(
        _REPO_ROOT / "egs3/librispeech_100/vc/conf/inference.yaml",
        "inference.yaml",
        default_package="egs3.TEMPLATE.vc",
    )
...
def test_pretrained_inference_config_uses_a_released_generator() -> None:
    ...
    """
    cfg = load_and_merge_config(
        _REPO_ROOT / "egs3/librispeech_100/vc/conf/inference_pretrained.yaml",
        "inference.yaml",
        default_package="egs3.TEMPLATE.vc",
    )
