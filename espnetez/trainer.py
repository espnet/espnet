...
class Trainer:
    ...
    def train(self):
        ...
        if not os.path.exists(os.path.join(self.stats_dir, "train")) or \
                len(glob.glob(os.path.join(self.stats_dir, "train", "*shape"))) == 0:
            self.collect_stats()

        # after collect_stats, define shape files
        self.train_config.train_shape_file = glob.glob(os.path.join(self.stats_dir, "train", "*shape"))
        self.train_config.valid_shape_file = glob.glob(os.path.join(self.stats_dir, "valid", "*shape"))
