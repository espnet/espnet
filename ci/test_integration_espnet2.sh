...
# NOTE(LiyongGuo): pytorch 1.3 is not supported by k2
#   so we test this recipe using only pytorch > 1.3 here.
#   See also: https://github.com/k2-fsa/k2/issues/822
if python3 -c 'import torch as t; from distutils.version import LooseVersion as L; assert L(t.__version__) > L("1.4")' &> /dev/null; then
