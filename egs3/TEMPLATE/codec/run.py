...
def build_parser(
    ...
    Returns:
        argparse.ArgumentParser: Parser with ``--stages``, the five per-stage
        config options (``--training_config``, ``--inference_config``,
        ``--metrics_config``, ``--publication_config``, ``--demo_config``),
        ``--dry_run`` and ``--write_requirements``. Recipes may add their own
        arguments to it before parsing.
...
def main(
    ...
    Examples:
        ...
        parser = build_parser(stages=DEFAULT_STAGES)
        args, _ = parse_cli_and_stage_args(parser, stages=DEFAULT_STAGES)
        main(args=args, system_cls=CodecSystem, stages=DEFAULT_STAGES)
        ```
    """
