"""ESC-50 dataset implementation backed by TSV manifests."""
...
from egs3.esc50.cls.dataset.builder import ESC50Builder, resolve_data_root
...
class ManifestEntry:
    """One manifest row: utterance id, wav path, and sound class label."""
...
class ESC50Dataset(TorchDataset):
    """ESC-50 dataset that returns ``{"speech", "label"}`` samples.

    The audio read here is what ``build`` wrote under the data root, so it is
    already 16 kHz mono and no resampling happens per sample.
    ...
    Examples:
        >>> dataset = ESC50Dataset(split="test")
