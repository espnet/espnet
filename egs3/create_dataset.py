        ...
        for file in files:
            if file.endswith(".trans.txt"):
                ...
                for utt_id, text in transcript_dict.items():
                    ...
                    examples["audio"].append({"path": flac_path, "bytes": open(flac_path, "rb").read()})
...
dataset_dict = DatasetDict({
    "train-clean-100": Dataset.from_dict(gather_examples(f"{BASE_PATH}/train-clean-100")),
...
for split in dataset_dict:
    dataset_dict[split] = dataset_dict[split].cast_column("audio", datasets.Audio(decode=True))
