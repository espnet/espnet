"""LibriTTS-specific TTS system implementation.

This module adds LibriTTS-specific stages on top of the generic
``espnet3.systems.tts.system.TTSSystem`` base: computing x-vectors,
removing long-short utterances, and creating token lists. GAN-TTS
training/collect_stats support is inherited from the base system.
"""
...
from espnet3.systems.tts.system import TTSSystem as _BaseTTSSystem

logger = logging.getLogger(__name__)


class TTSSystem(_BaseTTSSystem):
    """LibriTTS-specific system.
    ...
    def remove_long_short(self, *args, **kwargs):
        ...
        It checks audio durations via ``soundfile`` and saves filtered
        ...
        Notes:
            Configuration should include:
                training_config.remove_long_short.min_wav_duration:
                    Minimum duration in seconds.
                training_config.remove_long_short.max_wav_duration:
                    Maximum duration in seconds.
                training_config.remove_long_short.save_path: Directory to
                    save filtered manifests.
                training_config.remove_long_short.splits: Splits to
                    process (train, valid, test).
                training_config.remove_long_short.manifest_paths: Optional
                    dict of split to manifest path (default:
    ...
    def create_token_list(self, *args, **kwargs):
        ...
        Notes:
            Configuration should include:
                training_config.create_token_list.save_path: Path to save
                    the token list file.
                training_config.create_token_list.manifest_path: Path to
                    the training manifest file (default:
        ...
        if not manifest_path.exists():
            raise RuntimeError(
                "Manifest file not found for token list creation: "
                f"{manifest_path}. Please ensure the manifest file is "
                "generated and the path is correct."
            )

        # Text processing defaults mirror espnet2's defaults where applicable.
