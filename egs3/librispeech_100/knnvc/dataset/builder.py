"""LibriSpeech 100h dataset builder for the kNN-VC recipe.

Raw-passthrough builder: kNN-VC reads the original LibriSpeech directory
layout directly (``train-clean-100`` for vocoder training, ``dev-clean`` for
conversion pairs), so no manifests are produced here. The WavLM features the
vocoder trains on are written by the ``prepare_features`` stage instead.
"""
...
def resolve_librispeech_root(data_dir: str | Path) -> Path:
    ...
    Returns:
        The ``LibriSpeech`` directory.

    Raises:
        FileNotFoundError: If neither layout is found.
    """
...
def resolve_source_root(
    ...
    Candidates are checked in order: ``<recipe_dir>/download``, the explicit
    ``source_dir``, then the ``LIBRISPEECH`` environment variable.

    Raises:
        FileNotFoundError: If no candidate contains a LibriSpeech tree.
    """
    checked: list[str] = []
...
class LibriSpeech100Builder(DatasetBuilder):
    """Validate LibriSpeech source availability for the kNN-VC recipe.

    ``is_built``/``build`` delegate to the source checks because the recipe
    has no build artifacts of its own; features come from ``prepare_features``.
    """
    ...
    def prepare_source(
        ...
        **_kwargs,
        ...
        Raises:
            FileNotFoundError: If the LibriSpeech root or a required split
                (``train-clean-100``, ``dev-clean``) is missing.
        """
        recipe_root = Path(recipe_dir).resolve()
        source_root = resolve_source_root(recipe_root, source_dir=source_dir)
    ...
    def is_built(
        ...
        **_kwargs,
        ...
        return self.is_source_prepared(recipe_dir=recipe_dir, source_dir=source_dir)
