...
if [ $num_spkrs -eq 1 ]; then
    echo "write a CER (or TER) result in ${dir}/result_r${ind_r}h${ind_h}.txt"
    grep -e Avg -e SPKR -m 2 ${dir}/result_r${ind_r}h${ind_h}.txt
    ...
    echo "Number of speakrs is not valid."
    exit 1;
fi
...
if ${wer}; then
    ...
    if [ $num_spkrs -eq 1 ]; then
        echo "write a WER result in ${dir}/result_r1h1.wrd.txt"
        grep -e Avg -e SPKR -m 2 ${dir}/result_r1h1.wrd.txt
    elif [ $num_spkrs -eq 2 ]; then
