...
def compute_cosine_distances(
    ...
    Examples:
        ...
        >>> pool = torch.tensor([[1.0, 0.0], [-1.0, 0.0]])
        >>> compute_cosine_distances(source, pool)
        tensor([[0., 2.],
                [1., 1.]])

        Row 0 of ``source`` matches row 0 of ``pool`` exactly (distance 0) and
        opposes row 1 (distance 2); row 1 is orthogonal to both (distance 1).
    """
