...
class STOI(BaseMetric):
    ...
    def __call__(
        ...
        scores = []
        with (test_dir / "stoi").open("w", encoding="utf-8") as f:
            for i in trange(0, len(pairs), self.batch_size):
                ...
                for uid, ref_item, inf_item in zip(batch_uids, ref_audios, inf_audios):
                    ...
                    scores.append(score)
                    metric_name = "ESTOI" if self.extended else "STOI"
                    f.write(f"{uid} {score[metric_name]}\n")

        if not scores:
            raise ValueError("No scores were computed. Please check the input data.")

        if self.extended:
            return {"ESTOI": float(np.mean([s["ESTOI"] for s in scores]))}
        return {"STOI": float(np.mean([s["STOI"] for s in scores]))}
