"""Generic TTS system implementation for ESPnet3.

Provides the reusable collect_stats/train stages shared by any TTS recipe,
including automatic dispatch to a GAN-specific Lightning trainer for
GAN-TTS models. Recipe-specific stages (e.g. x-vector extraction, manifest
filtering, token-list creation) belong in a recipe-local subclass.
"""
...
class TTSSystem(BaseSystem):
    """Generic TTS-specific system.

    Provides ``collect_stats`` and ``train`` stages, dispatching to a
    GAN-specific Lightning trainer automatically when the configured model
    is a GAN-TTS model. Recipe-specific extra stages should be added by
    ...
    def collect_stats(self, *args, **kwargs):
        ...
        Notes:
            The ``normalize: null`` pattern from recipe configs is
            intentionally preserved — no normalization is applied during
            stats collection.
    ...
    def train(self, *args, **kwargs):
        ...
        is used automatically; for all other models,
        ``ESPnet3LightningTrainer`` is used.
