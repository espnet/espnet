"""Intent classification accuracy for the esp2_slu system."""
...
class IntentAccuracy(BaseMetric):
    ...
    utterance counts as correct only on an exact label match. The recipe's
    ``output_fn`` has already split that token off, so this metric reads the
    dedicated ``*_intent`` SCP fields rather than re-parsing the full
    hypothesis.
    ...
    def __init__(
        ...
        """Store the SCP field names this metric reads and what it writes."""
