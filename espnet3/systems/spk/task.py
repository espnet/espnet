...
class SpeakerTask(AbsTask):
    ...
    def optional_data_names(
        ...
        During validation/inference, ``speech2`` and ``trial`` are used
        for computing EER over trial pairs.
        """
    ...
    def build_model(cls, args: argparse.Namespace) -> ESPnetSpeakerModel:
        """Build the ESPnet speaker model from parsed arguments."""
        if args.frontend is not None:
            ...
            if input_size is None:
                raise ValueError(
                    "input_size must be specified when frontend is not used."
                )
