"""Shared GAN adapter for ESPnet3's Lightning integration.

Any ESPnet2 model implementing ``AbsGANESPnetModel`` (``forward_generator``
flag, dict return with ``optim_idx``) needs this adapter instead of the base
``ESPnetLightningModule``, since that base class expects a single
``model(**batch)`` call returning ``(loss, stats, weight)``. This module is
shared across task families (e.g. GAN-TTS, GAN-based neural codec) so the
adapter is written once against the ``AbsGANESPnetModel`` contract rather
than duplicated per task.
"""
...
class GANLightningModule(ESPnetLightningModule):
    """Adapt ``AbsGANESPnetModel`` outputs to ESPnet3's named optimizer path."""
        ...
        if isinstance(optim_idx, int):
            ...
            if optim_idx.dim() >= 2:
                raise AssertionError(
                    "GAN optim_idx must be an int or a 0/1-dim tensor, "
                    f"but got {optim_idx.dim()} dims."
                )
            if optim_idx.dim() == 1:
                if optim_idx.numel() == 0:
                    raise AssertionError("GAN optim_idx tensor must not be empty.")
                first = optim_idx[0]
                if not torch.all(optim_idx == first):
                    raise AssertionError(
                        "GAN optim_idx 1-dim tensor must have identical values."
                    )
            ...
            raise AssertionError(
                "GAN optim_idx must be an int or torch.Tensor, "
                f"but got {type(optim_idx)!r}."
            )

        if normalized not in self._GAN_STEP_NAMES:
            raise AssertionError(
                "GAN models must set optim_idx to 0 (generator) or 1 "
    ...
    def _forward_gan_turn(
        ...
        if not isinstance(loss, torch.Tensor):
            raise AssertionError(
                f"GAN '{name}' loss must be a tensor, but got {type(loss)!r}."
            )

        stats = output.get("stats")
        if not isinstance(stats, dict):
            raise AssertionError(
                f"GAN '{name}' stats must be a dict, but got {type(stats)!r}."
            )
