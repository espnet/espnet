...
from espnet3.systems.vc.models.knnvc.checkpoint import load_generator_state_dict
...
class KNNVCModel:
    ...
    named by ``inference_config.input_key``; the recipe dataset provides
    ``speech`` (source utterance), ``reference_speech`` (target speaker's
    utterances) and ``target_speaker`` (a cache key), see
    ``egs3/librispeech_100/vc``.

    Args:
        vocoder_checkpoint: Path or URL of the HiFi-GAN generator weights. Any
            layout accepted by
            :func:`espnet3.systems.vc.models.knnvc.checkpoint.load_generator_state_dict`
        ...
        encoder: Optional frozen feature encoder, either a built instance or a
            ...
            :class:`espnet3.systems.vc.models.knnvc.wavlm_encoder.WavLMEncoder`
        ...
        wavlm_checkpoint: Path or URL of the WavLM checkpoint. Ignored when
            ``encoder`` is given.
    ...
    def __init__(
        ...
        wavlm_checkpoint: str | Path = WAVLM_LARGE_URL,
        layer: int = 6,
    ...
    def build_encoder(
        ...
        Args:
            ...
            wavlm_checkpoint: WavLM checkpoint used when ``encoder`` is ``None``.
