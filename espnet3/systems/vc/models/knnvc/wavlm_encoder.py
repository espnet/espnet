...
from espnet3.systems.vc.models.knnvc.vendored_wavlm import WavLM, WavLMConfig
...
class WavLMEncoder(torch.nn.Module):
    ...
    - the ``prepare_features`` stage of
      :class:`espnet3.systems.vc.system.VCSystem`, to precompute (optionally
      prematched) vocoder training features for every corpus utterance;
    - :class:`espnet3.systems.vc.models.knnvc.model.KNNVCModel` at inference time, to
    ...
    Args:
        checkpoint: Path or URL of the WavLM checkpoint in the original
            ...
            entries. Defaults to the WavLM-Large checkpoint redistributed by the
            kNN-VC authors.
    ...
    def __init__(
        self,
        checkpoint: str | Path = WAVLM_LARGE_URL,
        layer: int = 6,
