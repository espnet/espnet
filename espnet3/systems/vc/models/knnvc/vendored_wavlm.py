...
# Processing (https://arxiv.org/abs/2110.13900.pdf)
...
# --------------------------------------------------------
"""Vendored WavLM model definition used by the kNN-VC voice conversion system.

Merged copy of ``wavlm/modules.py`` and ``wavlm/WavLM.py`` from the official
kNN-VC repository (https://github.com/bshall/knn-vc), which itself carries
them over unchanged from microsoft/unilm. Both files are MIT licensed. They
depend only on ``torch`` and ``numpy``, so vendoring them removes the need
for a ``fairseq``/``s3prl`` install just to encode speech.

The model logic is unchanged -- only formatting, to satisfy ESPnet's linters
-- so the public ``WavLM-Large.pt`` checkpoint (``{"cfg": ..., "model": ...}``)
loads via ``WavLM(WavLMConfig(ckpt["cfg"]))`` and reproduces the upstream
features.

Use :class:`espnet3.systems.vc.models.knnvc.wavlm_encoder.WavLMEncoder` rather
...
def compute_mask_indices(
    ...
    Args:
        ...
        mask_prob: probability for each token to be chosen as start of the span to
            be masked. this will be multiplied by number of timesteps divided by
            length of mask span to mask approximately this percentage of all
            elements. however due to overlaps, the actual number will be smaller
            (unless no_overlap is True)
        ...
        no_overlap: if false, will switch to an alternative recursive algorithm
            that prevents spans from overlapping
        min_space: only used if no_overlap is True, this is how many elements to
            keep unmasked between spans
    """
...
class WavLMConfig:
    def __init__(self, cfg=None):
        self.extractor_mode: str = (
            ...
            # groups in the first conv block, whereas layer_norm has layer norms
            # in every block (meant to use with normalize=True)
            "default"
        )
        ...
        self.conv_feature_layers: str = (
            # string describing convolutional feature extraction layers in form
            # of a python list that contains [(dim, kernel_size, stride), ...]
        ...
        self.mask_other: float = (
            # secondary mask argument (used for more complex distributions), see
            # help in compute_mask_indices
            0
        )
