...
server an assistant calls, a notebook, and a hosted Space. ESPnet offers six
tasks that way today — transcription, translation, phone recognition,
alignment, synthesis and enhancement — and the point of this page is that the
seventh should cost an afternoon rather than a design discussion.
...
**A task is a verb, and the same verb everywhere it appears.**

| task | CLI | MCP tool | Space | notebook |
|---|---|---|---|---|
| transcription | `espnet transcribe` | `transcribe` | [owsm-ctc-v4](https://huggingface.co/spaces/espnet/owsm-ctc-v4), [owsm-v4](https://huggingface.co/spaces/espnet/owsm-v4) | `s2t_asr` / `asr_demo` |
| translation | `espnet translate` | `translate` | the same two | `st_demo` |
| phone recognition | `espnet phonemize` | `phonemize` | [powsm-ctc](https://huggingface.co/spaces/espnet/powsm-ctc) | `s2t_pr_demo` |
| alignment | `espnet align` | `align` | [forced-alignment](https://huggingface.co/spaces/espnet/forced-alignment) | `s2t_align_demo` |
| synthesis | `espnet synthesize` | `synthesize` | [ljspeech-vits](https://huggingface.co/spaces/espnet/ljspeech-vits) | `tts_demo` |
| enhancement | `espnet enhance` | `enhance` | [universal-se](https://huggingface.co/spaces/espnet/universal-se) | `enh_demo` |
...
- **the verb** names what a person asks for, and so names the CLI subcommand
  and the MCP tool. `asr` and `tts` were the names once and still work, which
  is the only reason there are aliases at all.
...
- **`app.py`** — loads a model, picks a device, asks for a slice of GPU time.
  ...
  speech-to-text Space and `espnet demo`. Write your own page only when the
  task's shape is different — alignment does, with two inputs and a table —
