...
def speechmatics_asr(speech_file):
    with open(speech_file, 'rb') as f:
        files = {
            'data_file': f,
            'config': (None, '{"type": "transcription",'
                             '"transcription_config": {"operating_point": "standard", "language": "en" }}'),
        }
        response = requests.post(url=SPEECHMATICS_POST_URL, headers=SPEECHMATICS_HEADERS, files=files)
    if response.ok:
        job_id = literal_eval(response.text)['id']
        kwargs = {'url': SPEECHMATICS_GET_URL.format(job_id=job_id),
                  'params': {'format': 'txt'},
...
def get_true_normalized_text(metadata_csv):
    import csv
    df = pd.read_csv(metadata_csv, sep='|', quoting=csv.QUOTE_NONE, names=['wav_name', 'raw_text', 'clean_text'])
...
def compute_wer(transcripts_csv, wav_name_to_text, wav_suffix='_gen', verbose=False):
    ...
    for wav_name, transcript in zip(trans_df.wav_name, trans_df.transcript):
        ...
        if verbose:
            print(wav_name, f'True text: {true_text}', f'Transcript: {transcript}', wer, sep='\n')
        else:
            print(wav_name, wer)
...
def compute_f0_rmse(wav_dir, wav_suffix='_gen', ground_truth_dir=GROUND_TRUTH_DIR, verbose=False):
...
if __name__ == '__main__':
    exp_dirs = ['fastspeech2_varlr_40to0max75',
                'fastspeech2_varlr_40max75',
                'fastspeech2_varlr_20to0max75',
                'fastspeech2_varlr_20max75',
                'fastspeech2_0']

    wav_dirs = [f'/home/perry/PycharmProjects/espnet/egs2/ljspeech/tts1/exp/' \
                f'{exp_dir}/decode_fastspeech_valid.loss.best/eval1_parallel_wavegan'
                for exp_dir in exp_dirs]

    wav_name_to_text = get_true_normalized_text('/home/perry/PycharmProjects/LJSpeech-1.1/metadata.csv')
    ...
    transcribe_dir('/home/perry/PycharmProjects/LJSpeech-1.1/test_wavs', asr_func=speechmatics_asr)
    compute_wer('/home/perry/PycharmProjects/LJSpeech-1.1/test_wavs/transcripts.csv', wav_name_to_text, verbose=True)
