...
if [ ! -z ${bpecode} ]; then
    ...
    utils_espnet/text2token.py -s 1 -n 1 -l ${nlsyms} ${dir}/text > ${tmpdir}/token.scp
else
    utils_espnet/text2token.py -s 1 -n 1 ${dir}/text > ${tmpdir}/token.scp
fi
...
utils_espnet/mergejson.py --verbose ${verbose} ${tmpdir}/*.json
