...
for my $option (keys %cli_options) {
  ...
  if ($option eq "gpu" and $value > 0) {
    $use_gpu = 1;
  } elsif ($option eq "time") {
    $job_time = $value;
  } elsif ($option eq "gpu_limit") {
    $gpu_limit = $value;
  } elsif ($option eq "cpu_limit") {
    $cpu_limit = $value;
  } elsif ($option eq "num_threads" or $option eq "num_nodes") {
    $resource_multiplier *= $value;
  }
}

# Check job time limit, refuse to submit job if it's over a certain threshold
...
if ($job_time eq "0") {
    print STDERR "SLURM --time=0 option not allowed\n";
    exit(1);
}
# parse time string into hours
my $hours = 0.0;
my @time = split('-', $job_time);
my $size = @time;
...
my $max_time_limit = $cpu_limit;
if ($use_gpu > 0) {
  $max_time_limit = $gpu_limit;
}
if ($hours > $max_time_limit) {
    print STDERR "$0: Job time ($hours hours) exceeds the limit ($max_time_limit hours), ";
    print STDERR "cancelling job submission\n";
    exit(1);
}
