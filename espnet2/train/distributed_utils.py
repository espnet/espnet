...
def resolve_distributed_mode(args) -> bool:
    ...
    if distributed and args.dist_init_method == "env://":
        if get_master_addr(args.dist_master_addr, args.dist_launcher) is None:
            raise RuntimeError(
...
def get_rank(prior=None, launcher: str = None) -> Optional[int]:
    if prior is None:
        ...
        elif launcher == "slurm" and is_in_slurm_step():
...
def get_world_size(prior=None, launcher: str = None) -> Optional[int]:
    if prior is None:
        if torch.distributed.is_initialized():
...
def get_local_rank(prior=None, launcher: str = None) -> Optional[int]:
    if prior is None:
        if launcher == "slurm" and is_in_slurm_step():
...
def get_node_rank(prior=None, launcher: str = None) -> Optional[int]:
    ...
    if prior is not None:
        ...
        assert os.environ["SLURM_STEP_NUM_NODES"] == os.environ["SLURM_NTASKS"]
