        ...
        if self.train or self.label_downsampling > 1:
            for uid, sample in data:
                ...
                if self.label_downsampling > 1 and label is not None:
                    if self.input_emb == "subsampling":
                        label = label[:-2:self.label_downsampling//2][:-2: self.label_downsampling//2]
                    else:
...
def _crop_audio_label(
    ...
    label_offset = max(
        math.floor(
            (label_offset) / (window_shift * sample_rate)
        )
        + 1,
        0,
    )
    num_label = (
        math.floor(
            (num_label) / (window_shift * sample_rate)
        )
        + 1
    )
