...
class MERTFrontend(AbsFrontend):
    ...
    def forward(
        ...
        """
        with torch.no_grad():
            mask = torch.zeros_like(input, dtype=torch.long)
            for i in range(input.shape[0]):
                mask[i, input_lengths[i] :].fill_(1)

            feats, feats_padding_mask = [], []
            for start in range(0, input.size(1), self.max_chunk):
                input_chunk = input[:, start : start + self.max_chunk]
                feats = self.model(
