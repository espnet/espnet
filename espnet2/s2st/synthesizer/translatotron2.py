...
class GaussianUpsampling(nn.Module):
    ...
    def forward(self, encoder_outputs, durations, vars, input_lengths=None):
        ...
        t = torch.arange(frame_length, device=encoder_outputs.device).expand(batch_size, hidden_length, frame_length).float()  # [batch_size, hidden_length, frame_length]
        vars = vars.view(batch_size, -1, 1) # [batch_size, hidden_length, 1]


        w_t = -0.5 * (np.log(2.0 * np.pi) + torch.log(vars) + torch.pow(t - c, 2) / vars) # [batch_size, hidden_length, frame_length]

        if input_lengths is not None:
            input_masks = ~self.get_mask_from_lengths(input_lengths, hidden_length) # [batch_size, hidden_length]
        ...
        encoder_upsampling_outputs = torch.bmm(w_t.transpose(1, 2), encoder_outputs)  # [batch_size, frame_length, encoder_hidden_size]
