...
class DecoderLayer(nn.Module):
    ...
    def forward(
        ...
        if self.concat_after:
            x_concat = torch.cat(
                (x, self.src_attn(x, memory, memory, memory_mask, attn_logit_bias=xattn_logit_bias)), dim=-1
            )
            x = residual + self.concat_linear2(x_concat)
        else:
            x = residual + self.dropout(self.src_attn(x, memory, memory, memory_mask, attn_logit_bias=xattn_logit_bias))
        if not self.normalize_before:
            x = self.norm2(x)
