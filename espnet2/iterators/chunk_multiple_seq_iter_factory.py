...
class ChunkIterFactory(AbsIterFactory):
    ...
    def __init__(
        ...
        self.per_batch_iter_factory = SequenceIterFactory(
            dataset=dataset,
            batches=batches,
    ...
    def build_iter(
        ...
        per_batch_loader = self.per_batch_iter_factory.build_iter(epoch, shuffle)
        ...
        for ids, in_batch in per_batch_loader:

            # Get keys of sequence data
            sequence_keys = []
            for key in in_batch:
                if key + "_lengths" in in_batch:
                    sequence_keys.append(key)

            L = max(in_batch[f"{sequence_keys[0]}_lengths"])
            # Select chunk length
            chunk_lengths = [lg for lg in self.chunk_lengths if lg < L]
            if len(chunk_lengths) == 0:
                logging.warning(
                    f"The max length of '{ids}' is {L}, but it is shorter than "
            ...
            for index, id_ in enumerate(ids):
                tmp_L = in_batch[f"{sequence_keys[0]}_lengths"][index]
                if W > tmp_L:
                    logging.warning(
                        f"The max length of '{id_}' is {tmp_L}, but it is shorter than "
                        f"any candidates of chunk-length: {self.chunk_lengths}"
                    )
                    continue

                # Remove lengths data and get the first sample
                batch = {k: v[index] for k, v in in_batch.items() if not k.endswith("_lengths")}
