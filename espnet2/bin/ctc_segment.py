...
class AbsCTCSegmentation:
    """CTC segmentation, less the model that produces the posteriors.

    A subclass loads a model and implements ``get_lpz``; this class holds
    ...
    def __init_subclass__(cls, **kwargs):
        """Give each subclass its own segmentation parameters.

        ``set_config`` writes into ``config``, so one object inherited by
        both subclasses would be one set of parameters shared between two
        models - which is not what a caller aligning with both would expect.
        """
        super().__init_subclass__(**kwargs)
        cls.config = CtcSegmentationParameters()

    @staticmethod
    def _resolve_device(device: Optional[str], ngpu: int) -> str:
        """Say where to run, from either of the two ways of asking.
    ...
    def from_pretrained(cls, model_tag: Optional[str] = None, **kwargs):
        ...
        which is what the examples used to do, through espnet_model_zoo's
        downloader.

        Each subclass takes one kind of model, and a tag for the other kind
        arrives with artifacts its constructor does not take. `espnet align`
        picks between them for you and says so in a sentence; here it is a
        TypeError naming the argument, as it is for every other class handed
        the wrong tag.
    ...
    def estimate_samples_to_frames_ratio(self, speech_len=215040):
        ...
        lpz, _ = self._lpz_and_length(random_input)
        lpz_len = lpz.shape[0]
    ...
    def get_lpz(self, speech: Union[torch.Tensor, np.ndarray]):
        ...
        Implemented by each subclass: this is the one step that depends on
        the model.
    ...
    def _tokenize(self, text: str):
        ...
        return self.preprocess_fn("<dummy>", {"text": text})["text"]
    ...
    def prepare_segmentation_task(self, text, lpz, name=None, speech_len=None):
        ...
        if self.text_converter == "tokenize":
            ...
            token_list = [self._tokenize(utt) for utt in text]
    ...
    def __call__(
        ...
        lpz, speech_len = self._lpz_and_length(speech)
        # Conflate text & lpz & config as a segmentation task object
        task = self.prepare_segmentation_task(text, lpz, name, speech_len)
...
def align_file(
    ...
    **kwargs,
    ...
    `aligner_class` is the CTCSegmentation class of the calling script; the
    arguments that name the model - `asr_train_config`, `s2t_model_file`,
    whichever pair that class takes - arrive in `kwargs` and go to it
    unchanged.
    """
    logging.basicConfig(
        level=log_level,
...
def build_parser(model_arg_prefix: str, description: str):
    """Obtain an argument-parser for a script interface.

    The two scripts take the same arguments but for the pair that names
    the model, which each spells after its own task.
    """
    ...
    group.add_argument(f"--{model_arg_prefix}_train_config", type=str, required=True)
    group.add_argument(f"--{model_arg_prefix}_model_file", type=str, required=True)
    ...
    group.add_argument(
        "--time_stamps",
        type=str,
        default=AbsCTCSegmentation.time_stamps,
        choices=AbsCTCSegmentation.choices_time_stamps,
    ...
    group.add_argument(
        "--text_converter",
        type=str,
        default=AbsCTCSegmentation.text_converter,
        choices=AbsCTCSegmentation.choices_text_converter,
