...
class UniversaInference:
    ...
    def __init__(
        ...
        if getattr(self.universa, "sequential_metrics", False):
            metrics = metric_list or list(self.universa.metric2id)
            if fixed_metric_name_order:
                metrics = fixed_metric_name_order.replace(",", " ").split()
            self.universa.set_inference(
                beam_size,
                metrics,
    ...
    def __call__(
        ...
        audio_lengths: Optional[Union[np.ndarray, torch.Tensor]] = None,
        ...
        **kwargs,
        ...
        if ref_text is not None:
            if isinstance(ref_text, str):
                ...
                ref_text_lengths = torch.tensor([ref_text.shape[1]])
            batch.update(ref_text=ref_text, ref_text_lengths=ref_text_lengths)
        for name in ("audio", "ref_audio", "ref_text"):
            ...
            batch[name] = value
            if batch.get(name + "_lengths") is None:
                batch[name + "_lengths"] = torch.full(
...
def inference(
    ...
    with DatadirWriter(output_dir) as writer:
        for keys, batch in loader:
            ...
            results.pop("encoded_feat", None)
            results.pop("use_tokenizer_metrics", None)
            results.pop("sequential_metrics", None)
            token_sequences = results.pop("token_seq", None)
            for i in range(_bs):
                key = keys[i]
                if token_sequences is not None:
                    writer["token.scp"][key] = " ".join(map(str, token_sequences[i]))
                # NOTE(jiatong): assume the prediction target is 1-dimensional.
                metrics_info = {
                    k: (
                        v[i].item()
                        if isinstance(v, torch.Tensor)
                        else np.asarray(v[i]).item()
                    )
...
def get_parser():
    ...
    parser.add_argument("--skip_meta_label_score", type=str2bool, default=False)
    parser.add_argument("--save_token_seq", type=str2bool, default=False)
    parser.add_argument("--use_fixed_order", type=str2bool, default=False)
    parser.add_argument("--fixed_metric_name_order", type=str, default="")
    parser.add_argument(
        "--ngpu",
        type=int,
        default=0,
