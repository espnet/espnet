...
"""Script to run the inference stage for voice conversion models."""
...
class VoiceConversion:
    """VoiceConversion class.

    Examples:
        >>> from espnet2.bin.vc_inference import VoiceConversion
        ...
        >>> import soundfile as sf
        >>> audio, _ = sf.read("hello_world.wav")
        >>> wav = voice_conversion(audio)["wav"]
        >>> sf.write("out.wav", wav.numpy(), voice_conversion.fs, "PCM_16")
    ...
    def __init__(
        ...
        """Initialize VoiceConversion module."""
        assert check_argument_types()

        # setup model
        model, train_args = VCTask.build_model_from_file(
        ...
        self.preprocess_fn = VCTask.build_preprocess_fn(train_args, False)
        self.use_teacher_forcing = use_teacher_forcing
        self.seed = seed
        self.always_fix_seed = always_fix_seed
        self.vocoder = None
        if self.vc.require_vocoder:
            vocoder = VCTask.build_vocoder_from_file(
        ...
        if isinstance(self.vc, Transformer):
            decode_conf.update(
                threshold=threshold,
    ...
    def __call__(
        self,
        in_speech: Union[torch.Tensor, np.ndarray],
        out_speech: Union[torch.Tensor, np.ndarray] = None,
        text: Union[str, torch.Tensor, np.ndarray] = None,
        ...
        # check inputs
        if self.use_speech and out_speech is None:
            raise RuntimeError("Missing required argument: 'out_speech'")
        ...
        # prepare batch
        batch = dict(in_speech=in_speech)
        if out_speech is not None:
            batch.update(out_speech=out_speech)
    ...
    def use_speech(self) -> bool:
        ...
        return self.use_teacher_forcing or getattr(self.vc, "use_gst", False)

    @property
    def use_sids(self) -> bool:
        """Return sid is needed or not in the inference."""
        return self.vc.spks is not None

    @property
    def use_lids(self) -> bool:
        """Return sid is needed or not in the inference."""
        return self.vc.langs is not None

    @property
    def use_spembs(self) -> bool:
        """Return spemb is needed or not in the inference."""
        return self.vc.spk_embed_dim is not None
    ...
    def from_pretrained(
        ...
        **kwargs: Optional[Any],
        ...
        """Build VoiceConversion instance from the pretrained model.
        ...
        Returns:
            VoiceConversion: VoiceConversion instance.
...
def inference(
    ...
    voice_conversion = VoiceConversion.from_pretrained(
        ...
        **voice_conversion_kwargs,
    )

    # 3. Build data-iterator
    if not voice_conversion.use_speech:
        data_path_and_name_and_type = list(
            filter(lambda x: x[1] != "out_speech", data_path_and_name_and_type)
        )
    loader = VCTask.build_streaming_iterator(
        data_path_and_name_and_type,
        dtype=dtype,
        batch_size=batch_size,
        key_file=key_file,
        num_workers=num_workers,
        preprocess_fn=VCTask.build_preprocess_fn(voice_conversion.train_args, False),
        collate_fn=VCTask.build_collate_fn(voice_conversion.train_args, False),
    ...
    with NpyScpWriter(
        ...
        for idx, (keys, batch) in enumerate(loader, 1):
            ...
            output_dict = voice_conversion(**batch)
            ...
            if output_dict.get("feat_gen") is not None:
                ...
                # end-to-end speech2wav model case
                wav = output_dict["wav"]
                logging.info(
