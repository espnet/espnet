...
#CHANGING FOR TRUCLES INFERENCE
...
from espnet.nets.batch_beam_search_TruCLeS import BatchBeamSearch
from espnet.nets.batch_beam_search_online_sim import BatchBeamSearchOnlineSim
from espnet.nets.beam_search_TruCLeS import BeamSearch, Hypothesis
...
class Speech2Text:
    ...
    def __call__(
        ...
        # to get attention from the encoder for TruCLeS
        self.attn = self.asr_model.encoder.attn_output_final
        self.soft = self.asr_model.decoder.softmax
        self.deco = self.asr_model.decoder.decoder
        # _________________
        return results
    ...
    def get_TruCleS_output(
        self, speech: Union[torch.Tensor, np.ndarray]
    ):
        req_ind = self.beam_search.required_index[1][1]
        soft = None
        deco = None
        for i in range(len(req_ind)):
            ind = req_ind[i][0]
            if soft == None:
                soft = self.soft[i][ind,:].unsqueeze(0)
                deco = self.deco[i][ind,0,:].unsqueeze(0)
            else:
                soft = torch.cat((soft,self.soft[i][ind,:].unsqueeze(0)), dim=0)
                deco = torch.cat((deco,self.deco[i][ind,0,:].unsqueeze(0)), dim=0)
            if req_ind[i][1] == 4999:
                break
            
        att = self.attn
        out = {'attention': att, 'decoder': deco, 'softmax': soft}
        return out
