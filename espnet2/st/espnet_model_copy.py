        ...
        **kwargs,
        ...
        if self.continuous_prompt:
            ...
            if self.continuous_prompt_start:
                tag_encoder_out=torch.cat((src_tag_out, tgt_tag_out, encoder_out),1)
            else:
                tag_encoder_out=torch.cat((src_tag_out, encoder_out, tgt_tag_out),1)
            tag_encoder_out_lens=torch.add(encoder_out_lens,2)
            loss_st_att, acc_st_att, bleu_st_att = self._calc_mt_att_loss(
                tag_encoder_out, tag_encoder_out_lens, text, text_lengths, tgt_tag, st=True
            )
            # 2b. CTC branch
            if self.st_mtlalpha > 0:
                loss_st_ctc = self.st_ctc(
                    tag_encoder_out, tag_encoder_out_lens, text, text_lengths
            )
        else:
            loss_st_att, acc_st_att, bleu_st_att = self._calc_mt_att_loss(
