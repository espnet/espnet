...
class ESPnetSTHierMlModel(AbsESPnetModel):
    ...
    def forward(
        ...
        # check that all in batch are from the same src lang
        assert all(src_text[0][0] == x[0] for x in src_text), "not batched by category"
        cat = str(src_text[0][0].item())
        ...
        # 1.b Encoder Hier
        # encoder_hier_out, encoder_hier_out_lens, _ = self.encoder_hier[cat](encoder_out, encoder_out_lens)
        encoder_hier_out, encoder_hier_out_lens, _ = getattr(self, "encoder_hier"+cat)(encoder_out, encoder_out_lens)
        ...
        if mt_ctc_weight == 0.0:
            ...
            loss_st = (
                mt_ctc_weight * loss_mt_ctc + (1 - mt_ctc_weight) * loss_st_att
            )
    ...
    def _calc_mt_ctc_loss(
        ...
        # Calc CTC loss
        loss_ctc = self.mt_ctc(encoder_out, encoder_out_lens, ys_pad, ys_pad_lens)
