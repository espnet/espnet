...
class MultiMaskMLMDecoder(AbsDecoder):
    ...
    def forward(
        ...
        Returns:
            ...
            olens: (batch, )
        """
        num_hypotheses = int(ys_in_pad.shape[0] / hs_pad.shape[0])
        assert num_hypotheses >= 1, "num_hypotheses must be >= 1"
        batch, length = ys_in_pad.shape
        tgt = ys_in_pad
        # tgt_mask: (B, 1, L)
