        ...
        *,
        ...
        for c, decoder in zip(cache, self.decoders):
            x, tgt_mask, memory, memory_mask = decoder(
                x, tgt_mask, memory, memory_mask, cache=c, xattn_logit_bias=xattn_logit_bias
            )
            new_cache.append(x)
    ...
    def batch_score(
        ...
        if return_hs:
            (logp, hs), states = self.forward_one_step(
                ys, ys_mask, xs, cache=batch_state, return_hs=return_hs, xattn_logit_bias=xattn_logit_bias
            )
        else:
