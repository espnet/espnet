...
class SerialBiMambaBlock(nn.Module):
    ...
    def forward(
        ...
        hidden_states_f = self.mixer_forward(hidden_states_f, inference_params=inference_params, direction="forward")
        hidden_states_f = hidden_states_f + residual
        if flip_fn is None:
            hidden_states_b, residual = self.add_norm(hidden_states_f, None, self.norm_b)
            hidden_states_b = self.mixer_backward(hidden_states_b, inference_params=inference_params, direction="backward")
            hidden_states_b = hidden_states_b + residual
        else:
            hidden_states_b = flip_fn(hidden_states_f.transpose(-1, -2)).transpose(-1, -2)
            hidden_states_b, residual = self.add_norm(hidden_states_b, None, self.norm_b)
            hidden_states_b = self.mixer_backward(hidden_states_b, inference_params=inference_params, direction="backward")
            hidden_states_b = hidden_states_b + residual
            hidden_states_b = flip_fn(hidden_states_b.transpose(-1, -2)).transpose(-1, -2)
    ...
    def allocate_inference_cache(self, batch_size, max_seqlen=None, dtype=None, **kwargs):
        conv_state_f, ssm_state_f = self.mixer_forward.allocate_inference_cache(batch_size, max_seqlen, dtype=dtype, **kwargs)
        conv_state_b, ssm_state_b = self.mixer_backward.allocate_inference_cache(batch_size, max_seqlen, dtype=dtype, **kwargs)
