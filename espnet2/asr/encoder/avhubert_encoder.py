...
class FairseqAVHubertEncoder(AbsEncoder):
    ...
    def __init__(
        ...
        self.avhubert_model_path = download_avhubert(
            avhubert_url, avhubert_dir_path)

        ckpt = torch.load(self.avhubert_model_path)
        state = {
            k: v
...
class AVHubertConfig:
    ...
    sub_encoder_layers: int = field(
        default=0, metadata={
            "help": "number of transformer layers for single modality"})
    audio_feat_dim: int = field(
        default=-1, metadata={"help": "audio feature dimension"}
    )
    modality_dropout: float = field(
        default=0, metadata={
            "help": "drop one modality"})
    audio_dropout: float = field(
        default=0, metadata={
            "help": "drop audio feature"})
    modality_fuse: str = field(
        default="concat", metadata={
            "help": "fusing two modalities: add,concat"})
...
class TransformerEncoder(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.pos_conv = nn.Sequential(
            self.pos_conv, SamePad(
                args.conv_pos), nn.GELU())
...
class ResNet(nn.Module):
    def __init__(
        ...
        self.downsample_block = (
            downsample_basic_block_v2
            if avg_pool_downsample else downsample_basic_block)
