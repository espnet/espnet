...
class VisualEBranchformerEncoderLayer(torch.nn.Module):
    ...
    def forward(self, x_input, mask, aux_loss=None, cache=None):
        ...
        if self.use_moe:
            aux_loss = aux_loss + moe_loss
            return x, mask, aux_loss
        else:
            return x, mask


class VisualEBranchformerEncoder(AbsEncoder):
    ...
    def __init__(
        ...
        self.encoders = repeat(
            num_blocks,
            lambda lnum: VisualEBranchformerEncoderLayer(
                output_size,
    ...
    def forward(
        ...
        Args:
            ...
            clip_feature: Visual feature as prompt (#batch, frame_num, dim).
