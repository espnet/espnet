...
class EBranchformerEncoderLayer(torch.nn.Module):
    ...
    def __init__(
        ...
        # TruCLeS Variable to store the Attention embeddings from the Branchformer Layer
        self.train_attention = None
    ...
    def forward(self, x_input, mask, cache=None):
        ...
        #TruCLeS Saving attention output.
        self.attn_output = x_att
        self.train_attention = x_att
...
class EBranchformerEncoder(AbsEncoder):
    ...
    def __init__(
        ...
        # Saving attention output from the final E-Branchformer layer
        self.attn_output_final = None
        self.train_attention = None
        ...
        if len(self.interctc_layer_idx) == 0:
            if max_layer is not None and 0 <= max_layer < len(self.encoders):
                for layer_idx, encoder_layer in enumerate(self.encoders):
                    ...
                    # Added to store attn_output of the final_encoder_layer
                    self.attn_output_final = encoder_layer.attn_output
                    self.train_attention = self.encoders[-1].attn_output
                ...
                xs_pad, masks = self.encoders(xs_pad, masks)
                 # Added to store attn_output of the final_encoder_layer
                self.attn_output_final = self.encoders[-1].attn_output
                self.train_attention = self.encoders[-1].attn_output
            ...
            for layer_idx, encoder_layer in enumerate(self.encoders):
                ...
                # Added to store attn_output of the final_encoder_layer
                self.attn_output_final = encoder_layer.attn_output
                self.train_attention = encoder_layer.attn_output
