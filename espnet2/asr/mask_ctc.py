...
def expand(padded_tensor: torch.Tensor, segment_ids: torch.Tensor, original_lengths: torch.Tensor) -> torch.Tensor:
    """
    ...
    expanded_tensor = expanded_tensor = expanded_tensor * time_mask.type_as(expanded_tensor)
...
class MaskCTC(CTC):
    ...
    def __init__(
        ...
        super().__init__(
            odim, encoder_output_size, dropout_rate, ctc_type,
            reduce, ignore_nan_grad, zero_infinity, brctc_risk_strategy,
            brctc_group_strategy, brctc_risk_factor
            )
        ...
        self.num_iter = 1
        self.eos_token = F.one_hot(
            torch.tensor(odim-1), num_classes=odim
        ).to(float).scatter_(-1, torch.tensor(odim-1), float('inf'))
    ...
    def estimate_ctc_confidence(self, ctc_logits):
        ...
        if self.confidence_estimation == "uniform":
            ...
            mean, std = 0.5, 0.15
            confidence = torch.normal(mean=mean, std=std, size=(1, L, 1), device=device).expand(B, L, 1)    
    ...
    def gating(self, sequence, confidence, iter):
        ...
        if self.gate_type == "hard":
            ...
            sequence = eff_confidence * sequence + (1 - eff_confidence) * self.mask_token
