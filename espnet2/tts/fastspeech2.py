...
class FastSpeech2(AbsTTS):
    ...
    def forward(
        ...
        pitch = pitch[:, : pitch_lenghts.max()]  # for data-parallel
    ...
    def _average_by_duration(self, xs: torch.Tensor, ds: torch.Tensor) -> torch.Tensor:
        pass
...
class FastSpeech2Loss(torch.nn.Module):
    ...
    def forward(
        ...
        if self.use_weighted_masking:
            ...
            pitch_loss = (
                pitch_loss.mul(duration_weights).masked_select(duration_masks).sum()
            )
            energy_loss = (
                energy_loss.mul(duration_weights).masked_select(duration_masks).sum()
            )
