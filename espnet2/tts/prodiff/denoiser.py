...
def _vpsde_beta_t(t, T, min_beta, max_beta):
    t_coef = (2 * t - 1) / (T ** 2)
    return 1. - np.exp(-min_beta / T - 0.5 * (max_beta - min_beta) * t_coef)


def noise_scheduler(sched_type, timesteps, min_beta: float = 0.0, max_beta: float = 0.01, s: float = 0.008):
    if sched_type == "linear":
        ...
        scheduler = np.array([
            _vpsde_beta_t(t, timesteps, min_beta, max_beta) for t in range(1, timesteps + 1)])
    else:
...
class ResidualBlock(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.conv = nn.Conv1d(channels, 2 * channels, 3, padding=dilation, dilation=dilation)
...
class SpectogramDenoiser(nn.Module):
    def __init__(
        ...
        self.register_buffer("min_alphas_cumulative", torch.sqrt(1. - alphas_cumulative))
    ...
    def forward(
        ...
        timesteps = torch.randint(0, self.timesteps + 1, (batch_size,)).to(xs.device).long()
    ...
    def forward_denoise(
        ...
        xs_noisy = torch.sum(torch.stack(skip_conns), dim=0) / math.sqrt(self.num_layers)
    ...
    def inference(self, condition: torch.Tensor) -> torch.Tensor:
        ...
        coef1 = beta * torch.sqrt(alph_prod_prv) / (1. - alph_prod)
        coef2 = (1. - alph_prod_prv) * torch.sqrt(alph) / (1. - alph_prod)
        post_var = beta * (1. - alph_prod_prv) / (1. - alph_prod)
