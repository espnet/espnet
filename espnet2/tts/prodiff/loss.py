...
class SSimLoss(torch.nn.Module):
    def __init__(self, bias = 6.0, window_size = 11, channels = 1, reduction = "none"):
        super().__init__()
        self.bias = bias
    ...
    def forward(self, outputs: torch.Tensor, target: torch.Tensor):
        # outputs: (batch, time, mels)
        # target: (batch, time, mels)
        with torch.no_grad():
            dim = target.size(-1)
    ...
    def ssim(self, tensor1: torch.Tensor, tensor2: torch.Tensor):
        ...
        ssim_map = ((2 * mu_corr + C1) * (2 * sigma_corr + C2)) / ((mu1 + mu2 + C1) * (sigma1 + sigma2 + C2))
