...
class TFGridNet(AbsSeparator):
    ...
    def __init__(
        ...
        for _ in range(n_layers):
            self.blocks.append(
                    GridNetBlock(
                        emb_dim, emb_ks, emb_hs, n_freqs, lstm_hidden_units,
                        n_head=attn_n_head, approx_qk_dim=attn_approx_qk_dim,
...
class GridNetBlock(nn.Module):
    ...
    def __init__(self,
            emb_dim, emb_ks, emb_hs, n_freqs, hidden_channels,
            n_head=4, approx_qk_dim=512,
            activation='prelu', eps=1e-5,
            ):
    ...
    def forward(self, x):
        ...
        T = math.ceil((old_T - self.emb_ks)*1./self.emb_hs) * self.emb_hs + self.emb_ks
        Q = math.ceil((old_Q - self.emb_ks)*1./self.emb_hs) * self.emb_hs + self.emb_ks
