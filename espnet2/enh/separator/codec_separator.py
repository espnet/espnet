...
class CodecSeparator(AbsSeparator):
    def __init__(
        ...
        self.ch_down = nn.Conv1d(
            input_dim, internal_dim, 1, bias=False
        )  # change the dimensions for separator
        self.ch_up = nn.Conv1d(
            internal_dim, input_dim, 1, bias=False
        )  # return to the original dimension

        self.masker = weight_norm(
            nn.Conv1d(input_dim, input_dim * self.num_outputs, 1, bias=False)
        )  # computes masks of the original input
        ...
        self.output = nn.Sequential(
            ...
            ),  # nn.ReLU() #Snake1d(input_dim) #nn.Tanh() #, Snake1d(channels)#
        )
    ...
    def forward(
        ...
        masks = [masks[:, i, :, :].permute(0, 2, 1) for i in range(self.num_outputs)]
