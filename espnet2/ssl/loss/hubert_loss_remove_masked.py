...
def _compute_logits(
    ...
    """
    logit_temp = 0.1
    print(f"Max logit: {torch.max(target)}", flush=True)
...
class LogitGenerator(nn.Module):
    ...
    def __init__(
        ...
        if self.mix_tokens:
            self.attn_proj = torch.nn.Linear(encoder_embed_dim, mix_dim)
            self.attn = MultiHeadedAttention(4, mix_dim, 0.1)
            self.final_proj = torch.nn.Linear(mix_dim, final_dim)
        else:
...
class HuBERTLoss(AbsLoss):
    ...
    def forward(self, 
        ...
        stats = dict(
            ...
            hubert_acc_m=correct_m / count_m,
            hubert_correct_u=correct_u,
            hubert_count_u=count_u,
            hubert_acc_u=correct_u / count_u,
        )
