...
class HubertMasker(AbsMasker):
    ...
    def restore_mask(self, x: Tensor, mask_info) -> Tensor:
        _, T_new, C_new = x.shape
        ids_restore = mask_info['ids_restore']

        mask_tokens = x.new_empty(self.B, self.T - T_new, C_new)
        mask_tokens[:,:,:] = self.mask_embedding.to(x.dtype)
        x_ = torch.cat([x[:, 0:], mask_tokens], dim=1)
        x = torch.gather(x_, dim=1, index=ids_restore.expand(-1, -1, C_new))

        return x

    def regen_padding_mask(self, x: Tensor, padding_mask: Optional[Tensor], lengths: Optional[Tensor], masked_indices) -> Tensor:
        if lengths is not None:
            B, T_new, C_new = x.shape

            masked_lengths = masked_indices.long().sum(dim=-1)
            lengths = lengths - masked_lengths

            padding_mask = (make_pad_mask(lengths)).to(x.device)

        return padding_mask

    def forward(self, x: Tensor, padding_mask: Optional[Tensor], lengths: Optional[Tensor]) -> Tensor:
        """
        Args:
        ...
        if self.mask_prob > 0:
            ...
            if not self.remove_masked:
                x[mask_indices] = self.mask_embedding.to(x.dtype)
            else:
                mask = mask_indices.to(torch.uint8)
                ids_shuffle = mask.argsort(dim=1)
                ids_restore = ids_shuffle.argsort(dim=1).unsqueeze(-1)
                len_keep = self.T - mask[0].sum()
                ids_keep = ids_shuffle[:, :len_keep]

                ids_keep = ids_keep.unsqueeze(-1).expand(-1, -1, self.C)
                x = torch.gather(x, dim=1, index=ids_keep)

                padding_mask = self.regen_padding_mask(x, padding_mask, lengths, mask_indices)
        else:
            mask_indices = None
        ...
        return x, padding_mask, {'mask_m': mask_indices, 'mask_u': ~mask_indices, 'ids_restore': ids_restore}
