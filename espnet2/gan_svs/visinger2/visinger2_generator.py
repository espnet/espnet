...
class VISinger2Generator(torch.nn.Module):
    """Generator module in VISinger2."""
    ...
    def __init__(
        ...
        if self.use_visinger:
            ...
            self.mel_prenet = torch.nn.Conv1d(
                acoustic_dim, hidden_channels + 2, 3, padding=1
            )
    ...
    def forward(
        ...
        if not self.use_dp:
            ...
            # LR
            decoder_input, mel_len = self.lr(x, duration, use_state_info=True)
            decoder_input_pitch, mel_len = self.lr(
                x_pitch, duration, use_state_info=True
            )
            ...
            predict_lf0, predict_bn_mask = self.f0_decoder(
                decoder_input + decoder_input_pitch, pitch_lengths, g=g
            )
            predict_mel, predict_bn_mask = self.mel_decoder(
                decoder_input + self.f0_prenet(LF0), pitch_lengths, g=g
            )

            predict_energy = (
                predict_mel.detach().sum(1).unsqueeze(1) / self.acoustic_dim
            )
            ...
            decoder_output, predict_bn_mask = self.prior_decoder(
                decoder_input, pitch_lengths, g=g
            )
        ...
        gt_dur = torch.stack([duration, beat], dim=1).to(torch.float)
