...
"""Generator module in PITS.

This code is based on https://github.com/anonymous-pits/pits.
...
class PITSGenerator(torch.nn.Module):
    ...
    This is a module of PITS described in `Conditional Variational Autoencoder
    ...
    def __init__(
        ...
        # ying
        yin_channels: int = 80,
        yin_start: int = 15,  # scope start bin in nansy = 1.5/8
        yin_scope: int = 50,  # scope ratio in nansy = 5/8
        yin_shift_range: int = 15,  # same as default start index of yingram
        midi_start: int = -5,
        midi_end: int = 75,
        midis: int = 80,
        octave_range: int = 24,
    ):
        """Initialize PITS generator module.
        ...
        self.posterior_encoder = PosteriorEncoder(
            ...
            out_channels=hidden_channels - yin_channels,
            hidden_channels=hidden_channels - yin_channels,
        ...
        self.yin_encoder = PosteriorEncoder(
            in_channels=yin_channels,
            out_channels=yin_channels,
            hidden_channels=yin_channels,
            kernel_size=posterior_encoder_kernel_size,
            layers=posterior_encoder_layers,
            stacks=posterior_encoder_stacks,
            base_dilation=posterior_encoder_base_dilation,
        ...
        self.yin_decoder = YingDecoder(
            hidden_channels=yin_scope,
            kernel_size=5,
            dilation_rate=1,
            n_layers=4,
            yin_start=yin_start,
            yin_scope=yin_scope,
            yin_shift_range=yin_shift_range,
            gin_channels=global_channels,
        )

        self.pitch = Ying(
            fs=fs, midi_start=midi_start, midi_end=midi_end, octave_range=octave_range
        )

    def crop_scope(
        self, x, scope_shift=0
    ):  # x: list #need to modify for non-scalar shift
        return [
            i[
                :,
                self.yin_start
                + scope_shift : self.yin_start
                + self.yin_scope
                + scope_shift,
                :,
            ]
    ...
    def forward(
        ...
        z_spec, m_spec, logs_spec, y_mask = self.posterior_encoder(
            feats, feats_lengths, g=g
        )

        # ying encoder
        z_yin, m_yin, logs_yin, yin_mask = self.yin_encoder(ying, feats_lengths, g=g)
        z_yin_crop, logs_yin_crop, m_yin_crop = self.crop_scope(
            [z_yin, logs_yin, m_yin], scope_shift
        )

        # yin dec loss
        # print("z_yin", z_yin.shape)
        # print("ying", ying.shape)
        # print("yin_mask", yin_mask.shape)
        ...
        (
            ...
            z_yin_crop_shifted,
            scope_shift,
        ) = self.yin_decoder(z_yin, ying, yin_mask, g)

        # print("z_yin_crop", z_yin_crop.shape)
        # print("yin_gt_crop", yin_gt_crop.shape)
        # print("yin_gt_shifted_crop", yin_gt_shifted_crop.shape)
        # print("yin_dec_crop", yin_dec_crop.shape)
        # print("z_yin_crop_shifted", z_yin_crop_shifted.shape)
        # print("scope_shift", scope_shift.shape)
        ...
        o_pad = F.pad(
            ...
            (
                768,
                768 + (-wav[-1].shape[-1]) % 256 + 256 * (wav[-1].shape[-1] % 256 == 0),
            ),
            mode="constant",
        ).squeeze(1)
        ...
        # print("wav", wav[-1].shape)
        # print("o_pad", o_pad.shape)
        # print("yin_hat", yin_hat.shape)
        # print("yin_hat_crop", yin_hat_crop.shape)
        # print("yin_hat_shifted", yin_hat_shifted.shape)
        # raise ValueError
        ...
        return (
            ...
            (z_spec, m_spec, logs_spec, z_yin, m_yin, logs_yin, yin_mask),
            (
                yin_gt_crop,
                yin_gt_shifted_crop,
                yin_dec_crop,
    ...
    def inference(
        ...
        if use_teacher_forcing:
            ...
            wav = self.decoder((z_crop * y_mask)[:, :, :max_len], g=g)[-1]
