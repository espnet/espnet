...
def get_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--srcspk", required=True, help="Source speaker")
    parser.add_argument("--trgspk", required=True, help="Target speaker")
    parser.add_argument("--src_dir", required=True, help="Source dir")
    parser.add_argument("--trg_dir", required=True, help="Target dir")
...
def main(cmd=None):
    ...
    trg_fs = [
        f
        for f in trg_fs
        if os.path.isfile(os.path.join(args.trg_dir, f))
        and not f.startswith(".")
        and not f.endswith(".npz")
    ]
        ...
        if src_elem0.startswith(args.srcspk):
            if not src_elem0.startswith(pair_spk):
                ...
                if src_elem0 == args.srcspk:
                    ...
                    new_fids = [
                        pair_spk + fid[len(args.srcspk) :]
                        for fid in src_lines[0].split()[1:]
                    ]
                    new_fids_str = " ".join(new_fids)
                    new_src_line = "%s %s" % (pair_spk, new_fids_str)
                    new_src_lines.append(new_src_line)
                    new_fids = [
                        pair_spk + fid[len(args.trgspk) :]
                        for fid in trg_lines[0].split()[1:]
                    ]
                    new_fids_str = " ".join(new_fids)
