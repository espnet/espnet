...
def truncate_align(
    ...
    for b in range(B):
        ...
        indices = torch.clamp(
            indices, min=0, max=len(source_to_modify[b]) - 1
        ).unsqueeze(-1)
    ...
    assert (speech.size(0) == vision.size(0), speech.size(1) == vision.size(1)), (
        speech.shape,
        vision.shape,
    )
    ...
    for b in range(B):
        ...
        for index in indices:
            if index == 0:
                ...
                avg_pool.append(
                    torch.mean(source_to_modify[b, prev_index:index, :], 0).unsqueeze(0)
                )
            prev_index = index
...
class MM_Aligner:
    ...
    def align(
        ...
        if ssr is not None and vsr is not None:
            ...
            if ssr == vsr:
                ...
                feat_lengths = torch.min(
                    torch.cat([speech_lengths, vision_lengths], dim=1), dim=1
                )[0]
            else:
        ...
        assert (
            speech.size(0) == feat_lengths.size(0),
            feat_lengths.dim() == 1,
        ), feat_lengths.shape
        return speech, vision, feat_lengths
