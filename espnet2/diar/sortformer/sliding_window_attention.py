...
class RelPositionLocalAttention(RelPositionMultiHeadAttention):
    ...
    # ----- band helpers (ported verbatim from NeMo) ----- #
    @staticmethod
    ...
    @lru_cache()
    def _invalid_locations_mask(self, w: int, device):
        diagonals = []
        for j in range(-w, 1):
    ...
    def _mask_invalid_locations(self, x, w: int):
        beg, end = self._invalid_locations_mask(w, x.device)
        seq_len = x.size(2)
        bi = x[:, :, :w, : w + 1]
    ...
    def sliding_chunks_matmul_qk(self, q, k, w: int, padding_value: float):
        bsz, n_head, seqlen, head_dim = q.size()
        chunks_count = seqlen // w - 1
        q = q.reshape(bsz * n_head, seqlen, head_dim)
        k = k.reshape(bsz * n_head, seqlen, head_dim)
        cq = self._chunk_overlap(q, w)
        ck = self._chunk_overlap(k, w)
        ...
        self._mask_invalid_locations(out, w)
        return out

    def sliding_chunks_matmul_pv(self, prob, v, w: int):
        ...
        skewed = self._skew2(chunk_prob, padding_value=0)
    ...
    def forward(self, x, pos_emb, pad_mask, n_global: int = 0):
        ...
        ac = self.sliding_chunks_matmul_qk(qu, k, w, padding_value=0.0)  # (B,H,Tp,2w+1)
        ...
        d_mask = self.sliding_chunks_matmul_qk(ones, fmask, w, padding_value=0.0)
        scores += d_mask
        ...
        if n_global > 0:
            ...
            out = self.sliding_chunks_matmul_pv(band_attn, v, w)  # (B,Tp,H,d_k)
            ...
            attn = attn.masked_fill(mask.unsqueeze(1).unsqueeze(-1), 0.0)
            attn = self.dropout(attn)
            out = self.sliding_chunks_matmul_pv(attn, v, w)
            out = out.reshape(n_batch, -1, self.h * self.d_k)[:, :T]
