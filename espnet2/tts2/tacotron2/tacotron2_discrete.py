...
#           2024 Carnegie Mellon University (Jinchuan Tian)
#  Apache 2.0  (http://www.apache.org/licenses/LICENSE-2.0)

"""Tacotron 2 related modules for ESPnet2, with discrete predicting target"""
...
class Tacotron2Discrete(AbsTTS2):
    """Tacotron2 module for end-to-end text-to-speech
       A revised version, with discrete predicting target
    ...
    def __init__(
        ...
        Args:
            ...
            odim (int): Vocabulary size of discrete predicting targets
            hidden_dim: (int) Dimension of the Tacotron hidden space before classification.
        ...
        if self.use_gst:
            self.gst = StyleEncoder(
                idim=hidden_dim,  # the input is mel-spectrogram
        ...
        self.emb = torch.nn.Embedding(odim, hidden_dim)
        ...
        if atype == "location":
            ...
            att = AttForwardTA(
                dec_idim, dunits, adim, aconv_chans, aconv_filts, hidden_dim
            )
            if self.cumulate_att_w:
                logging.warning(
                    "cumulation of attention weights is disabled "
        ...
        self.taco2_loss = Tacotron2LossDiscrete(
            idim=hidden_dim,
            vocab_size=odim,
    ...
    def forward(
        ...
        Args:
            ...
            discrete_feats (Tensor): Batch of padded target features (B, T_feats).
            discrete_feats_lengths (LongTensor): Batch of the lengths of each target (B,).
        ...
        ce_loss, bce_loss, stats = self.taco2_loss(
            after_outs, before_outs, logits, discrete_feats, labels, olens
        )
    ...
    def inference(
        ...
        Returns:
            Dict[str, Tensor]: Output dict including the following items:
                * feat_gen (Tensor): Output sequence of features (T_feats, hidden_dim).
        ...
        if use_teacher_forcing:
            ...
            outs = self.taco2_loss.linear(outs).softmax(dim=-1)
        ...
        out = self.taco2_loss.linear(out).softmax(dim=-1)
