...
class PartiallyARBeamSearch(BatchBeamSearch):
    ...
    def init_hyp(self, x: torch.Tensor) -> PartiallyARHypothesis:
        ...
        yseq_tensor = torch.zeros((len(self.masks), self.beam_size, longest_yseq), dtype=torch.long, device=x.device) # (n_mask, n_beam, max_yseq)
        yseq_length = torch.zeros((len(self.masks)), dtype=torch.long, device=x.device)
        score = torch.zeros((len(self.masks) * self.beam_size), dtype=x.dtype, device=x.device)
        ...
        self.num_hyps_for_masks = torch.ones(len(self.masks), dtype=torch.long, device=x.device)
    ...
    def score_full(
        ...
        for k, d in self.full_scorers.items():
            ...
            mask_scores, mask_states = d.batch_score_partially_AR(
                hyp.yseq.reshape(-1, hyp.yseq.shape[-1]), # (n_mask * n_beam, y_seq_len)
                hyp.states[k],
                x.expand(hyp.yseq.size(0) * hyp.yseq.size(1), *x.shape), # (n_beam, *x.shape)
                hyp.yseq_length.unsqueeze(1).expand(n_mask, self.beam_size).reshape(-1), # (n_mask * n_beam)
            )
    ...
    def _select(self, hyps: PartiallyARHypothesis, i_mask: int, i_beam: int) -> Hypothesis:
        return Hypothesis(
    ...
    def forward(
        self, x: torch.Tensor, max_seq_len: int = None
    ) -> List[Hypothesis]:
        """Perform beam search.
        ...
        for i, nbest_mask in enumerate(nbest_hyps_for_masks):
            if len(nbest_mask) == 0:
                logging.warning(
                    f"there is no N-best results for mask {i}"
                )

        bests = [
            nbest_mask[0]
            for nbest_mask in nbest_hyps_for_masks
        ]
    ...
    def search(self, running_hyps: PartiallyARHypothesis, x: torch.Tensor) -> PartiallyARHypothesis:
        ...
        weighted_scores = torch.zeros(
            len(self.masks) * self.beam_size, self.n_vocab, dtype=x.dtype, device=x.device
        ) # (n_mask * n_beam, n_vocab)
        beam_mask = (self.beam_arange >= self.num_hyps_for_masks).transpose(0, 1).reshape(-1, 1) # (n_beam, n_mask) -> (n_mask, n_beam) -> (n_mask * n_beam, 1)
        beam_mask = beam_mask * -100000.0

        ##############################
        #### COMPUTE FULL SCORERS ####
        ##############################
        scores, states = self.score_full(running_hyps, x) # scorers: (n_mask * n_beam, n_vocab)
        for k in self.full_scorers:
            weighted_scores += self.weights[k] * scores[k]
        
        weighted_scores = weighted_scores + beam_mask # (n_mask * n_beam, n_vocab). no score for padded hypos

        # add previous hyp scores
        weighted_scores += running_hyps.score.unsqueeze(1) # (n_mask * n_beam, n_vocab) + (n_mask * n_beam, 1)
        weighted_scores = weighted_scores + beam_mask # (n_mask * n_beam, n_vocab). no score for padded hypos
        ...
        new_hyp = self._get_new_mask_parallel_hyp(running_hyps, prev_hyp_ids, new_token_ids,
                        weighted_scores, states)

        return new_hyp
    
    def _get_new_mask_parallel_hyp(
        self, running_hyps: PartiallyARHypothesis, prev_hyp_ids: torch.Tensor, new_token_ids: torch.Tensor,
        weighted_scores: torch.Tensor, states: Dict[str, torch.Tensor]#, part_states: Dict[str, torch.Tensor]
        ...
        new_yseq = torch.zeros((n_mask, self.beam_size, yseq_shape[-1] + 1), dtype=torch.int64, device=running_hyps.yseq.device)
        new_yseq[:, :, :-1] = running_hyps.yseq[self.mask_ids, prev_hyp_ids]
        new_yseq[self.mask_ids, :, new_yseq_length.view(-1, 1) - 1] = new_token_ids.unsqueeze(1) # (n_mask, 1, n_beam)

        current_score = weighted_scores.view(n_mask, self.beam_size, -1) # (n_mask, n_beam, vocab_size)
        new_score = current_score[self.mask_ids, prev_hyp_ids, new_token_ids].view(-1) # (n_mask, n_beam) => (n_mask * n_beam)
        ...
        for k, v in states.items():
            if isinstance(self.full_scorers[k], MaskParallelScorerInterface):
                ...
                state_shape = v.size()
                new_states[k] = v.view(n_mask, self.beam_size, *state_shape[1:])[self.mask_ids, prev_hyp_ids].reshape(n_mask * self.beam_size, *state_shape[1:])
            else:
                # Then v depends on scorers. We need to use select_state of each scorers step by step.
                new_states[k] = [
                        [
                            self.full_scorers[k].select_state(v[i_mask, i_beam], prev_hyp_ids[i_mask, i_beam])
    ...
    def _batch_select_and_pad(self, hyps: PartiallyARHypothesis, ids_list: List[torch.Tensor]) -> PartiallyARHypothesis:
        """
        Args:
        ...
        n_mask = len(self.masks)
        new_yseq = torch.zeros((n_mask, self.beam_size, hyps.yseq.shape[-1]), dtype=torch.int64, device=hyps.yseq.device)
        new_score = torch.zeros((n_mask, self.beam_size), dtype=hyps.score.dtype, device=hyps.score.device)
        for i_mask in range(n_mask):
            remaining_ids = ids_list[i_mask]
            self.num_hyps_for_masks[i_mask] = remaining_ids.size(0)

            new_yseq[i_mask, :remaining_ids.size(0)] = hyps.yseq[i_mask, remaining_ids] # (remain_beam, max_yseq_len)
            new_score[i_mask, :remaining_ids.size(0)] = hyps.score.view(n_mask, -1)[i_mask, remaining_ids]
            ...
            for k, v in self.full_scorers.items():
                if k in hyps.states and isinstance(v, MaskParallelScorerInterface):
                    ...
                    current_states = hyps.states[k].view(n_mask, self.beam_size, *state_shape[1:])
                    current_states[i_mask, :remaining_ids.size(0)] = current_states[i_mask, remaining_ids]

                    hyps.states[k] = \
                        current_states.view(n_mask * self.beam_size, *state_shape[1:])
                else:
                    # Then v depends on scorers. We need to use select_state of each scorers step by step.
                    hyps.states[k][i_mask] = [hyps.states[k][i_mask][ri] for ri in remaining_ids]
    ...
    def post_process(
        ...
        if i == maxlen - 1 and any([len(ended_hyps[i_mask]) == 0 for i_mask in range(n_mask)]):
            yseq_shape = running_hyps.yseq.shape
            yseq_eos = torch.zeros((n_mask, self.beam_size, yseq_shape[-1] + 1), dtype=torch.int64, device=running_hyps.yseq.device)
            yseq_eos[:, :, :-1] = running_hyps.yseq
            yseq_eos[self.mask_ids, :, running_hyps.yseq_length.view(-1, 1)] = running_hyps.eos.view(-1, 1, 1)
            running_hyps = PartiallyARHypothesis(
                yseq=yseq_eos,
        ...
        if self.beam_ids is None:
            self.beam_ids = torch.arange(self.beam_size, dtype=torch.int64, device=running_hyps.yseq.device)

        remained_ids_list = []
        for i_mask in range(len(self.masks)):
            # check if there is any ended hypo for mask i.
            is_eos = (
                running_hyps.yseq[i_mask, self.beam_ids, running_hyps.yseq_length[i_mask] - 1]
            ...
            remained_ids_list.append(torch.nonzero(is_eos == 0, as_tuple=False).view(-1).cpu())
    ...
    def _log_bests(self, bests: List[BatchHypothesis], maxlen, nbest_hyps_for_masks):
        for i, best in enumerate(bests):
            ...
            logging.info(f"normalized log probability for mask {i}: {best.score / len(best.yseq):.2f}")
            logging.info(f"total number of ended hypotheses for mask {i}: {len(nbest_hyps_for_masks[i])}")
