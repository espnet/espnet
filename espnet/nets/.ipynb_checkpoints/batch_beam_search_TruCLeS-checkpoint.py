...
class BatchBeamSearch(BeamSearch):
    ...
    def search(
        ...
        for (
            ...
            ## For TruCLeS getting the required index
            check_ind = 0
            if len(self.required_index) != 0:
                check_ind = self.required_index[full_prev_hyp_id][1][-1][1]
            
            if check_ind == 4999:
                score = self.required_index[full_prev_hyp_id][0]
            else:
                score = weighted_scores[full_prev_hyp_id, full_new_token_id]

            
            if len(self.required_index) == 0:
                curr_index.append([score, [[full_prev_hyp_id, full_new_token_id]]])
            else: 
                it_list = copy.deepcopy(self.required_index[full_prev_hyp_id][1])
                it_list.append([full_prev_hyp_id,full_new_token_id])
                curr_index.append([score, it_list])
        ...
        ##Getting the TruCLeS index
        self.required_index = curr_index
