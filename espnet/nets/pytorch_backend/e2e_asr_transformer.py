...
class E2E(ASRInterface, torch.nn.Module):
    ...
    def ctc_align(self, x, y):
        ...
        def interpolate_blank(label, blank_id=0):
            ...
            label = numpy.expand_dims(label, 1)
            blanks = numpy.zeros((label.shape[0], 1), dtype=numpy.int64) + blank_id
            label = numpy.concatenate([blanks, label], axis=1)
            label = label.reshape(-1)
            label = numpy.append(label, label[0])
            return label
        ...
        y_int = interpolate_blank(y, self.blank)

        logdelta = (
            numpy.zeros((lpz.size(0), len(y_int))) - 100000000000.0
        )  # log of zero
        state_path = (
            numpy.zeros((lpz.size(0), len(y_int)), dtype=numpy.int16) - 1
        )  # state path
        ...
        for t in six.moves.range(1, lpz.size(0)):
            for s in six.moves.range(len(y_int)):
                if y_int[s] == self.blank or s < 2 or y_int[s] == y_int[s - 2]:
                ...
                logdelta[t, s] = numpy.max(candidates) + lpz[t][y_int[s]]
