...
def get_token_level_ids_probs(ctc_ids, ctc_probs):
    ...
    y_hat_score = torch.from_numpy(np.array([
        (i, j) for i, j in zip(y_hat, score)]))
...
def tie_breaking(pairs, probs):
    ...
    return torch.tensor(path, dtype=torch.int64), torch.tensor(probs_path, dtype=torch.float)


def dynamic_matching(tensor1, tensor2, prob1=None, prob2=None, reserved_t=None, reserved_p=None):
    ...
    return dp_light, \
        ...
        tensor1[:end_point[0]], tensor2[:end_point[1]],\
        reserved_t, reserved_p


def dynamic_matching_xl(t1, t2, prob1, prob2,
                         dp_prev=None, t1_prev=None, t2_prev=None,
                         reserved_t=None, reserved_p=None):
