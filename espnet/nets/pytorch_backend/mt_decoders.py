...
class MTDecoder(torch.nn.Module):
    ...
    def __init__(self, eprojs, odim, dlayers, dunits, sos, eos, att, verbose=0,
        ...
        self.decoder += [torch.nn.LSTMCell(dunits + dunits, dunits)]
    ...
    def forward(self, hs_pad, hlens, ys_pad):
        ...
        for i in six.moves.range(olength):
            ...
            z_all.append(torch.cat((z_list[-1], att_c), dim=-1))
    ...
    def recognize_beam(self, h, recog_args, char_list, rnnlm=None):
        ...
        for i in six.moves.range(maxlen):
            ...
            for hyp in hyps:
                ...
                local_att_scores = F.log_softmax(self.output(
                    torch.cat((self.dropout_dec[-1](z_list[-1]), att_c), dim=-1)), dim=1)
                if rnnlm:
        ...
        if len(nbest_hyps) == 0:
            ...
            return self.recognize_beam(h, recog_args, char_list, rnnlm)
    ...
    def recognize_beam_batch(self, h, hlens, recog_args, char_list, rnnlm=None,
        ...
        for i in six.moves.range(maxlen):
            ...
            local_scores = att_weight * \
                F.log_softmax(self.output(torch.cat((self.dropout_dec[-1](z_list[-1]), att_c), dim=-1)), dim=1)
