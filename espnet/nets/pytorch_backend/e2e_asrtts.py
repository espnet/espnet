...
class Reporter(chainer.Chain):
    ...
    def report(self, loss_att, acc, cer, wer, mtl_loss):
        reporter.report({'loss_att': loss_att}, self)
        reporter.report({'acc': acc}, self)
        reporter.report({'cer': cer}, self)
        reporter.report({'wer': wer}, self)
        logging.info('att loss:' + str(loss_att))
        reporter.report({'loss': loss_att}, self)
...
class E2E(torch.nn.Module):
    ...
    def forward(self, xs_pad, ilens, ys_pad, spembs=None):
        ...
        if loss_data < CTC_LOSS_THRESHOLD and not math.isnan(loss_data):
            self.reporter.report(loss_att_data, acc, cer, wer, loss_data)
        else:
