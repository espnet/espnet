...
from espnet.nets.pytorch_backend.lip2wav.encoder import Encoder
...
class Tacotron2(TTSInterface, torch.nn.Module):
    """LTS Tacotron2 module for LTS.

    This is a module of Tacotron2-based LTS model,
    which convert the sequence of video features (lip features, in fact)
    ...
    def add_arguments(parser):
        ...
        #Special related, Load a pretrained TTS decoder and fix/fintune it. 
        group.add_argument(
            "--pretrained-tts-model", default=None, type=str, help="Pretrained TTS model path"
        )
    ...
    def load_pretrained_tts_model(self, pretrained_tts_model):
        
        import os
        if "snapshot" in os.path.basename(pretrained_tts_model):
            tts_model_state_dict = torch.load(pretrained_tts_model, map_location=lambda storage, loc: storage)[
                "model"
            ]
        else:
            tts_model_state_dict = torch.load(pretrained_tts_model, map_location=lambda storage, loc: storage)

        tts_decoder_state_dict = {k[4:]: v for k, v in tts_model_state_dict.items() if k[:4]=='dec.'}
    
        if hasattr(self, "module"):
            self.module.dec.load_state_dict(tts_decoder_state_dict)
        else:
            self.dec.load_state_dict(tts_decoder_state_dict)

        del tts_model_state_dict
        del tts_decoder_state_dict
    ...
    def __init__(self, idim, odim, args=None):
        ...
        self.enc = Encoder(
            ...
            reduce_time_length=args.reduce_time_length,
        ...
        if args.pretrained_tts_model is not None:
            self.load_pretrained_tts_model(args.pretrained_tts_model)
