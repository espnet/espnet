...
class FeedForwardTransformerLoss(torch.nn.Module):
    ...
    def forward(self, after_outs, before_outs, d_outs, ys, ilens, olens):
...
class FeedForwardTransformer(TTSInterface, torch.nn.Module):
    ...
    def _forward(
        ...
        d_outs = (torch.ones(d_masks.shape)*2).to(xs.device).long()
        d_outs = d_outs.masked_fill(d_masks, 0.0)
        if is_inference:
    ...
    def forward(self, xs, ilens, ys, olens, spembs=None, extras=None, *args, **kwargs):
        ...
        # for lts, we set the sampling rate of input (image) to be 20ms, 
        # and the sampling rate of output (mel) to be 10ms.
        for i in range(len(ilens)):
            if olens[i] > ilens[i]*2:
                olens[i] = ilens[i]*2
            elif olens[i] < ilens[i]*2:
                ilens[i] = olens[i] // 2
                olens[i] = ilens[i]*2
        xs = xs[:, : max(ilens)]
        ys = ys[:, : max(olens)]
        ...
        if self.postnet is None:
            l1_loss = self.criterion(
                None, before_outs, d_outs, ys, ilens, olens
            )
        else:
            l1_loss = self.criterion(
                after_outs, before_outs, d_outs, ys, ilens, olens
            )
        loss = l1_loss
        report_keys = [
