...
def cov(mat, mu):
    '''Estimate the covariance of the matrix
    Args:   m: Matrix 2-D
            rowvar:
    Return: Covariance of the matrix
    '''
    # assert mat.dim() == 2
    if mat.size(0) != 1 and mu is None:
        mu = mat.mean(1, keepdim=True)
    return (mat - mu).t().mm(mat - mu) / mat.size(0)


def kld_gauss_gauss(xs, ys):
    n_batch, n_hidden = xs.size()
    mu_xs = xs.mean(0, keepdim=True)
    mu_ys = ys.mean(0, keepdim=True)
    cov_xs = cov(xs, mu_xs)
    cov_ys = cov(ys, mu_ys)
    # log |cov(q)| - log |cov(p)|
    log_ratio = cov_ys.slogdet()[1] - cov_xs.slogdet()[1]
    # trace(cov_q^-1 cov_p)
    icov_ys = cov_ys.inverse()
    tr = torch.trace(icov_ys.mm(cov_xs))
    # (mu_q - mu_p).T cov_q^-1 (mu_q - mu_p)
    mu_yx = mu_ys - mu_xs  # (1, n_hidden)
    hess = (mu_yx).mm(icov_ys).mm(mu_yx.t())
...
class HLoss(torch.nn.Module):
    ...
    def forward(self, inp, tgt):
        out = F.softmax(inp, dim=1) * F.log_softmax(inp, dim=1)
        assert out.size(0) == tgt.size(0) == inp.size(0)
        tgt_dim = inp.size(1)
        indices = [i * tgt_dim for i in range(0, int(inp.size(0)))]
        tgt = tgt + torch.LongTensor(indices).cuda()
        out_sel = torch.index_select(out.view(-1), 0, tgt)
        out_sel = -1.0 * out_sel.sum()
...
class Reporter(chainer.Chain):
    ...
    def report_cyc(self, loss_att, acc, loss_cyc, expected_loss):
        reporter.report({'loss_att': loss_att}, self)
        reporter.report({'acc': acc}, self)
        reporter.report({'loss_cyc': loss_cyc}, self)
        logging.info('expected loss:' + str(expected_loss))
        reporter.report({'loss': expected_loss}, self)

    def report_tts(self, l1_loss, mse_loss, bce_loss, tts_loss):
        reporter.report({'loss_l1': l1_loss}, self)
        reporter.report({'loss_mse': mse_loss}, self)
        reporter.report({'loss_bce': bce_loss}, self)
        reporter.report({'loss': tts_loss}, self)

    def report_asr2tts(self, asr_loss, acc, l1_loss, mse_loss, bce_loss, tts_loss):
        reporter.report({'loss_att': asr_loss}, self)
        reporter.report({'acc': acc}, self)
        reporter.report({'loss_l1': l1_loss}, self)
        reporter.report({'loss_mse': mse_loss}, self)
        reporter.report({'loss_bce': bce_loss}, self)
...
class Loss(torch.nn.Module):
    ...
    def __init__(self, predictor, mtlalpha, weight=1.0):
    ...
    def forward(self, xs_pad, ilens, ys_pad):
        ...
        alpha = self.mtlalpha
        if alpha == 0:
            self.loss = loss_att * self.weight
            loss_att_data = float(loss_att)
            loss_ctc_data = None
        elif alpha == 1:
            self.loss = loss_ctc * self.weight
            loss_att_data = None
            loss_ctc_data = float(loss_ctc)
        else:
            self.loss = (alpha * loss_ctc + (1 - alpha) * loss_att) * self.weight
            loss_att_data = float(loss_att)
            loss_ctc_data = float(loss_ctc)
...
class DualLoss(torch.nn.Module):
    ...
    def __init__(self, asr, tts, mtlalpha, weight=1.0, n_samples_per_input=5):
    ...
    def forward(self, xs_pad, ilens, ys_pad):
        ...
        :rtype: torch.Tensor
        '''
        # ASR - Encoder (Speech Encoder - E_s)
        torch.set_grad_enabled(self.training)
        filtered_index = filter(lambda i: len(xs_pad[i]) > 0, range(len(xs_pad)))
        sorted_index = sorted(filtered_index, key=lambda i: -len(xs_pad[i]))
        if len(sorted_index) != len(xs_pad):
            logging.warning('Target sequences include empty tokenid (batch %d -> %d).' % (
                len(xs_pad), len(sorted_index)))
        xs_pad = [xs_pad[i] for i in sorted_index]

        # subsample frame
        xs_pad = [xx[::self.subsample[0], :] for xx in xs_pad]
        ilens = np.fromiter((xx.shape[0] for xx in xs_pad), dtype=np.int64)
        if self.training:
            hs = [to_cuda(self, xx) for xx in xs_pad]
        else:
            hs = [to_cuda(self, xx) for xx in xs_pad]
        # 1. encoder
        xpad = pad_list(hs, 0)
        ...
        hpad, hlens = mask_by_length_and_multiply(hpad, hlens, 0, self.n_samples_per_input)
        #if freeze_encoder:
        ...
        y_gen = to_cuda(self, torch.from_numpy(y_gen))
        ylens = to_cuda(self, torch.from_numpy(ylens))
...
class ExpectedLoss(torch.nn.Module):
    def __init__(self, predictor, args, loss_fn=None, reporter=None, rnnlm=None,
                 lm_loss_weight=1.0, softargmax=False):
        ...
        self.oracle = args.sample_oracle
        self.rnnlm = rnnlm
    ...
    def forward(self, xs, ilens, ys, spembs=None):
        ...
        loss_ctc, loss_att, ys, ygen, y_all, ylens, hpad, hlens = \
        ...
        if alpha == 0:
            loss = loss_att
            loss_att_data = loss_att.detach().cpu().numpy()
            # loss_ctc_data = None
        elif alpha == 1:
            loss = loss_ctc
            loss_att_data = None
            # loss_ctc_data = loss_ctc.detach().cpu().numpy()
        else:
        ...
        # Get (log) posterior probabilities from loss. We need to normalize
        # within the samples (batch_size * n_samples_per_input)
        if self.policy_gradient:  # use log posterior probs
            weight = self.sample_scaling * -loss
        else:  # use posterior probs
            weight = (self.sample_scaling * -loss).view(len(xs), self.n_samples_per_input)
        ...
        if self.rnnlm:
            with torch.no_grad():
                ...
                for i in six.moves.range(1, ylens[0]):
                    ...
                    elif self.rnnloss == 'kld':
                        #logging.info("Using KL divergence between prob distributions")
                        lm_loss = 0.0
                        loss_i = kld_gauss_gauss(F.softmax(lmz), F.softmax(y_all[:][i]))
                    elif self.rnnloss == 'mmd':
                        #logging.info("Using MMD between distributions")
                        lm_loss = 0.0
                        loss_i = mmd(lmz, y_all[:][i])
                    if (self.rnnloss != 'kld') and (self.rnnloss != 'mmd'):
                        loss_i[ygen[:, i] == 0] = 0
                        lm_loss += loss_i
                    else:
                        lm_loss += loss_i
                if self.rnnloss != 'kld':
                    lm_loss = lm_loss / ylens.type_as(lm_loss)
                else:
        ...
        if self.loss_fn is not None:
            ...
            self.loss = ((taco_loss - taco_loss.mean()) * weight).mean()
            #self.loss = (taco_loss).mean()
        else:
            if lm_loss is not None:
                self.loss = (-weight + lm_loss * self.lm_loss_weight).mean()
            else:
        ...
        if self.loss_fn is not None:
            if self.policy_gradient:  # use log posterior probs
                ...
                loss_data = loss_cyc_data
            else:
                loss_cyc_data = float((taco_loss * weight.detach()).sum()) / len(xs)
                loss_data = loss_cyc_data
        else:
            loss_cyc_data = None
            loss_data = self.sample_scaling * loss_att_data
        if loss_data < CTC_LOSS_THRESHOLD and not math.isnan(loss_data):
            self.reporter.report_cyc(loss_att_data, acc, loss_cyc_data, loss_data)
        else:
...
class E2E(torch.nn.Module):
    ...
    def generate(self, xs, ys, n_samples_per_input=10, topk=0, maxlenratio=1.0,
        ...
        ilens = np.fromiter((xx.shape[0] for xx in xs), dtype=np.int64)
        if self.training:
            hs = [to_cuda(self, xx) for xx in xs]
        else:
            hs = [to_cuda(self, xx) for xx in xs]
        ...
        if oracle:
            ...
            ys = [to_cuda(self, torch.from_numpy(y)) for y in ys for _ in range(n_samples_per_input)]
...
class AttLoc(torch.nn.Module):
    ...
    def forward(self, enc_hs_pad, enc_hs_len, dec_z, att_prev, scaling=2.0):
        ...
        if att_prev is None:
            # if no bias, 0 0-pad goes 0
            att_prev = to_cuda(self, make_pad_mask(enc_hs_len).float())
...
class Decoder(torch.nn.Module):
    ...
    def forward(self, hs_pad, hlens, ys_pad, zero_att=False, rnnlm=None):
        '''Decoder forward
        ...
        ndist = normal.Normal(torch.tensor([0.0]), torch.tensor([1.0]))

        # loop for an output sequence
        for i in six.moves.range(olength):
            if zero_att:
                #att_z = ndist.sample((ys_out_pad.size(0), self.eprojs)).squeeze(2).cuda()
        ...
        self.loss = F.cross_entropy(y_all, ys_out_pad.view(-1),
                                    ignore_index=self.ignore_id, reduction='elementwise_mean')
        # -1: eos, which is removed in the loss computation
        self.loss *= (np.mean([len(x) for x in ys_in]) - 1)
        #norm = np.mean([len(x) for x in ys_in]) - 1
        #self.loss = self.loss / norm
    ...
    def generate(self, hpad, hlen, topk=0, maxlenratio=1.0, minlenratio=0.3, rnnlm=None):
        ...
        y = to_cuda(self, torch.from_numpy(np.full(n_samples, self.sos, dtype=np.int64)))
        ...
        suppress_mask = to_cuda(self, torch.from_numpy(suppress_mask))
        ...
        for i in six.moves.range(maxlen):
            if i > 0:
                yg = y_gen[i - 1]
                yg[yg == -1] = 0
                y = to_cuda(self, torch.from_numpy(yg))
            ...
            t = to_cuda(self, torch.from_numpy(y_gen[i]))
...
class VGG2L(torch.nn.Module):
    ...
    def forward(self, xs_pad, ilens):
        ...
        xs_pad = F.max_pool2d(xs_pad, 2, stride=2, ceil_mode=True)
        ilens = np.array(
            np.ceil(np.array(torch.tensor(ilens).cpu(), dtype=np.float32) / 2), dtype=np.int64)
        ilens = np.array(
