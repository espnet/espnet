...
class TimeSyncBeamSearch(torch.nn.Module):
    ...
    def cached_score(
        self, h: Tuple[int], cache: dict, scorer: ScorerInterface
    ) -> Any:
        root = h[:-1]   #prefix
        if root in cache:
            ...
            root_scores, root_state = scorer.score(torch.tensor(root).long(), root_root_state, self.enc_output)
            root_log_sum = cache[root_root].log_sum + float(cache[root_root].scores[root[-1]])
    ...
    def joint_score(
        ...
        for h in hyps:
            ...
            if len(h) > 1 and self.decoder_weight > 0 and self.decoder is not None:
                score += self.cached_score(h, self.attn_cache, self.decoder)*self.decoder_weight #attn score
            if len(h) > 1 and self.lm_weight > 0 and self.lm is not None:  
                score += self.cached_score(h, self.lm_cache, self.lm)*self.lm_weight #lm score
            score += self.penalty*(len(h)-1)    #penalty score
            scores[h] = score
        return scores

    def time_step(
        self, p_ctc: Any, ctc_score_dp: Any, hyps: Any
    ) -> Any:
