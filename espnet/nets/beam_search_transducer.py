...
def greedy_search(decoder, h, recog_args):
    ...
    for i, hi in enumerate(h):
        ...
        if pred != decoder.blank:
            ...
            hyp["dec_state"], hyp["att_state"] = state
        ...
        while True:
            ...
            for logp, k in zip(*ytu):
                ...
                if k == decoder.blank:
                    ...
                    new_hyp["dec_state"], new_hyp["att_state"] = state
...
def time_sync_decoding(decoder, h, recog_args, rnnlm=None):
    ...
    beam_state = (decoder.init_state(torch.zeros((beam, decoder.dunits))), None)
                ...
                for i, hyp in enumerate(C):
                    for logp, k in zip(beam_topk[0][i], beam_topk[1][i]):
                        ...
                        (
                            new_hyp["dec_state"],
                            new_hyp["att_state"],
                        ) = decoder.select_state(beam_state, i)
        ...
        for n in range(nstep):
            ...
            if n < (nstep - 1):
                for i, v in enumerate(V):
                    ...
                    v["dec_state"], v["att_state"] = decoder.select_state(beam_state, i)
