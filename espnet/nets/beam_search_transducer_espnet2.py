...
class BeamSearchTransducerESPnet2:
    ...
    def greedy_search(self, h: torch.Tensor) -> List[Hypothesis]:
        ...
        for i, hi in enumerate(h):
            ytu = torch.log_softmax(self.decoder.joint(hi, y[0]), dim=-1)
    ...
    def time_sync_decoding(self, h: torch.Tensor) -> List[Hypothesis]:
        ...
        for hi in h:
            ...
            for v in range(self.max_sym_exp):
                ...
                beam_logp = torch.log_softmax(self.decoder.joint(h_enc, beam_y), dim=-1)
                ...
                for i in range((j + 1), len(hyps)):
                    if (
                        ...
                        ytu = torch.log_softmax(
                            self.decoder.joint(hi, hyps[i].y[-1]), dim=0
                        )
                        ...
                        for k in range(next_id, (len(hyps[j].yseq) - 1)):
                            ytu = torch.log_softmax(
                                self.decoder.joint(hi, hyps[j].y[k]), dim=0
                            )
            ...
            for n in range(self.nstep):
                ...
                if n < (self.nstep - 1):
                    ...
                    beam_logp = torch.log_softmax(
                        self.decoder.joint(h_enc, beam_y), dim=-1
                    )
