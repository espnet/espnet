...
def remove_output_layer(pretrained_model, odim, eprojs, dunits, model_type):
    ...
    bs = torch.nn.init.uniform_(torch.FloatTensor(odim).cuda(),
                               -stdv_bias, stdv_bias)
    if model_type == 'asr':
        pretrained_model['predictor.ctc.ctc_lo.weight'] = wt
        pretrained_model['predictor.ctc.ctc_lo.bias'] = bs
        pretrained_model['predictor.dec.embed.weight'] = ewt
        pretrained_model['predictor.dec.output.weight'] = eot
        pretrained_model['predictor.dec.output.bias'] = bs
    elif model_type == 'tts':
