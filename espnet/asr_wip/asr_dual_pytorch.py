...
class CustomUpdater(training.StandardUpdater):
    ...
    def __init__(self, asr2tts_model, tts2asr_model, asr_model, tts_model,
                 ...
                 optimizer_A, optimizer_B, asr_converter, tts_converter, device, ngpu,
                 update_asr_only=False, freeze_asr=False, freeze_tts=False):
        super(CustomUpdater, self).__init__(train_iter, optimizer_A, optimizer_B)
    ...
    def update_core(self):
        ...
        optimizer_A = self.get_optimizer('main')
        ...
        if np.array_equal(batch[0][0][0], batch[0][1][0]):
            if isinstance(batch[0][0][0][0], np.int64):
                logging.info("Batch id: %s is text" % self.count)
                ...
                logging.info("Batch id: %s is speech" % self.count)
                x = self.asr_converter(batch, self.device)
                #self.asr_model.train()
                self.tts_model.train()
                loss_A = 1. / self.ngpu * self.asr2tts_model(*x)
                optimizer_A.zero_grad()  # Clear the parameter gradients
                if self.ngpu > 1:
                    loss_A.sum().backward()
                else:
                    loss_A.backward()
                loss_A.detach()  # Truncate the graph
                # compute the gradient norm to check if it is normal or not
                grad_norm_A = torch.nn.utils.clip_grad_norm_(
                    self.asr2tts_model.parameters(), self.grad_clip_threshold)
                logging.info('grad norm A={}'.format(grad_norm_A))
                if math.isnan(grad_norm_A):
                    logging.warning('grad norm is nan. Do not update model.')
                else:
                    optimizer_A.step()
...
def train(args):
    ...
    if args.opt == 'adadelta':
        optimizer_A = torch.optim.Adadelta(
            asr2tts_model.parameters(), rho=0.95, eps=args.eps)
        optimizer_B = torch.optim.Adadelta(
            tts2asr_model.parameters(), rho=0.95, eps=args.eps)
    elif args.opt == 'adam':
        optimizer_A = torch.optim.Adam(asr2tts_model.parameters())
        optimizer_B = torch.optim.Adam(tts2asr_model.parameters())

    # FIXME: TOO DIRTY HACK
    setattr(optimizer_A, "target", reporter)
    setattr(optimizer_B, "target", reporter)
    setattr(optimizer_A, "serialize", lambda s: reporter.serialize(s))
    ...
    train_subsets_A = make_batchset(train_json[0], args.batch_size, args.maxlen_in,
                                    args.maxlen_out, args.minibatches,
                                    min_batch_size=args.ngpu if args.ngpu > 1 else 1)
    train_subsets_B = make_batchset(train_json[1], args.batch_size, args.maxlen_in,
                                    args.maxlen_out, args.minibatches,
                                    min_batch_size=args.ngpu if args.ngpu > 1 else 1)
    ...
    # Save best models
    trainer.extend(extensions.snapshot_object(asr2tts_model, 'model_A.loss.best', savefun=torch_save),
                   trigger=training.triggers.MinValueTrigger('validation/main/loss'))
    trainer.extend(extensions.snapshot_object(tts2asr_model, 'model_B.loss.best', savefun=torch_save),
                   trigger=training.triggers.MinValueTrigger('validation/main/loss'))

    if mtl_mode is not 'ctc':
        trainer.extend(extensions.snapshot_object(asr2tts_model, 'model_A.acc.best', savefun=torch_save),
    ...
    if args.opt == 'adadelta':
        if args.criterion == 'acc' and mtl_mode is not 'ctc':
            trainer.extend(restore_snapshot(asr2tts_model, args.outdir + '/model_A.acc.best', load_fn=torch_load),
