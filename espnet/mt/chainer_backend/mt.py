...
# from espnet.asr.asr_utils import chainer_resume
...
# duplicate
# from espnet.asr.chainer_backend.asr import CustomEvaluator
...
class CustomConverter(object):
    ...
    def __call__(self, batch, device):
        ...
        xs_pad = F.pad_sequence(xs, padding=self.pad)
        xs_pad.to_device(device)
        ys_pad = F.pad_sequence(ys, padding=self.ignore_id)
        ys_pad.to_device(device)
...
def train(args):
    ...
    # Set up a trainer
    if ngpu <= 1:
        updater = CustomUpdater(train_iter, optimizer, converter, device=gpu_id)
    ...
    if args.opt == 'adadelta':
        if args.criterion == 'acc':
            trainer.extend(restore_snapshot(model, args.outdir + '/model.acc.best', load_fn=chainer_load),
            ...
            trainer.extend(restore_snapshot(model, args.outdir + '/model.loss.best', load_fn=chainer_load),
                           trigger=CompareValueTrigger(
