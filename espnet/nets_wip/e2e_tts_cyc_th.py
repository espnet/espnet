...
from espnet.nets.e2e_asr_cyc_th import AttForward
from espnet.nets.e2e_asr_cyc_th import AttForwardTA
from espnet.nets.e2e_asr_cyc_th import AttLoc
from espnet.nets.e2e_asr_cyc_th import to_cuda
...
class Tacotron2TTELoss(torch.nn.Module):
    ...
    def forward(self, xs, ilens, ys, labels, olens=None, spembs=None):
        ...
        with torch.no_grad():
            ...
            sorted_olens, indices = torch.sort(olens.cpu(), descending=True)
        ...
        if self.use_masking and hlens is not None:
            ...
            weights = weights.masked_select(mask[:, :, 0]) if weights is not None else None
            # calculate loss
            l1_loss = F.l1_loss(after_outs, hpad, reduce=self.reduce) + F.l1_loss(
                before_outs, hpad, reduce=self.reduce)
            mse_loss = F.mse_loss(after_outs, hpad, reduce=self.reduce) + F.mse_loss(
                before_outs, hpad, reduce=self.reduce)
            ...
            # calculate loss
            l1_loss = F.l1_loss(after_outs, hpad, reduce=self.reduce) + F.l1_loss(before_outs,
                                                                                  hpad, reduce=self.reduce)
            mse_loss = F.mse_loss(after_outs, hpad, reduce=self.reduce) + F.mse_loss(before_outs,
                                                                                     hpad, reduce=self.reduce)
            if self.use_bce_loss:
...
class ASR2Tacotron2Loss(torch.nn.Module):
    """ASR2 TACOTRON2 LOSS FUNCTION
    ...
    def __init__(self, tts_model, asr_model, reporter=None, weight=1.0, rnnlm=None):
        super().__init__()
    ...
    def forward(self, xs, ilens, olens=None, spembs=None):
        ...
        logging.warn("both false")
        #set_requires_grad([self.asr_model.enc], False)
        #set_requires_grad([self.asr_model.dec], False)

        with torch.no_grad():
            outs, flens = self.asr_model.enc(xs, ilens)

        with torch.no_grad():
            asr_loss, ys, ygen, ylens, yall = self.asr_model.dec.generate(outs, flens)
        ilens = [len(yall) for i in six.moves.range(flens.size(0))]
        yall = torch.stack(yall).reshape(flens.size(0), len(yall), -1)

        torch.set_grad_enabled(self.training)
        set_requires_grad([self.tts_model], False)
        after_outs, before_outs, logits = self.tts_model.softgenerate(yall, ilens, xs, spembs)

        olens = [logits.size(1) for i in six.moves.range(logits.size(0))]

        # calculate loss
        olens = torch.LongTensor(olens)
        labels = torch.zeros(after_outs.size(0), after_outs.size(1), dtype=torch.float32).cuda()
        labels[torch.arange(after_outs.size(0), dtype=torch.int64), olens - 1] = 1
        ys = xs[:, :max(olens)]
        # prepare weight of positive samples in cross entorpy
        #if self.bce_pos_weight != 1.0:
        #    weights = ys.new(*labels.size()).fill_(1)
        ...
        bce_loss = F.binary_cross_entropy_with_logits(logits, labels, weights, reduce=False)
        # integrate loss
        loss = l1_loss.mean(0) + mse_loss.mean(0) + bce_loss.mean(0)
        ygen = torch.tensor(ygen).cuda()
        if self.rnnlm:
            with torch.no_grad():
                sos= ygen.data.new([self.asr_model.sos] * len(ygen))
                rnnlm_state, lmz = self.rnnlm.predictor(None, sos.cuda())
        ...
        if lm_loss is not None:
            loss += lm_loss.mean(0)
        # report loss values for logging
        logging.info("loss = asr: " + str(asr_loss) + ", mse: " + str(mse_loss) + ", loss:  " + str(loss))
        loss = loss + asr_loss.mean(0)
        self.reporter.report_asr2tts(asr_loss.mean(0).detach().cpu().numpy(),
                                     acc,
                                     l1_loss.mean(0).detach().cpu().numpy(),
                                     mse_loss.mean(0).detach().cpu().numpy(),
                                     bce_loss.mean(0).detach().cpu().numpy(),
                                     loss.mean(0).detach().cpu().numpy())

        return loss


class TacotronTTS2ASRLoss(torch.nn.Module):
    ...
    def forward(self, xs, ilens, ys, labels, olens=None, spembs=None):
        ...
        hpad, hlens = self.asr_model.enc(outs, flens)
        asr_loss, acc = self.asr_model.dec(hpad, hlens, xs)
        # calculate loss
...
class Tacotron2(torch.nn.Module):
    ...
    def softgenerate(self, xs, ilens, ys, spembs=None, anneal=100):
        """TACOTRON2 FEATURE GENERATION

        :param torch.Tensor xs: batch of padded character embeddings (B, Tmax)
        :param list ilens: list of lengths of each input batch (B)
        :param torch.Tensor spembs: batch of speaker embedding vector (B, spk_embed_dim)
        :return: outputs with postnets (B, Lmax, odim)
        :rtype: torch.Tensor
        ...
        if isinstance(ilens, torch.Tensor) or isinstance(ilens, np.ndarray):
            ilens = list(map(int, ilens))
        hs, hlens = self.enc.softforward(xs, ilens, anneal)
        if self.spk_embed_dim is not None:
            spembs = F.normalize(spembs).unsqueeze(1).expand(-1, hs.size(1), -1)
            hs = torch.cat([hs, spembs], dim=-1)
        after_outs, before_outs, logits = self.dec(hs, hlens, ys)
...
class Encoder(torch.nn.Module):
    ...
    def __init__(self, idim,
        ...
        self.softembed = torch.nn.Linear(self.idim, self.embed_dim)
    ...
    def softforward(self, xs, ilens, anneal):
        """CHARACTER ENCODER FORWARD CALCULATION
        ...
        y_one_hot = torch.log_softmax(xs * anneal, dim=-1)
        embedding = self.embed.weight
        y_embed = torch.matmul(torch.exp(y_one_hot), embedding).view(len(xs), len(xs[0]), -1)

        xs = self.softembed(xs).transpose(1, 2)
        for l in six.moves.range(self.econv_layers):
            if self.use_residual:
                xs += self.convs[l](xs)
            else:
