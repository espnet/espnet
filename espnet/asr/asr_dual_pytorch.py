...
from espnet.nets.e2e_asr_dual_th import E2E
from espnet.nets.e2e_asr_dual_th import Loss
from espnet.nets.e2e_asr_cyc_th import pad_list

from espnet.nets.e2e_asr_dual_th import ExpectedLoss
...
class CustomUpdater(training.StandardUpdater):
    ...
    def __init__(self, asr2tts_model, tts2asr_model, asr_model, tts_model,
                 ...
                 optimizer, asr_converter, tts_converter, cyc_converter,
                 device, ngpu,
                 update_asr_only=False, freeze_asr=False, freeze_tts=False, alpha=0.5, zero_att=False):
        ...
        self.cyc_converter = cyc_converter
        self.device = device
        self.ngpu = ngpu
    ...
    def update_core(self):
        ...
        if utt_type == 'unpaired':
            ...
            xs_asr, ilens_asr, ys_asr, xs_tts, ys_tts, ilens_tts, labels, olens_tts, spembs = self.cyc_converter(batch, self.device)
            ...
            #self.tts_model.eval()
            self.asr_model.train()
            loss_tts2asr = self.tts2asr_model(xs_tts, ilens_tts, ys_tts, labels, olens_tts, spembs, zero_att=self.zero_att)
        ...
        if math.isnan(grad_norm):
            ...
            #if utt_type != 'unpaired':
                #logging.warn("Finally saw a paired grad, os updating and grad norm is " + str(grad_norm))
            optimizer.step()
        #if utt_type != 'unpaired':
        #    optimizer.zero_grad()  # Clear the parameter gradients

class CustomDualConverter(object):
    """CUSTOM CONVERTER"""
...
def train(args):
    ...
    if args.asr_model:
        if args.modify_output:
            odim = int(valid_json[utts[0]]['output'][0]['shape'][1])
            if args.asr_model_conf:
                e2e = E2E(idim, odim, args) #, args.char_list, args)
            else:
                e2e = E2E(idim, odim, args)
            asr_model = Loss(e2e, args.mtlalpha, weight=args.teacher_weight)
    ...
    if args.expected_loss:
        ...
        if args.check:
            asr2tts_model = ASR2Tacotron2Loss(loss_fn, asr_model.predictor,
    ...
    # Setup a converter
    converter = CustomConverter(e2e.subsample[0], args.use_speaker_embedding)
    #converter = CustomConverter(e2e.subsample[0])
    #converter = ASRConverter(e2e.subsample[0])
    ...
    updater = CustomUpdater(
        ...
        TTSConverter(),
        CustomDualConverter(args.use_speaker_embedding),
        device,
        args.ngpu,
