...
def batchfy_by_bin(data, batch_bins, num_batches=0, min_batch_size=1, shortest_first=False):
    ...
    sorted_data = sorted(data.items(), key=lambda sample: int(
        sample[1]['input'][0]['shape'][0]), reverse=not shortest_first)
