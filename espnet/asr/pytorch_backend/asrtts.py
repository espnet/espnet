...
class Reporter(chainer.Chain):
    ...
    def report(self, pa_loss_att, sa_loss_att, ta_loss_att, ta_acc, acc, cer, wer, sa_loss_mse, ta_loss_bce):
        reporter_module.report({'pa_loss_att': pa_loss_att}, self)
        reporter_module.report({'sa_loss_att': sa_loss_att}, self)
        reporter_module.report({'ta_loss_att': ta_loss_att}, self)
        reporter_module.report({'ta_acc': ta_acc}, self)
        reporter_module.report({'acc': acc}, self)
        reporter_module.report({'cer': cer}, self)
        reporter_module.report({'wer': wer}, self)
        reporter_module.report({'sa_loss_mse': sa_loss_mse}, self)
        reporter_module.report({'ta_loss_bce': ta_loss_bce}, self)
...
def train(args):
    ...
    if args.asr_model_conf:
        ...
        asr_model = asrtts(idim, odim, train_args, reporter=sa_reporter)
    else:
        asr_model = asrtts(idim, odim, args, reporter=sa_reporter)
    if args.asr_model:
    ...
    if args.expected_loss == 'tts':
        ...
        loss_fn = load_tacotron_loss(args.tts_model_conf, args.tts_model, args, sa_reporter)
    ...
    asr2tts_model = asrtts(idim, odim, args, predictor=asr_model,
                           loss_fn=loss_fn, rnnlm=rnnlm, softargmax=args.softargmax,
                           asr2tts=True, reporter=sa_reporter)
