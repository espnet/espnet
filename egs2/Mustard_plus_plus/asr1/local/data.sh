...
. ./path.sh
. ./cmd.sh
ASVSpoof_CMD=/scratch/bbjs/shared/corpora/Mustard_plus_plus
...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
    if [ ! -e "${ASVSpoof_CMD}/mustard++_text.csv" ]; then
	    echo "stage 1: Download data to ${ASVSpoof_CMD}"
        # git clone https://github.com/kolesov93/lt_speech_commands.git ${LT_SPEECH_CMD}
    else
        log "stage 1: ${ASVSpoof_CMD}/mustard++_text.csv is already existing. Skip data downloading"
    fi
fi
