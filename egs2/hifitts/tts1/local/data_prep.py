...
def main():
    ...
    all_manifests = [os.path.join(dataset_path, fname) for fname in os.listdir(dataset_path) if fname.endswith(".json")]
    train_manifests = [manifest for manifest in all_manifests if re.search("train", manifest)]
    dev_manifests = [manifest for manifest in all_manifests if re.search("dev", manifest)]
    test_manifests = [manifest for manifest in all_manifests if re.search("test", manifest)]
