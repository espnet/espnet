    ...
    if model_name not in tts_models:
        ...
        if model_name in ESPNET_TTS_OPTIONS:
            ...
            try:
                ...
                tts_models[model_name] = Text2Speech.from_pretrained(
...
def load_asr_model(model_name):
    ...
    if (
        model_name not in asr_models
        and model_name not in espnet_models
        and model_name not in owsm_models
    ):
...
def load_llm_model(model_name):
    ...
    if model_name not in llm_models:
        ...
        try:
            ...
            llm_models[model_name] = {"model": model, "tokenizer": tokenizer}
...
def process_speech_and_clear(
    audio_input, asr_option, llm_option, system_prompt, tts_option=None, clear=True
):
    ...
    try:
        ...
        return (
            ...
            "I couldn't process your speech properly. Please try again.",
            None,
        )
...
def create_demo():
    with gr.Blocks(
        title="Speech Conversation System",
        css="""
    ...
    """,
        ...
        with gr.Row():
            with gr.Column(scale=2):
                ...
                system_prompt = gr.Textbox(
                    label="System Prompt", value="You are a helpful AI assistant."
                )
                ...
                tts_dropdown = gr.Dropdown(
                    choices=tts_choices, value=tts_choices[0], label="Select TTS Model"
                )

                with gr.Row():
                    submit_btn = gr.Button(
                        "Process Speech", variant="primary", elem_id="submit-btn"
                    )
        ...
        ).then(display_latency, inputs=[], outputs=[latency_info]).then(
        ...
        ).then(display_conversation, inputs=[], outputs=[conversation_display]).then(
            fn=lambda: show_record_message() if t else "",
            inputs=[],
            outputs=[record_message],
        )
