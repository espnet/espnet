...
class ChatAgent(BaseAgent):
    ...
    def step(self, state: State) -> Action:
        ...
        prompt = get_prompt(
            state.conversation, state.dst, state.history[-1]["action"]["type"]
        )
        ...
        if action_type == "chat":
            ...
            action = ChatAction(
                thought="Invalid action type",
                payload="There was an error parsing the action type. Please try again.",
            )
        return action
    ...
    def parse_response(self, response: str) -> tuple[str, str, str]:
        ...
        thought_match = re.search(
            r"<thought>\s*(.*?)\s*</thought>", response, re.DOTALL
        )
        action_match = re.search(r"<action>\s*(.*?)\s*</action>", response, re.DOTALL)
        payload_match = re.search(
            r"<payload>\s*(.*?)\s*</payload>", response, re.DOTALL
        )
        ...
        action = (
            action_match.group(1).strip() if action_match else "chat"
        )  # Default to chat if no action found
        payload = (
            payload_match.group(1).strip() if payload_match else "No payload provided"
        )
