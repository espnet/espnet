...
class State:
    ...
    def get_tts_output(self):
        ...
        if self.dst_class is None:
            initial_dst_classify = DSTAction(
                thought="Have to get DST category", payload=self.history
            ).execute(type="get_category")
        ...
        self.dst = DSTAction(
            thought="Have to get DST for conversation", payload=self.history
        ).execute(type=self.dst_class)
        return self.dst
