...
class Controller:
    ...
    def add_user_input(self, user_input):
        ...
        if (
            len(self.state.history) > 0
            and self.state.history[-1]["observation"]["payload"] is None
        ):
    ...
    def run(self):
        while True:
            ...
            if isinstance(action, ChatAction):
                self.state.conversation.extend(
                    [
            ...
            if self.state.dst_class is None:
                self.state.dst_class = DSTAction(
                    thought="Have to get DST category", payload=state.conversation
                ).execute(type="get_category")["output"]
            else:
                self.state.dst = DSTAction(
                    thought="Have to get DST category", payload=self.state.conversation
                ).execute(type=self.state.dst_class)
