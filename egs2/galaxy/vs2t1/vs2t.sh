...
if [ -z "${s2t_stats_dir}" ]; then
    ...
    if [ "${use_mask_frame}" = true ]; then
        s2t_stats_dir="${s2t_stats_dir}_mask"
    fi
fi
...
if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ] && ! [[ " ${skip_stages} " =~ [[:space:]]4[[:space:]] ]]; then
    ...
    echo "lm_train_text ${lm_train_text[@]}"
...
if [ ${stage} -le 10 ] && [ ${stop_stage} -ge 10 ] && ! [[ " ${skip_stages} " =~ [[:space:]]10[[:space:]] ]]; then
    ...
    if ${use_mask_frame}; then
        _opts+="--train_data_path_and_name_and_type ${_s2t_train_dir}/${mask_frame},${mask_frame},text_int "
        # _opts+="--valid_data_path_and_name_and_type ${_s2t_valid_dir}/${mask_frame},${mask_frame},text "
    fi
    # shellcheck disable=SC2046,SC2086
    ${train_cmd} JOB=1:"${_nj}" "${_logdir}"/stats.JOB.log \
    ...
    if ${use_mask_frame}; then
        _mask_frame=${mask_frame//./_}
        <"${s2t_stats_dir}/train/${mask_frame}_shape" \
            awk -v N="$(<${token_list} wc -l)" '{ print $0 "," N }' \
            >"${s2t_stats_dir}/train/${mask_frame}_shape.${token_type}"
    fi
    if ${use_visual_feature}; then
...
if [ ${stage} -le 11 ] && [ ${stop_stage} -ge 11 ] && ! [[ " ${skip_stages} " =~ [[:space:]]11[[:space:]] ]]; then
    ...
    if [ "${num_splits_s2t}" -gt 1 ]; then
        ...
        if ${use_mask_frame}; then
            _opts+="--fold_length ${s2t_text_fold_length} "
            _opts+="--train_data_path_and_name_and_type ${_split_dir}/${mask_frame},${mask_frame//./_},text_int "
            _opts+="--train_shape_file ${_split_dir}/${mask_frame//./_}_shape.${token_type} "
        fi
        _opts+="--multiple_iterator true "
        ...
        if ${use_mask_frame}; then
            _opts+="--fold_length ${s2t_text_fold_length} "
            _opts+="--train_data_path_and_name_and_type ${_s2t_train_dir}/${mask_frame},${mask_frame},npy "
            _opts+="--train_shape_file ${s2t_stats_dir}/train/${mask_frame//./_}_shape.${token_type} "
        fi
    fi
...
if [ ${stage} -le 13 ] && [ ${stop_stage} -ge 13 ] && ! [[ " ${skip_stages} " =~ [[:space:]]13[[:space:]] ]]; then
    ...
    for dset in ${_dsets}; do
        ...
        for _tok_type in "word" "char" "bpe"; do
            ...
            sclite \
                ${score_opts} \
                -r "${_scoredir}/cleaned_ref.trn" trn \
