...
nbpe=40000
# s2t config based on owsm-ctc v4, modified to be comparable with powsm
s2t_config=conf/tuning/train_conv2d_size768_e9_d9_mel80.yaml

# Specify nodelist based on your environment with --host "$nodelist"
# nodelist=$(scontrol show hostnames $SLURM_JOB_NODELIST)
# nodelist=$(echo $nodelist | tr ' ' ',')

# Run stage 1-10 for data prep, bpe, collect stats in egs2/powsm/s2t1
./s2t.sh \
    --s2t_ctc true \
    --stage 11 \
    --stop_stage 11 \
