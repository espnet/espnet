...
if [ ${stage} -le 7 ] && [ ${stop_stage} -ge 4 ]; then
	...
	if  [ -e "${expdir}/pretrained_model_iter$((iter-1))" ]; then
	    log "Symbolic link ${expdir}/pretrained_model_iter$((iter-1)) already exists, remove it."
	    rm "${expdir}/pretrained_model_iter$((iter-1))"
	fi
	if [ "${iter}" -ge 1 ]; then
