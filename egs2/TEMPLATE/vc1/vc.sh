...
vc_task=non_parallel_vc                # VC task (parallel_vc or non_parallel_vc).
...
token_type=   # Transcription type (char or phn or none).
...
Options:
    ...
    --use_dvector      # Whether to use D-vector (default="${use_dvector}").
    --dvector_tool     # Toolkit for generating the D-vectors (default="${dvector_tool}").
    --dvector_model    # Pretrained model to generate the D-vectors (default="${dvector_model}").
    ...
    --vc_task              # vc task {parallel_vc or non_parallel_vc} (default="${vc_task}").
...
if ! "${skip_data_prep}"; then
    ...
    if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
        ...
        if "${use_xvector}"; then
            if [ "${xvector_tool}" = "kaldi" ]; then
                ...
                for dset in "${train_set}" "${valid_set}" ${test_sets}; do
                    ...
                    echo "=====Copy datadir and resample to 16k====="
        ...
        # Extract D-vector need to fix
        if "${use_dvector}"; then
            if [ "${dvector_tool}" = "kaldi" ]; then
                log "Stage 2+: Extract X-vector: data/ -> ${dumpdir}/xvector (Require Kaldi)"
                # Download X-vector pretrained model
                xvector_exp=${expdir}/xvector_nnet_1a
                if [ ! -e "${xvector_exp}" ]; then
    ...
    if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 && "${token_type}"!=none ]; then
...
if ! "${skip_train}"; then
    if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ]; then
        ...
        echo ${_train_dir}
        log "Stage 5: VC collect stats: train_set=${_train_dir}, valid_set=${_valid_dir}"

        _opts=
        if [ -n "${train_config}" ]; then
            # To generate the config file: e.g.
            #   % python3 -m espnet2.bin.parallel_vc_train --print_config --optim adam
        ...
        echo "feats_extract ${feats_extract}"
        ...
        elif [ "${vc_task}" == "non_parallel_vc" ]; then
            target_scp=${_scp}
            source_scp=${_scp}
        fi
        ...
        ${train_cmd} JOB=1:"${_nj}" "${_logdir}"/stats.JOB.log \
            ${python} -m "espnet2.bin.${vc_task}_train" \
                ...
                --train_data_path_and_name_and_type "${_train_dir}/${source_scp},source_speech,${_type}" \
                --train_data_path_and_name_and_type "${_train_dir}/${target_scp},target_speech,${_type}" \
                --valid_data_path_and_name_and_type "${_valid_dir}/${source_scp},source_speech,${_type}" \
                --valid_data_path_and_name_and_type "${_valid_dir}/${target_scp},target_speech,${_type}" \
        ...
        if [ "${token_type}" != none ]; then
            <"${vc_stats_dir}/train/text_shape" \
                awk -v N="$(<${token_list} wc -l)" '{ print $0 "," N }' \
                >"${vc_stats_dir}/train/text_shape.${token_type}"

            <"${vc_stats_dir}/valid/text_shape" \
                awk -v N="$(<${token_list} wc -l)" '{ print $0 "," N }' \
                >"${vc_stats_dir}/valid/text_shape.${token_type}"
        fi
    fi
    ...
    if [ ${stage} -le 6 ] && [ ${stop_stage} -ge 6 ]; then
        _train_dir="${data_feats}/${train_set}"
        _valid_dir="${data_feats}/${valid_set}"
        log "Stage 6: VC Training: train_set=${_train_dir}, valid_set=${_valid_dir}"

        _opts=
        if [ -n "${train_config}" ]; then
            # To generate the config file: e.g.
            #   % python3 -m espnet2.bin.paralle_vc_train --print_config --optim adam
            _opts+="--config ${train_config} "
        fi

        if [ "${vc_task}" == "non_parallel_vc" ]; then
            ################################################################
            #     CASE 1:  Non paralle voice conversion model training     #
            ################################################################
            _scp=wav.scp
            if [[ "${audio_format}" == *ark* ]]; then
                _type=kaldi_ark
            ...
            if [ "${num_splits}" -gt 1 ]; then
                ...
                _split_dir="${vc_stats_dir}/splits${num_splits}"
                if [ ! -f "${_split_dir}/.done" ]; then
                    ...
                    ${python} -m espnet2.bin.split_scps \
                      --scps \
                          "${_train_dir}/text" \
                          "${_train_dir}/${_scp}" \
                          "${vc_stats_dir}/train/speech_shape" \
                          "${vc_stats_dir}/train/text_shape.${token_type}" \
                ...
                _opts+="--train_shape_file ${vc_stats_dir}/train/text_shape.${token_type} "
                _opts+="--train_shape_file ${vc_stats_dir}/train/speech_shape "
            fi
            ...
            _opts+="--valid_shape_file ${vc_stats_dir}/valid/text_shape.${token_type} "
            _opts+="--valid_shape_file ${vc_stats_dir}/valid/speech_shape "
        else
            #####################################
        ...
        if [ -e "${vc_stats_dir}/train/collect_feats/pitch.scp" ]; then
            _scp=pitch.scp
            _type=npy
            _train_collect_dir=${vc_stats_dir}/train/collect_feats
            _valid_collect_dir=${vc_stats_dir}/valid/collect_feats
        ...
        if [ -e "${vc_stats_dir}/train/collect_feats/energy.scp" ]; then
            _scp=energy.scp
            _type=npy
            _train_collect_dir=${vc_stats_dir}/train/collect_feats
            _valid_collect_dir=${vc_stats_dir}/valid/collect_feats
        ...
        if [ -e "${vc_stats_dir}/train/pitch_stats.npz" ]; then
            _opts+="--pitch_extract_conf fs=${fs} "
            _opts+="--pitch_extract_conf n_fft=${n_fft} "
            _opts+="--pitch_extract_conf hop_length=${n_shift} "
            _opts+="--pitch_extract_conf f0max=${f0max} "
            _opts+="--pitch_extract_conf f0min=${f0min} "
            _opts+="--pitch_normalize_conf stats_file=${vc_stats_dir}/train/pitch_stats.npz "
        fi
        if [ -e "${vc_stats_dir}/train/energy_stats.npz" ]; then
            _opts+="--energy_extract_conf fs=${fs} "
            _opts+="--energy_extract_conf n_fft=${n_fft} "
            _opts+="--energy_extract_conf hop_length=${n_shift} "
            _opts+="--energy_extract_conf win_length=${win_length} "
            _opts+="--energy_normalize_conf stats_file=${vc_stats_dir}/train/energy_stats.npz "
        fi
        ...
        if [ "${feats_normalize}" = "global_mvn" ]; then
            _opts+="--normalize_conf stats_file=${vc_stats_dir}/train/feats_stats.npz "
        fi
...
if ! "${skip_eval}"; then
    if [ ${stage} -le 7 ] && [ ${stop_stage} -ge 7 ]; then
        log "Stage 7: Decoding: training_dir=${vc_exp}"
        ...
        mkdir -p "${vc_exp}/${inference_tag}"; echo "${run_args} --stage 7 \"\$@\"; exit \$?" > "${vc_exp}/${inference_tag}/run.sh"; chmod +x "${vc_exp}/${inference_tag}/run.sh"


        for dset in ${test_sets}; do
            _data="${data_feats}/${dset}"
            _speech_data="${_data}"
            _dir="${vc_exp}/${inference_tag}/${dset}"
            _logdir="${_dir}/log"
            mkdir -p "${_logdir}"
            ...
            log "Decoding started... log: '${_logdir}/vc_inference.*.log'"
            # shellcheck disable=SC2046,SC2086
            ${_cmd} --gpu "${_ngpu}" JOB=1:"${_nj}" "${_logdir}"/vc_inference.JOB.log \
                ${python} -m espnet2.bin.vc_inference \
                    --ngpu "${_ngpu}" \
                    --data_path_and_name_and_type "${_data}/text,text,text" \
                    --data_path_and_name_and_type ${_speech_data}/${_scp},speech,${_type} \
                    --key_file "${_logdir}"/keys.JOB.scp \
                    --model_file "${vc_exp}"/"${inference_model}" \
                    --train_config "${vc_exp}"/config.yaml \
                    --output_dir "${_logdir}"/output.JOB \
                    --vocoder_file "${vocoder_file}" \
                    ${_opts} ${_ex_opts} ${inference_args} || { cat $(grep -l -i error "${_logdir}"/vc_inference.*.log) ; exit 1; }
...
packed_model="${vc_exp}/${vc_exp##*/}_${inference_model%.*}.zip"
if [ -z "${download_model}" ]; then
    ...
    if [ ${stage} -le 8 ] && [ ${stop_stage} -ge 8 ]; then
        ...
        _opts=""
        if [ -e "${vc_stats_dir}/train/feats_stats.npz" ]; then
            _opts+=" --option ${vc_stats_dir}/train/feats_stats.npz"
        fi
        if [ -e "${vc_stats_dir}/train/pitch_stats.npz" ]; then
            _opts+=" --option ${vc_stats_dir}/train/pitch_stats.npz"
        fi
        if [ -e "${vc_stats_dir}/train/energy_stats.npz" ]; then
            _opts+=" --option ${vc_stats_dir}/train/energy_stats.npz"
        fi
        if "${use_xvector}"; then
        ...
        ${python} -m espnet2.bin.pack vc \
            --train_config "${vc_exp}"/config.yaml \
            --model_file "${vc_exp}"/"${inference_model}" \
            --option "${vc_exp}"/images  \
            --outpath "${packed_model}" \
            ${_opts}

        # NOTE(kamo): If you'll use packed model to inference in this script, do as follows
        #   % unzip ${packed_model}
        #   % ./run.sh --stage 8 --vc_exp $(basename ${packed_model} .zip) --inference_model pretrain.pth
    fi
fi
...
git checkout $(git show -s --format=%H)"
        ...
        # /some/where/espnet/egs2/foo/vc1/ -> foo/tt1
...
<li><strong>Config</strong><pre><code>$(cat "${vc_exp}"/config.yaml)</code></pre></li>
</ul>
EOF
        ...
        espnet_model_zoo_upload \
            ...
            --description_file "${vc_exp}"/description \
