...
local_data_feats_type=  # The feats type given to local/data.sh
...
src_lang=dc                # source language abbrev. id (e.g., es)
...
Options:
    ...
    --local_data_feats_type  # The feats type give to local/data.sh (default="${local_data_feats_type}").
...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ] && ! [[ " ${skip_stages} " =~ [[:space:]]1[[:space:]] ]]; then
    ...
    local_data_opts="--src_lang ${src_lang} "${local_data_opts}
    local_data_opts="--src_case ${src_case} "${local_data_opts}
    local_data_opts="--tgt_lang ${tgt_lang} "${local_data_opts}
    local_data_opts="--tgt_case ${tgt_case} "${local_data_opts}
    local_data_opts="--nclusters ${nclusters} "${local_data_opts}
    # [Task dependent] Need to create data.sh for new corpus
    local/data.sh \
        --speed_perturb_factors "${speed_perturb_factors}" \
        --feats_type "${local_data_feats_type}" \
        ${local_data_opts}
fi
...
if [ ${stage} -le 13 ] && [ ${stop_stage} -ge 13 ] && ! [[ " ${skip_stages} " =~ [[:space:]]13[[:space:]] ]]; then
    ...
    _km_dir="exp/kmeans/"$(echo "${local_data_feats_type}" | tr "+" "_")"_${nclusters}clusters"
