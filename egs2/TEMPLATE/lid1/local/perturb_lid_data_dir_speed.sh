...
# 2025 @Qingzheng-Wang
# Copied from ./scripts/utils/perturb_enh_data_dir_speed.sh
...
<"${srcdir}"/utt2spk awk -v p="${utt_suffix}" '{printf("%s %s%s\n", $1, $1, p);}' > "${destdir}/utt_map"
...
if [[ ! -f ${srcdir}/utt2uniq ]]; then
    ...
    <"${srcdir}/utt2uniq" awk -v p="${utt_suffix}" '{printf("%s%s %s\n", $1, p, $2);}' > "${destdir}/utt2uniq"
fi
