...
Options:
    ...
    --enh_args   # Arguments for enhancement model training, e.g., "--max_epoch 10" (default="${enh_args}").
                 # Note that it will overwrite args in enhancement config.
    --ref_num    # Number of speakers in the input audio (default="${ref_num}")
...
if ! "${skip_data_prep}"; then
    ...
    if [ ${stage} -le 3 ] && [ ${stop_stage} -ge 3 ]; then
        ...
        for dset in "${train_set}" "${valid_set}" ${test_sets}; do
            ...
            _spk_list=" "
            for i in $(seq ${ref_num}); do
                _spk_list+="spk${i} enroll_spk${i} "
            done

            for spk in ${_spk_list} "wav" ; do
                if [ "${dset}" = "${train_set}" ] && [ "${spk}" = "enroll_spk${i}" ]; then
    ...
    if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
        ...
        # NOTE: Not applying to test_sets to keep original data
            ...
            for i in $(seq ${ref_num}); do
                _spk_list+="spk${i} enroll_spk${i} "
                _scp_list+="spk${i}.scp enroll_spk${i}.scp "
            done
...
if ! "${skip_train}"; then
    if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ]; then
        ...
        ${train_cmd} JOB=1:"${_nj}" "${_logdir}"/stats.JOB.log \
            ${python} -m espnet2.bin.enh_tse_train \
                --collect_stats true \
                ${_train_data_param} \
                ${_valid_data_param} \
    ...
    if [ ${stage} -le 7 ] && [ ${stop_stage} -ge 7 ]; then
        ...
        for dset in "${valid_set}" ${test_sets}; do
            ...
            ${_cmd} --gpu "${_ngpu}" JOB=1:"${_nj}" "${_logdir}"/enh_inference.JOB.log \
                ${python} -m espnet2.bin.enh_tse_inference \
                    --ngpu "${_ngpu}" \
                    --fs "${fs}" \
                    ${_data_param} \
