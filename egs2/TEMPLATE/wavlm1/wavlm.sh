...
# Apache 2.0
#
# WavLM pre-training driver. It is the hubert1 template (iterative offline
# k-means pseudo-labelling + masked prediction) with the WavLM encoder and
# WavLM's masked speech denoising, so refer to egs2/TEMPLATE/hubert1/hubert.sh
# for the shared pipeline.
#     WavLM paper: https://arxiv.org/abs/2110.13900
#     Original code: https://github.com/microsoft/unilm/tree/master/wavlm
...
features_km=            # Feature for k-means clustering (multiple values, e.g. "mfcc espnet_wavlm espnet_wavlm")
...
Options:
    ...
    --features_km      # feature for k-means clustering of training stages (e.g. "mfcc espnet_wavlm espnet_wavlm")
    --layers_km        # layers of feature for k-means clustering of training stages (e.g. "0 6 9")
    --wavlm_args      # Arguments for wavlm model training (default="${wavlm_args}").
                       # e.g., --wavlm_args "--max_epoch 10"
    ...
    --speech_fold_length  # fold_length for speech data during WavLM training (default="${speech_fold_length}").
    --text_fold_length    # fold_length for text data during WavLM training (default="${text_fold_length}").
...
if ! "${skip_train}"; then
    ...
    for ((iter=${train_start_iter}; iter<=${train_stop_iter};iter++)); do
        ...
        ssl_stats_dir="${expdir}/wavlm_iter${iter}_stats_${feats_type}"
        ssl_exp="${expdir}/wavlm_iter${iter}_${ssl_tag}"
        ...
        if [ -n "${download_model}" ]; then
            ...
            last_iter_ssl_model+="${expdir}/wavlm_iter$(( iter-1 ))_"
        ...
        if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ]; then
            ...
            if [ ${feats_km} != "mfcc" ]; then
                ...
                if ! "${gpu_dump_feature}"; then
                    log "Warning: It is recommented to use GPU in WavLM feature extraction for K-means clustering."
                fi
                _opts+="--use_gpu ${gpu_dump_feature} "

                # perform_kmeans.sh names these options after HuBERT, but
                # they carry the SSL feature type and model path for any
                # espnet-trained teacher, WavLM included.
        ...
        if [ ${stage} -le 6 ] && [ ${stop_stage} -ge 6 ]; then
            ...
            log "Stage 6 [Iter ${iter} / ${train_stop_iter}]: WavLM collect stats: input_feats=${feats_km} train_set=${_ssl_train_dir}, valid_set=${_ssl_valid_dir}"

            _opts=
            if [ -n "${ssl_config}" ]; then
                # To generate the config file: e.g.
                #   % python3 -m espnet2.bin.wavlm_train --print_config --optim adam
            ...
            # 3. Submit jobs
            log "WavLM collect-stats started... log: '${_logdir}/stats.*.log'"
            ...
            ${train_cmd} JOB=1:"${_nj}" "${_logdir}"/stats.JOB.log \
                ${python} -m espnet2.bin.wavlm_train \
                    --collect_stats true \
                    ...
                    ${_opts} ${wavlm_args} || { cat $(grep -l -i error "${_logdir}"/stats.*.log) ; exit 1; }
        ...
        if [ ${stage} -le 7 ] && [ ${stop_stage} -ge 7 ]; then
            ...
            log "Stage 7 [Iter ${iter} / ${train_stop_iter}]: WavLM Training: input_feats=${feats_km}, train_set=${_ssl_train_dir}, valid_set=${_ssl_valid_dir}"
            ...
            log "WavLM Training started... log: '${ssl_exp}/train.log'"
            ...
            log "Iter ${iter} WavLM Training completed, model saved in: ${ssl_exp}"
        fi
    done
else
...
ssl_exp="${expdir}/wavlm_iter${train_stop_iter}_${ssl_tag}"
