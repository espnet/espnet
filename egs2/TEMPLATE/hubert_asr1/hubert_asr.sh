...
# Finetuen related
finetune_config=           # Configration files of finetune stage    
...
pretrain_train_set=     # Name of pretrain training set
pretrain_valid_set=     # Name of pretraining valid set
finetune_train_set=     # Name of training set used for monitoring/tuning network training (required).
...
finetune_test_sets=     # Name of test sets used for evaluating network training (required).
                        # Multiple items (e.g., both dev and eval sets) can be specified (required).# [Task dependent] Set the datadir name created by local/data.sh
...
Options:
    ...
    --pretrain_ngpu        # The number of gpus in pretrain stage ("0" uses cpu, otherwise use gpu, default="${pretrain_ngpu}").
    --pretrain_num_nodes   # The number of nodes in pretrain stage (default="${pretrain_num_nodes}").
    --finetune_ngpu        # The number of gpus in finetune stage ("0" uses cpu, otherwise use gpu, default="${finetune_ngpu}").
    --finetune_num_nodes   # The number of nodes in finetune stage (default="${finetune_num_nodes}").
    ...
    # Finetuen related
    --finetuen_config  # configration files of finetune stage
...
if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 2 ]; then
	...
	ln -s "${asr_exp}" "${expdir}/pretrained_model_iter((iter-1))"
...
if [ ${stop_stage} -ge 6 ]; then
    asr_stage=$((stage>6 ? stage : 6))
    ./asr.sh \
	--stage ${asr_stage} --stop-stage "${stop_stage}" \
	--lang en \
	--ngpu "${finetune_ngpu}" \
