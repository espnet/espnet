...
# Extra files for SVS
utt_extra_files="label score.scp phoneme_pseudo_label_quality.txt"
train_sp_sets=
...
# ESPnet-codec
codec_choice=ESPnet
codec_checkpoint_path=/data1/wyn/Mus_data/16khz_soundstream/train.total_count.best.pth      # path to codec checkpoint file
codec_config_path=/data1/wyn/Mus_data/16khz_soundstream/config.yaml          # path to codec config file
...
if [ ${kmeans_feature} = "mfcc" ]; then  # MFCC has no layer
    ...
    elif [ ${kmeans_feature_type} = "codec" ]; then
        kmeans_feature_conf="{type=codec}"
        token_format="kaldi_ark"
...
if [ ${kmeans_feature_type} = "multi" ]; then
    ...
    if [ ${kmeans_feature} = "mfcc" ]; then
        ...
        token_file=token_${kmeans_feature_type}
        discrete_token_layers=32 # follow codec config
    else
...
if ! "${skip_data_prep}"; then
    ...
    if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
        if [ ${kmeans_feature_type} = "codec" ]; then
            for dset in "${train_set}" "${valid_set}" ${test_sets}; do
                # NOTE (Jinchuan) bias=2, reserve two slots for <blk> and <unk>
                cp ${data_feats}/${dset}/wav.scp ${data_feats}/${dset}/_wav.scp
                _dump_dir=${data_feats}/${dset}/data
                ...
                # done | sed 's/ \[ \| \]//g' | sort -u > "${_dump_dir}"/codec_token.txt || exit 1;
    ...
    if [ ${stage} -le 6 ] && [ ${stop_stage} -ge 6 ]; then
        ...
        ${train_cmd} JOB=1:"${_nj}" "${_logdir}"/stats.JOB.log \
            ${python} -m "espnet2.bin.${svs_task}_train" \
                ...
                --train_data_path_and_name_and_type "${_train_dir}/${token_file},discrete_token,${token_format}" \
                ...
                --valid_data_path_and_name_and_type "${_valid_dir}/${token_file},discrete_token,${token_format}" \
    ...
    if [ ${stage} -le 7 ] && [ ${stop_stage} -ge 7 ]; then
        ...
        if [ -z "${teacher_dumpdir}" ]; then
            ...
            if [ "${num_splits}" -gt 1 ]; then
                ...
                _opts+="--train_data_path_and_name_and_type ${_train_dir}/${token_file},discrete_token,${token_format} "
            ...
            _opts+="--valid_data_path_and_name_and_type ${_valid_dir}/${token_file},discrete_token,${token_format} "
        ...
        for dset in ${test_sets}; do
            ...
            ${_cmd} --gpu "${_ngpu}" JOB=1:"${_nj}" "${_logdir}"/svs_inference.JOB.log \
                ${python} -m espnet2.bin.svs_inference \
                    ...
                    --data_path_and_name_and_type "${_data}/${token_file},discrete_token,${token_format}" \
