...
# BEATs stage-7 helpers (setup_common_vars, train_encoder, train_tokenizer,
# tokenizer_inference, generate_checkpoint). Kept in a sibling file to keep this
# script readable; resolve the real path first so recipe symlinks still work.
beats_template_dir="$(cd "$(dirname "$(readlink -f "${BASH_SOURCE[0]}")")" && pwd)"
# shellcheck source=/dev/null
. "${beats_template_dir}/beats_utils.sh"
