...
score_lang_id=false        # Language identification scoring (for multi-lingual asr)
...
Options:
    ...
    --score_lang_id          # Language identification scoring (for multi-lingual asr)
...
if ! "${skip_eval}"; then
    ...
    if [ ${stage} -le 12 ] && [ ${stop_stage} -ge 12 ]; then
        ...
        [ -f local/score.sh ] && local/score.sh --score_lang_id ${score_lang_id} "${asr_exp}"
