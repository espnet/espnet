...
if ! "${skip_data_prep}"; then
            ...
            for dset in "${train_set}" "${valid_set}" ${test_sets}; do
                ...
                if [ ${#ref_text_files[@]} -gt 1 ]; then
                    ...
                    for ref_txt in ${ref_text_files[@]}; do
                        [ -f data/${dset}/${ref_txt} ] && cp data/${dset}/${ref_txt} ${data_feats}/${dset}
                    done
                fi
    ...
    if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
        log "Stage 4: Remove long/short data: ${data_feats}"
        ...
        for dset in "${train_set}" "${valid_set}"; do
            ...
            if [ "${_feats_type}" = raw ]; then
                ...
                <"${data_feats}/${dset}/utt2num_samples" \
                    awk -v min_length="${_min_length}" -v max_length="${_max_length}" \
                        '{ if ($2 < min_length || $2 > max_length ) print $1; }' \
                        >"${data_feats}/${dset}/filtered_key.txt"
            else
            ...
            # shellcheck disable=SC2068
            for ref_txt in ${ref_text_files[@]}; do
                <"${data_feats}/${dset}/${ref_txt}" \
                    awk ' { if( NF == 1 ) print $1; } ' >>"${data_feats}/${dset}/filtered_key.txt"
            done
...
if ! "${skip_train}"; then
    ...
    if [ ${stage} -le 10 ] && [ ${stop_stage} -ge 10 ]; then
        ...
        if [ -e "${_asr_train_dir}/filtered_key.txt" ]; then
            _opts+="--filtered_train_key_file ${_asr_train_dir}/filtered_key.txt "
        fi
        if [ -e "${_asr_valid_dir}/filtered_key.txt" ]; then
            _opts+="--filtered_valid_key_file ${_asr_valid_dir}/filtered_key.txt "
        fi
