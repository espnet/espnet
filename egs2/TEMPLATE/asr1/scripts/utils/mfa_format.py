    ...
    with open(wavs_dir / '../metadata.csv') as f:
        for line in f:
            ...
            with open(wavs_dir / f'{filename}.lab', 'w') as lab_f:
                ...
                # Convert single quotes into double quotes so that MFA doesn't confuse them with clitics
...
def get_phoneme_durations(tg: textgrid.Textgrid,
    ...
    # We preserve the start/end timings and not interval lengths due to rounding errors when converted to frames
    timings = [0.0]
    new_phones = []
    prev_end = 0.0
    for phone in phones:
