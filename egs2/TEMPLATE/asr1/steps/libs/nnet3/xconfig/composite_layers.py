...
class XconfigTdnnfLayer(XconfigLayerBase):
    ...
    def check_configs(self):
        ...
        if not self.config["context"] in ["default", "left-only", "shift-left", "none"]:
            raise RuntimeError(
                "context must be default, left-only shift-left or none, got {}".format(
                    self.config["context"]
                )
            )
    ...
    def _generate_config(self):
        ...
        configs.append(
            "component-node name={0}.linear component={0}.linear "
        ...
        configs.append(
            "component-node name={0}.affine component={0}.affine "
        ...
        configs.append(
            ...
            "self-repair-scale={2}".format(name, output_dim, self_repair_scale)
        )
        configs.append(
        ...
        # The BatchNorm layer
        configs.append(
            "component name={0}.batchnorm type=BatchNormComponent "
        ...
        if bypass_scale != 0.0:
            ...
            # in the network.
            configs.append(
                "component name={0}.noop type=NoOpComponent "
                "dim={1}".format(name, output_dim)
            )
            configs.append(
                "component-node name={0}.noop component={0}.noop "
                "input=Sum(Scale({1}, {2}), {0}.{3})".format(
                    name, bypass_scale, input_descriptor, cur_component_type
                )
            )
...
class XconfigPrefinalLayer(XconfigLayerBase):
    ...
    def _generate_config(self):
        ...
        configs.append(
            "component name={0}.batchnorm1 type=BatchNormComponent "
        ...
        configs.append(
            "component name={0}.batchnorm2 type=BatchNormComponent "
