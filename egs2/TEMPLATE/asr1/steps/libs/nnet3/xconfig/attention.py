...
class XconfigAttentionLayer(XconfigLayerBase):
    ...
    def check_configs(self):
        ...
        for conf in ["value-dim", "key-dim", "num-left-inputs", "num-right-inputs"]:
            if self.config[conf] < 0:
                raise RuntimeError(
                    "{0} has invalid value {1}".format(conf, self.config[conf])
                )
    ...
    def attention_input_dim(self):
        context_dim = (
            self.config["num-left-inputs"] + self.config["num-right-inputs"] + 1
        )
        ...
        for nonlinearity in nonlinearities:
            if nonlinearity == "relu":
                line = (
                    ...
                    " self-repair-scale={3}"
                    "".format(self.name, nonlinearity, dim, self_repair_scale)
                )
                ...
                line = (
                    ...
                    "dim={2} dropout-proportion={3}".format(
                        self.name, nonlinearity, dim, self.config["dropout-proportion"]
                    )
                )
