...
class XconfigRenormComponent(XconfigLayerBase):
    ...
    def _generate_config(self):
        ...
        configs = []
        line = (
            "component name={0} type=NormalizeComponent dim={1} target-rms={2}".format(
...
class XconfigBatchnormComponent(XconfigLayerBase):
    ...
    def _generate_config(self):
        ...
        configs = []
        line = (
            "component name={0} type=BatchNormComponent dim={1} target-rms={2}".format(
                self.name, input_dim, target_rms
            )
        )
        configs.append(line)
...
class XconfigDeltaLayer(XconfigLayerBase):
    ...
    def _generate_config(self):
        ...
        configs = []
        line = (
            "dim-range-node name={0}_copy1 input-node={0} dim={1} dim-offset=0".format(
                input_desc, input_dim
            )
        )
        configs.append(line)
        line = (
            "dim-range-node name={0}_copy2 input-node={0} dim={1} dim-offset=0".format(
                input_desc, input_dim
            )
        )
        configs.append(line)
