...
if [ -f $srcdir/score.scp ]; then
  utils/apply_map.pl -f 1 $destdir/utt_map <$srcdir/score.scp >$destdir/score.scp
fi
...
for f in feats.scp cmvn.scp vad.scp utt2lang utt2uniq utt2dur utt2num_frames text wav.scp score.scp reco2file_and_channel frame_shift stm glm ctm; do
