...
if ! "${skip_data_prep}"; then
    if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
        ...
        src_datadir=~/scratch/kaldi_data/seame
        echo "stage 1: Data preparation"
        for part in train dev_man dev_sge; do
           utils/copy_data_dir.sh ${src_datadir}/${part} data/${part}|| exit 1;
           utils/validate_data_dir.sh --no-feats data/${part} || exit 1;
        done

        utils/subset_data_dir_tr_cv.sh  --cv-spk-percent 10 data/train data/train_trn data/train_dev || exit 1;
