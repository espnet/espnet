...
ref_num=1   # Number of references for training.
...
Options:
    ...
    --inf_num    # Number of inference audio generated by the model (default="${inf_num}")
                 # Note that if it is not specified, it will be the same as ref_num. Otherwise, it will be overwritten.
    ...
    if [ ${stage} -le 13 ] && [ ${stop_stage} -ge 13 ]; then
        ...
        for dset in ${test_sets}; do
            ...
            for _type in cer wer ter; do
                ...
                if [ ${ref_num} -gt 1 ] && [ -n "${suffix}" ]; then
                    for i in $(seq ${ref_num}); do
                        for j in $(seq ${inf_num}); do
                            sclite \
                                ${score_opts} \
                    ...
                    scripts/utils/eval_perm_free_error.py --num-spkrs ${ref_num} \
