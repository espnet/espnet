...
def align_keys(file1, file2, output):
    """Align keys from two sorted files and write aligned results into two separate output files."""
    # Read the files into dictionaries (keys must be unique)
    data1 = {line.split(None, 1)[0]: line.strip() for line in open(file1, 'r') if line is not None}
    data2 = {line.split(None, 1)[0]: line.strip() for line in open(file2, 'r') if line is not None}
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Align keys between two sorted files into separate outputs.")
