...
num_splits_vad=1           # Number of splitting for lm corpus.
...
Options:
    ...
    --num_splits_vad   # Number of splitting for lm corpus  (default="${num_splits_vad}").
...
if ! "${skip_train}"; then
    ...
    if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ]; then
        ...
        if [ "${num_splits_vad}" -gt 1 ]; then
            # If you met a memory error when parsing text files, this option may help you.
            # The corpus is split into subsets and each subset is used for training one by one in order,
            # so the memory footprint can be limited to the memory required for each dataset.

            _split_dir="${vad_stats_dir}/splits${num_splits_vad}"
            if [ ! -f "${_split_dir}/.done" ]; then
                ...
                ${python} -m espnet2.bin.split_scps \
                  --scps \
                      "${_vad_train_dir}/${_scp}" \
                      "${_vad_train_dir}/text" \
                      "${vad_stats_dir}/train/speech_shape" \
                      "${vad_stats_dir}/train/text_shape" \
