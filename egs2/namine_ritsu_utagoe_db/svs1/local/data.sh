...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
    ...
    for x in ${train_set} ${train_dev} ${recog_set}; do
        src_data=data/${x}
        local/prep_segments.py --input_type hts --input_type hts --silence pau --silence sil --silence br ${src_data}
        mv ${src_data}/segments.tmp ${src_data}/segments
        mv ${src_data}/label.tmp ${src_data}/label
        local/prep_segments.py --input_type xml --input_type xml --silence P --silence B ${src_data}
