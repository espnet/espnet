...
with open(os.path.join(DATA_DIR, "train", "text"), "w") as f_text:
    with open(os.path.join(DATA_DIR, "train", "wav.scp"), "w") as f_wav:
        with open(
            os.path.join(TRAIN_TRANSCRIPT_DIR, "ca16_conv", "transcripts.txt"), "r"
        ) as f_trans:
            for line in f_trans:
        ...
        with open(
            os.path.join(TRAIN_TRANSCRIPT_DIR, "ca16_read", "conditioned.txt")
        ) as f_trans:
            for line in f_trans:
                utt_id, text = line.split(" ", 1)
                text = "".join([i.lower() for i in text if i not in string.punctuation])
...
TEST_TRANSCRIPT_PATH = os.path.join(
    DATASET_DIR, "transcripts", "test", "ca16", "prompts.txt"
)
...
VALID_TRANSCRIPT_PATH = os.path.join(
    DATASET_DIR, "transcripts", "devtest", "ca16_read", "conditioned.txt"
)
