...
die "Missing --filelist argument" if $filelist eq "";
# If filelist is provided, read into allowed_files hash
%allowed_files = ();
if ($filelist) {
    open(FH, $filelist) or die "Cannot open file list $filelist\n";
    while (<FH>) {
        chomp;
        $allowed_files{$_} = 1;
    }
...
die "Missing --iso3 argument" if $iso3 eq "";
if ($#ARGV == 1) {
    $inDir  = $ARGV[0];
    $outDir = $ARGV[1];
...
if ($vocabFile) {
    print STDERR "DEBUG: vocabFile=$vocabFile\n";
...
if (-d $TranscriptionDir) {
    ...
    if ($#TranscriptionFiles >= 0) {
        ...
        while ($filename = shift @TranscriptionFiles) {
            ...
            # new added
            # Skip if file is not in allowed_files hash
            next if (%allowed_files && !exists $allowed_files{$fileID});
            ...
            while ($line=<TRANSCRIPT>) {
                ...
                if ($line =~ m:^\[([0-9]+\.*[0-9]*)\]$:) {
                    ...
                    # new added
                    $utteranceID = "${iso3}_$utteranceID";
                    ##################################################
    ...
    if ($#AudioFiles >= 0) {
        ...
        while ($filename = shift @AudioFiles) {
            ...
            if (exists $numUtterancesInFile{$fileID}) {
                ...
                $waveformName{$fileID} = $filename;
                chomp $waveformName{$fileID};
                $duration{$fileID} = -1;  # duration unknown or ignored
                $numFiles++;
            } else {
                print STDOUT ("$0: No transcriptions for audio file ${fileID}.wav\n");
            }
        }
        print STDERR ("$0: Processed $numFiles .wav files in $AudioDir (no duration extraction)\n");
    } else {
