...
def collect_data(examples, lang_id_dict):
    ...
    for idx, eg in enumerate(examples):
        ...
        if lang_id is None:
            logging.warning(
                f"skip the example due to missing lang_id: {eg['lang_id']}"
            )
            continue
...
def main():
    ...
    for split, utts in utt_splits.items():
        ...
        utt2spk_fp = open(os.path.join(write_dir, "utt2spk"), "w")
