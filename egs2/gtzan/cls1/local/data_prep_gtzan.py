...
def split_fnames_with_balanced_genre(
    fnames_for_each_genre, ratio={"train": 0.75, "val": 0.15, "eval": 0.10}
):
    ...
    for genre in fnames_for_each_genre.keys():
        ...
        val_input += [
            fnames_for_each_genre[genre][i]
            for i in random_index[n_train : n_train + n_val]
        ]
...
train_input, train_label, val_input, val_label, eval_input, eval_label = (
    split_fnames_with_balanced_genre(fnames_for_each_genre, ratio)
)

# get shuffled indices of data
...
for sets, inputs, labels, name in [
    (train_set, train_input, train_label, "train"),
    (val_set, val_input, val_label, "val"),
    (eval_set, eval_input, eval_label, "eval"),
]:
    ...
    with open(text_write_path, "w") as text_f, open(
        ...
        for i in sets:
            ...
            # wav.scp:
            print(
                f"uttid{'{:0=4}'.format(i)} {os.path.join(DATA_READ_ROOT, labels[i], inputs[i])}",
                file=wav_f,
            )
            # utt2spk:
