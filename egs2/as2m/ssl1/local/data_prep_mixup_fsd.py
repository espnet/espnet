"""Prepares data for self-supervised audio only training with Mixup FSD."""
...
def split_audio_into_chunks(audio_path: str, chunk_size: float):
    ...
    """
    filename = ".".join(audio_path.split("/")[-3:]).replace("{", "").replace("}", "")
    ...
    if total_samples % chunk_samples != 0:
        ...
        if num_chunks == 0:
            chunks[audio_path] = None  # Do not write again
            return chunks, sample_rate
...
def process_file(audio_path, data_write_dir, wav_scp_f, wavscplock):
    ...
    file_id = ".".join(audio_path.split("/")[-3:]).replace("{", "").replace("}", "")
    uttid = f"mixup_fsd.{file_id}"
    ...
    try:
        ...
        for chunk_name, audio in audio_chunks.items():
            if audio is None and chunk_name is not None:
                audio_paths.append(chunk_name)
                continue
            chunk_path = os.path.join(
                data_write_dir, "mixup_fsd", "chunked", chunk_name
            )
...
def process_dataset(data_read_dir, data_write_dir, wavscplock):
    print(f"Processing Mixup FSD...", flush=True)
    pattern = f"{data_read_dir}/*/*/example*.wav"
    wav_files = glob.glob(pattern, recursive=True)
    print(len(wav_files), "files found", flush=True)
    wav_scp_write_path = os.path.join(data_write_dir, "mixup_fsd", "wav.scp")
