...
#       SSL_TAG="base_tok2.tune_lr${LEARNING_RATE}_warmup${WARMUP_STEPS}_bins${BATCH_BINS}_totalsteps${TOTAL_STEPS}"
...
#       external_teacher_model=/work/nvme/bbjs/sbharadwaj/model_checkpoints/ear_base/beats_iter1_base.tune_lr5e-4_warmup40000_bins1600000_totalsteps400000/epoch59.pt

#       tokenizer_inf_config=conf/tokenizer_inf_base_100k_steps.yaml

#       ./run_ear.sh --ngpu 4 --ssl_tag "${SSL_TAG}" --train_start_iter 2 --train_stop_iter 2 --external_teacher_model "${external_teacher_model}" \
...
for LEARNING_RATE in 5.0e-4; do
  for WARMUP_STEPS in 20000; do
    for BATCH_BINS in 300000; do
      ...
      SSL_TAG="large_tok2.tune_lr${LEARNING_RATE}_warmup${WARMUP_STEPS}_bins${BATCH_BINS}_totalsteps${TOTAL_STEPS}"
      N_EPOCH=$(awk "BEGIN {print int($TOTAL_STEPS * ($BATCH_BINS / 998) / 7220000)}")
      ...
      external_teacher_model=/work/nvme/bbjs/sbharadwaj/7Msounds/exp/beats_iter1_large1.tune_lr1.0e-4_warmup40000_bins1600000_totalsteps400000/epoch_latest.pt
