...
def process_and_dump(ex, split, dump_root, fs_out):
    ...
    try:
        ...
        return (utt, spk, str(out_wav), text, dur)
    except Exception as e:
        print(
            f"⚠️  Failed to process {ex.get('audio', {}).get('path','?')}: {e}",
            file=sys.stderr,
        )
        return None
...
def main():
    ...
    p.add_argument(
        "--hf_repo", required=True, help="HuggingFace dataset, e.g. MushanW/GLOBE_V2"
    )
    ...
    p.add_argument(
        "--fs_out", type=int, default=44100, help="sox will resample to this rate"
    )
    p.add_argument("--jobs", type=int, default=8, help="number of parallel threads")
    args = p.parse_args()
    ...
    for split in (args.train_set, args.dev_set, args.test_set):
        ...
        with ThreadPoolExecutor(args.jobs) as exe:
            ...
            futures = {
                exe.submit(
                    process_and_dump, next(in_iter), split, dump_root, args.fs_out
                )
