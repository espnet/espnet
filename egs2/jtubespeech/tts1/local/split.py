...
if __name__ == '__main__':
    ...
    parser.add_argument(
        "--db_raw",
        type=str,
        default="downloads/jtuberaw",
        required=False
    )
    parser.add_argument(
        "--db_split",
        type=str,
        default="downloads/jtubesplit",
        required=False
    )
    ...
    for path in text_paths:
        ...
        wav, sr = sf.read(os.path.join(rawdata_dir, 'wav16k', dirname, '{}.wav'.format(stem)))
        with open(path, 'r') as fr:
            for i, line in enumerate(fr):
                line_list = line.strip().split("\t", 2)
                idx = '0'*(4-len(str(i))) +  str(i)
                t_s, t_e, transcript = float(line_list[0]), float(line_list[1]), line_list[2].strip('"')
