...
def parse_trans_xml(trans_file, audio_file, out_audio_dir):
    ...
    extract_first_channel_16khz(
        audio_file,
        f"{out_audio_dir}/{wav_file}_spk1.wav",
        f"{out_audio_dir}/{wav_file}_spk2.wav",
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--trans_prefix_Zacatlan", type=str, help="Transcription prefix for Zacatlan."
    )
    parser.add_argument(
        "--trans_prefix_Tequila", type=str, help="Transcription prefix for Tequila."
    )
    parser.add_argument(
        "--trans_prefix_Hidalgo", type=str, help="Transcription prefix for Hidalgo."
    )

    parser.add_argument(
        "--audio_prefix_Zacatlan", type=str, help="Audio prefix for Zacatlan."
    )
    parser.add_argument(
        "--audio_prefix_Tequila", type=str, help="Audio prefix for Tequila."
    )
    parser.add_argument(
        "--audio_prefix_Hidalgo", type=str, help="Audio prefix for Hidalgo."
    )
    ...
    (
        ...
        {"train": [], "dev": [], "test": {k: [] for k in trans_prefix}}
        for _ in range(5)
    )
    ...
    for lan in trans_prefix:
        ...
        trans_partition = {
            ...
            "dev": all_trans[
                int(train_ratio * len(all_trans)) : int(dev_ratio * len(all_trans))
            ],
        ...
        for curr_set in trans_partition:
            for trans in trans_partition[curr_set]:
                ...
                choices = glob.glob(
                    os.path.join(audio_prefix[lan], date_folder + "*/*.wav")
                )
                ...
                (
                    ...
                    spk2utt_lines,
                    segments_lines,
                ) = parse_trans_xml(trans, wav, audio_data_path)
