...
if (@ARGV != 4) {
  print STDERR "Usage: $0 <path-to-commonvoice-corpus> <dataset> <valid-train|valid-dev|valid-test> <src_lang>\n";
...
open(TEXT,">", "$out_dir/text.$lang") or die "Could not open the output file $out_dir/text.$lang";
open(WAV,">", "$out_dir/wav.scp.$lang") or die "Could not open the output file $out_dir/wav.scp.$lang";
my $header = <CSV>;
while(<CSV>) {
  chomp;
