...
# From Brian: the following recordings in TEDLIUM training set
# are overlapped with MuST-C tst-COMMON
overlapped_ids = [
    "TonyFadell_2015",
    "MikkoHypponen_2011X",
    "BryanStevenson_2012",
...
def collect_data(
    ...
    for stm in stm_dir.iterdir():
        if stm.stem in overlapped_ids:
            print(f"Exclude {stm.stem} from {split} set.")
            continue

        sph = sph_dir / (stm.stem + ".sph")
