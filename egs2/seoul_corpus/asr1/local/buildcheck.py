...
def main():
    ...
    argv = [
        "--config", cfg,
        "--token_list", "data/ko_token_list/bpe_unigram2000/tokens.txt",
        "--token_type", "bpe",
        "--bpemodel", "data/ko_token_list/bpe_unigram2000/bpe.model",
        "--use_preprocessor", "true",
        "--non_linguistic_symbols", "data/nlsyms.txt",
        "--cleaner", "none",
        "--g2p", "none",
        "--output_dir", "exp_tags2/.buildcheck",
    ...
    print(f"  params: {total / 1e6:.1f} M total, {train / 1e6:.1f} M trainable, "
    ...
    with torch.no_grad():
        ...
        print(f"  encoder out: {tuple(enc.shape)} lengths={enc_lens.tolist()} "
              f"({enc.shape[1] / (80000 / 16000) :.1f} frames/s)")
        loss, stats, _ = model(speech, speech_lengths, text, text_lengths)
    print(f"  forward OK: loss={float(loss):.2f} "
