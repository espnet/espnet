...
if ! "${skip_scoring}"; then
    if [ ${stage} -le 9 ] && [ ${stop_stage} -ge 9 ]; then
        log "Stage 9: Scoring: TTS scoring via VERSA"
        ...
        for dset in ${test_sets}; do
            _gen_dir=${tts_exp}/${inference_tag}/${dset}
            log "Scoring TTS evaluation via VERSA on ${_gen_dir}"
            _data=${data_feats}/${dset}
            _opts=
            ...
            if [ ! -s "${_pred_file}" ]; then
                log "Error: Missing generated wav.scp: ${_pred_file}. Run stage 8 first."
                exit 1
            fi
            if [ ! -s "${_gt_file}" ]; then
                log "Error: Missing reference wav.scp: ${_gt_file}."
                exit 1
            fi
            ...
            _count=0
            for f in "${_eval_dir}"/result.*.txt; do
                if [ -f "$f" ]; then
                    c=$(wc -l < "$f")
                    _count=$(( _count + c ))
                fi
            ...
            log "Finished scoring evaluation for ${dset}; results are in ${_eval_dir}"
        done
    fi
fi
