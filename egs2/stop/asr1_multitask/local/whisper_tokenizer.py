...
class Tokenizer:
    """A thin wrapper around `GPT2TokenizerFast` providing quick access to
    special tokens"""
    ...
    def decode_with_timestamps(self, tokens) -> str:
        """
        Timestamp tokens are above the special tokens' id range and are
        ignored by `decode()`.
        This method decodes given tokens with timestamps tokens annotated, e.
        g. "<|1.08|>".
        """
        outputs = [[]]
        for token in tokens:
    ...
    def all_language_codes(self) -> Tuple[str]:
        return tuple(
            self.decode([tok]).strip("<|>") for tok in self.all_language_tokens
        )
    ...
    def non_speech_tokens(self) -> Tuple[int]:
        ...
        tags or non-speech
        annotations, to prevent sampling texts that are not actually spoken in
        the audio, e.g.
        ...
        # symbols that may be a single token or multiple tokens depending on
        # the tokenizer.
        ...
        # okay to suppress
        # in generations, and in the 3-byte UTF-8 representation they share
        # the first two bytes.
        miscellaneous = set("♩♪♫♬♭♮♯")
