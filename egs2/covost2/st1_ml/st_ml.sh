...
lang_pairs="en2de"         # source2target language pairs abbrev. id (e.g., en2de_de2en)
run_multilingual=true      # run with multiple langauge pairs
...
Options:
    ...
    --lang_pairs=        # source language abbrev. id (e.g., en2de_de2en). (default="${lang_pairs}")
...
if "${run_multilingual}"; then
    utt_extra_files="text.${src_case}.src text.${tgt_case}.tgt src_file.txt tgt_file.txt"
else
    utt_extra_files="text.${src_case}.src text.${tgt_case}.tgt"
fi
# Use the same text as ST for bpe training if not specified.
[ -z "${src_bpe_train_text}" ] && src_bpe_train_text="${data_feats}/${train_set}/text.${src_case}.src"
[ -z "${tgt_bpe_train_text}" ] && tgt_bpe_train_text="${data_feats}/${train_set}/text.${tgt_case}.tgt"
# Use the same text as ST for lm training if not specified.
[ -z "${lm_train_text}" ] && lm_train_text="${data_feats}/${train_set}/text.${tgt_case}.tgt"
# Use the same text as ST for lm training if not specified.
[ -z "${lm_dev_text}" ] && lm_dev_text="${data_feats}/${valid_set}/text.${tgt_case}.tgt"
# Use the text of the 1st evaldir if lm_test is not specified
[ -z "${lm_test_text}" ] && lm_test_text="${data_feats}/${test_sets%% *}/text.${tgt_case}.tgt"

# Check tokenization type
if [ "${lang_pairs}" != noinfo ]; then
    token_listdir=data/${lang_pairs}_token_list
else
...
if ! "${skip_data_prep}"; then
    ...
    if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
        ...
        for dset in "${train_set}" "${valid_set}"; do
            ...
            # Remove empty text
            <"${data_feats}/org/${dset}/text.${tgt_case}.tgt" \
                awk ' { if( NF != 1 ) print $0; } ' > "${data_feats}/${dset}/text.${tgt_case}.tgt"
    ...
    if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ]; then
        ...
        if "${token_joint}"; then
            ...
            cat $tgt_bpe_train_text > ${data_feats}/${train_set}/text.${lang_pairs}
            [ ! -z "${src_bpe_train_text}" ] && cat ${src_bpe_train_text} >> ${data_feats}/${train_set}/text.${lang_pairs}
            # Set the new text as the target text
            tgt_bpe_train_text="${data_feats}/${train_set}/text.${lang_pairs}"
        fi
        ...
        if [ "${num_splits_st}" -gt 1 ]; then
            ...
            if [ ! -f "${_split_dir}/.done" ]; then
                ...
                ${python} -m espnet2.bin.split_scps \
                  --scps \
                      "${_st_train_dir}/${_scp}" \
                      "${_st_train_dir}/text.${tgt_case}.tgt" \
                      "${_st_train_dir}/text.${src_case}.src" \
            ...
            _opts+="--train_data_path_and_name_and_type ${_split_dir}/text.${tgt_case}.tgt,text,text "
            _opts+="--train_data_path_and_name_and_type ${_split_dir}/text.${src_case}.src,src_text,text "
            ...
            _opts+="--train_data_path_and_name_and_type ${_st_train_dir}/text.${tgt_case}.tgt,text,text "
            _opts+="--train_data_path_and_name_and_type ${_st_train_dir}/text.${src_case}.src,src_text,text "
