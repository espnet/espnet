...
def normalize_text(text):
    ...
    for word in text.split():
        ...
        if len(word) >= 2 and word.isalpha() and word.upper() == word and word not in {"GMAT", "AIDS", "ARC"}:
            # print(word)
    ...
    spacing_list_pre = ['north east', 'north west', 'south east', 'south west', 'alright']
    spacing_list_post = ['northeast', 'northwest', 'southeast', 'southwest', 'all right']
...
if __name__ == '__main__':
    if len(sys.argv) != 3:
        print("Help: python local/text_normalization.py <path_to_transcript> <desired_output_path>")
        print("ex: python local/snippet_generation.py downloads/transcript.tsv.bak downloads/transcript.tsv")
        exit(1)
