...
stop_stage=100
SECONDS=0
lang=gn # en de fr cy tt kab ca zh-TW it fa eu es ru tr nl eo zh-CN rw pt zh-HK cs pl uk 
...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
    ...
    for part in "train"  "validated" "test" "dev"; do
        # use underscore-separated names in data directories.
        local/data_prep.pl "cv-corpus-15.0-2023-09-08/${lang}" ${part} data/"$(echo "${part}_${lang}" | tr - _)"
    done
