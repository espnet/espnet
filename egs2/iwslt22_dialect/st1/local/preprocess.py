...
def stm_to_kaldi(st_stm, asr_stm, dst):
    ...
    with open(st_stm, "r", encoding="utf-8") as st_stm, open(asr_stm, "r", encoding="utf-8") as asr_stm:
    ...
    with open(dst + "/wav.scp", "w", encoding="utf-8") as wav_scp, \
        ...
        for i in range(len(data["F"])):
            ...
            uttid = data["S"][i] + "_" + recid + "_" + time_to_hash(data["BT"][i]) + "-" + time_to_hash(data["ET"][i])
            sox_cmd = "sox -R -t wav - -t wav - rate 16000 dither |"
            wav_scp.write(" ".join([recid, "sph2pipe -f wav -p -c", data["C"][i], data["F"][i], "|", sox_cmd]) + "\n")
            utt2spk.write(" ".join([uttid, data["S"][i]]) +"\n")
            segments.write(" ".join([uttid, recid, data["BT"][i], data["ET"][i]]) +"\n")
...
if __name__ == "__main__":
    stm_to_kaldi(args.data + "/stm/st-aeb2eng.norm.train.stm", args.data + "/stm/asr-aeb.norm.train.stm", args.out + "/train")
    stm_to_kaldi(args.data + "/stm/st-aeb2eng.norm.dev.stm", args.data + "/stm/asr-aeb.norm.dev.stm", args.out + "/dev")
    stm_to_kaldi(args.data + "/stm/st-aeb2eng.norm.test1.stm", args.data + "/stm/asr-aeb.norm.test1.stm", args.out + "/test1")
