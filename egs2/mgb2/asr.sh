...
if ! "${skip_train}"; then
    ...
    if [ ${stage} -le 11 ] && [ ${stop_stage} -ge 11 ]; then
        log "copying the files to tmp ..." 
        tempdir=$(mktemp -d "/tmp/lifelongLearning-$$.XXXXXXXX")
        trap 'rm -rf ${tempdir}' EXIT
        cp -r "${data_feats}" ${tempdir}
        rsync -zav --progress --bwlimit=1000000 "${data_feats}" ${tempdir}
...
# I comment this        [ -f local/score.sh ] && local/score.sh ${local_score_opts} "${asr_exp}"
