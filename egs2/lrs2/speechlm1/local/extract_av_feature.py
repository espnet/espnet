...
def per_file(f, args, video_process, model, writer):
    f_name, video_path = f  # f_name: video_id
    ...
    if os.path.exists(os.path.join(args.landmark_dir, f_name + ".pkl")):
        with open(os.path.join(args.landmark_dir, f_name + ".pkl"), "rb") as pkl_file:
        ...
        # feats = model.forward_fusion(xs)  # 1, 2048 35
        xs = {"video": video_feats.cuda()}
        feats = model.forward_video(xs)
        # print(f"feats1: {feats.shape}")
        writer[f_name] = feats.squeeze(0).transpose(0, 1).contiguous().cpu().numpy()
        # 35, 2048
        os.system(f"rm {temp_aud_path}.wav")
    else:
        print(os.path.join(args.landmark_dir, f_name + ".pkl"))
    return
