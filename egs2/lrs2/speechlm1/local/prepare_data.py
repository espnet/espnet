...
def parse_arguments():
    ...
    parser.add_argument(
        "--wav_source_dir",
        type=str,
        default="/nfs-02/yuyue/visualtts/dataset/lrs2/wav_16k",
    )
    parser.add_argument(
        "--video_source_dir",
        type=str,
        default="/nfs-02/yuyue/visualtts/dataset/lrs2/av_feature",
    )
    parser.add_argument(
        "--data_split_dir",
        type=str,
        default="/nfs-02/yuyue/visualtts/dataset/lrs2/data_split",
    )
...
if __name__ == "__main__":
    ...
    for data_split in ["val", "test"]:
        ...
        for utt in tqdm.tqdm(utt_list):
            ...
            count = len(utt_list)
            if os.path.exists(
                os.path.join(args.wav_source_dir, utt + ".wav")
            ) and os.path.exists(os.path.join(args.wav_source_dir, utt + ".lab")):
                try:
                    with open(
                        os.path.join(args.wav_source_dir, utt + ".lab"), "r"
                    ) as file:
                        text = file.readline().strip()
                    spk = utt.split("_")[0]
                    wav_path = os.path.join(args.wav_source_dir, utt + ".wav")
                    video_feature_path = os.path.join(
                        args.video_source_dir, utt + ".mp4"
                    )
