...
class DatasetUtils:
    ...
    def generate_espnet_vision_data(
        ...
        dataset (str): The dataset we are working with (train, test, dev)
        Returns:
        utt2spk (list) : Utterence to MP4-Path data
        """
        mp4 = []
        mp4_full_path = ""
        ...
        for speaker_folder in speaker_folders:
            ...
            for mp4_file in os.listdir(speaker_folder):

                if not mp4_file.endswith(".mp4"):
                    continue
                else:
                    fname = os.path.splitext(mp4_file)[0]
                    if mouth_roi and fname.endswith("mouth_roi"):
                        mp4_full_path = os.path.join(speaker_folder, mp4_file)
                        utt_id = spk_id + "_" + Path(mp4_full_path).stem[:-10]
                    elif (not mouth_roi) and (not fname.endswith("mouth_roi")):
                        mp4_full_path = os.path.join(speaker_folder, mp4_file)
                        utt_id = spk_id + "_" + Path(mp4_full_path).stem
                    else:
                        continue
                    assert os.path.exists(mp4_full_path)
                    mp4.append(utt_id + " " + mp4_full_path)
        return mp4

    @staticmethod
    def perform_audio_data_prep(speaker_folders: list, dataset: str) -> None:
...
def main():
    ...
    if args.audio_input:
        ...
        DatasetUtils.perform_audio_data_prep(test_folders, "test")

        logging.info(f"Performing Data Preparation for TRAIN")
        DatasetUtils.perform_audio_data_prep(train_folders, "train")
