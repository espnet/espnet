...
def main():
    ...
    pretrain_file_lists = sorted(glob.glob(os.path.join(
        args.data_dir, 'pretrain', '**' ,'*.mp4'), recursive=True))
    pretrain_file_lists = [f'{os.sep}'.join(os.path.normpath(
        f).split(os.sep)[-3:])[:-4] for f in pretrain_file_lists]
    train_file_lists = sorted(glob.glob(os.path.join(
        args.data_dir, 'trainval', '**' ,'*.mp4'), recursive=True))
    train_file_lists = [f'{os.sep}'.join(os.path.normpath(
        f).split(os.sep)[-3:])[:-4] for f in train_file_lists]
    test_file_lists = sorted(glob.glob(os.path.join(
        args.data_dir, 'test', '**' ,'*.mp4'), recursive=True))
    test_file_lists = [f'{os.sep}'.join(os.path.normpath(
        f).split(os.sep)[-3:])[:-4] for f in test_file_lists]
