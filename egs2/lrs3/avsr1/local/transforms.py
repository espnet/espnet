...
__all__ = ["Compose", "Normalize", "CenterCrop", "AddNoise",
           "NormalizeUtterance", "Identity", "SpeedRate", "ExpandDims",
           "HorizontalFlip", "TimeMask", "CutoutHole", "RandomCrop"]
...
class AddNoise(object):
    ...
    def __init__(self, noise, snr_target=None, snr_levels=[-5, 0, 5, 10, 15]):
        assert noise.dtype in [
            np.float32, np.float64], "noise only supports float data type"
        self.noise = noise
    ...
    def __call__(self, signal):
        assert signal.dtype in [
            np.float32, np.float64], "signal only supports float32 data type"
        snr_target = random.choice(
        ...
        if snr_target == 9999:
            ...
            factor = (sig_power / noise_clip_power) / (10
                                                       ** (snr_target / 10.0))
            desired_signal = (
                signal + noise_clip * np.sqrt(factor)).astype(np.float32)
...
class SpeedRate(object):
    ...
    def __call__(self, x):
        ...
        new_indices = np.linspace(start=0, stop=old_length,
                                  num=new_length, endpoint=False)
