...
def stacker(feats, stack_order):
    ...
    feats = feats.reshape((-1, stack_order, feat_dim)
...
def per_file(f, args, video_process, model, writer):
    ...
    if os.path.exists(os.path.join(args.landmark_path, f_name + '.pkl')):
        ...
        audio_feats = logfbank(
            wav_data, samplerate=sample_rate).astype(
            np.float32)  # [T, F]
        ...
        # [B, T, H, W, C] -> [B, C, T, H, W]
        video_feats = torch.FloatTensor(
            video_feats).unsqueeze(-1).unsqueeze(0)  # B, T, H, W, C
...
def main():
    ...
    if args.model == 'base':
        model = FairseqAVHubertEncoder(
            ...
            avhubert_dir_path=f'./local/pre-trained', encoder_embed_dim=768,
            encoder_layers=12, encoder_ffn_embed_dim=3072,
            encoder_attention_heads=12,)
    else:
        model = FairseqAVHubertEncoder(
            avhubert_url=large_url,
            avhubert_dir_path=f'./local/pre-trained', encoder_embed_dim=1024,
            encoder_layers=24, encoder_ffn_embed_dim=4096,
