...
class VideoProcess:
    def __init__(
            ...
            window_margin=12,
            convert_gray=True):
        self.reference = np.load(os.path.join(
            os.path.dirname(__file__), mean_face_path))
    ...
    def crop_patch(self, video, landmarks):
        ...
        for frame_idx, frame in enumerate(video):
            window_margin = min(self.window_margin // 2, frame_idx,
                                len(landmarks) - 1 - frame_idx)
            ...
            smoothed_landmarks += landmarks[frame_idx].mean(
                axis=0) - smoothed_landmarks.mean(axis=0)
    ...
    def interpolate_landmarks(self, landmarks):
        ...
        for idx in range(1, len(valid_frames_idx)):
            if valid_frames_idx[idx] - valid_frames_idx[idx - 1] > 1:
                landmarks = linear_interpolate(
                    landmarks, valid_frames_idx[idx - 1],
                    valid_frames_idx[idx])
        ...
        if valid_frames_idx:
            landmarks[:valid_frames_idx[0]] = [
                landmarks[valid_frames_idx[0]]] * valid_frames_idx[0]
    ...
    def estimate_affine_transform(
            self, landmarks, stable_points, stable_reference):
    ...
    def apply_affine_transform(
        ...
        transformed_landmarks = np.matmul(
            landmarks, transform[:, :2].transpose()) + transform[:, 2].transpose()
        return transformed_frame, transformed_landmarks
    ...
    def get_video_transform(self, split='test'):
        ...
        return Compose(
            [Normalize(0.0, 255.0),
             ...
             if split == 'test' else RandomCrop(crop_size), Identity()
             if split == 'test' else HorizontalFlip(0.5), Normalize(
                 mean, std),
             Identity()
             if split == 'test' else TimeMask(max_mask_length=15), Identity()
             if split == 'test' else
             CutoutHole(min_hole_length=22, max_hole_length=44)])
