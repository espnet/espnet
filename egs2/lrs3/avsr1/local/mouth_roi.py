...
def get_parser():
    ...
    parser.add_argument("--mean_face_path", type=str, help="Path to mean face file")
...
def crop_video(input_video_path, output_video_path):
    ...
    if not success:
        print(
            ">>> {} Cannot read mp4 file: vision features unavailable".format(
                input_video_path
            )
        )
...
def main():
        ...
        for f in files:
            ...
            if f.endswith(".mp4"):
                if fname[0].endswith("mouth_roi"):
                    ...
                    os.path.join(root, fname[0] + "_mouth_roi" + fname[1])
                ):
                    continue
                else:
    ...
    landmark_status = "Failed to detect landmarks for {} / {} Train videos".format(
        count, file_count
    )
