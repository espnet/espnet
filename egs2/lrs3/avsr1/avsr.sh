...
if ! "${skip_data_prep}"; then
    if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
        ...
        local/data.sh --audio_input ${audio_input} --vision_input ${vision_input} --mouth_roi ${mouth_roi} ${local_data_opts}
    fi

    if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
    # TODO: Integrate Speed Perturb Factor for Multimodal Sources
        if [ -n "${speed_perturb_factors}" ]; then
           log "Stage 2: Speed perturbation: data/${train_set} -> data/${train_set}_sp"
            ...
            if "${video_input}"; then
                for factor in ${speed_perturb_factors}; do
                    if [[ $(bc <<<"${factor} != 1.0") == 1 ]]; then
                        printf "Speed Perturbation for other modalities besides Audio is not implemented \n"
                        # TODO: Implement Stage 2 for vision or other modalities
                        # utils/perturb_vision_data.py --train_dir "data/${train_set}" --factor "${factor}"
                    fi
                done
                # TODO: Create Speed Perturbed dirs for vision or other modalities
                # utils/create_vision_sp_data.py --train_dir "data/${train_set}_sp" --dirs ${_dirs}
            fi
    ...
    if [ ${stage} -le 3 ] && [ ${stop_stage} -ge 3 ]; then
        ...
        if "${vision_input}"; then
            log "[Multimodal - Vision] Stage 3: Vision extract: data/ -> ${data_feats}"
            for dset in "${train_set}" "${valid_set}" ${test_sets}; do
                if [ "${dset}" = "${train_set}" ] || [ "${dset}" = "${valid_set}" ]; then
                    _suf="/org"
                else
                ...
                cp data/"${dset}"/vision.txt "${data_feats}${_suf}/${dset}/" 
            done   
        fi
    fi
    ...
    if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
        ...
        if "${vision_input}"; then
            log "[Multimodal - Vision] Stage 4: Vision Filter Long/Short: ${data_feats}/org -> ${data_feats}"
            for dset in "${train_set}" "${valid_set}"; do
                cp "${data_feats}/org/${dset}"/vision.txt "${data_feats}/${dset}/" 
            done   
        fi
    fi
...
if ! "${skip_train}"; then
    ...
    if [ ${stage} -le 11 ] && [ ${stop_stage} -ge 11 ]; then
        ...
        _opts+="--valid_data_path_and_name_and_type ${_asr_valid_dir}/text,text,text "

        if "${vision_input}"; then
            _opts+="--train_data_path_and_name_and_type ${_asr_train_dir}/vision.txt,vision,vision "
            _opts+="--valid_data_path_and_name_and_type ${_asr_valid_dir}/vision.txt,vision,vision "
        fi
        ...
        ${python} -m espnet2.bin.launch \
            ...
            --multiprocessing_distributed true -- \
            ${python} -m espnet2.bin.avsr_train \
                --use_preprocessor true \
                --bpemodel "${bpemodel}" \
                --token_type "${token_type}" \
                --token_list "${token_list}" \
