...
def crop_roi(frames_array, roi_bound, roi_size=(96, 96)):
    ...
    for frame_idx in range(frames_num):
        roi_array.append(crop_frame_roi(frame=frames_array[frame_idx], roi_bound=roi_bound[frame_idx],
                                        roi_size=roi_size))
    return np.stack(roi_array, axis=0)


def segment_roi_json(roi_json_path, segments_name, segments_speaker, segments_start, segments_end, total_frames_num):
    ...
    for _, (name, speaker_id, frame_start, frame_end) in enumerate(zip(segments_name, segments_speaker, segments_start,
        ...
        if frame_end >= total_frames_num:
            ...
            print('{}: sengment end cross the line, {} but {}, skip'.format(name, frame_end, total_frames_num))
        else:
            segment_roi_bound = []
            segment_roi_idx = []
...
def segment_video_roi_json(video_path, roi_json_path, roi_store_dir, segments_name, segments_speaker, segments_start,
                           segments_end, file_handle):
    ...
    segments_roi_bound, delete_segments_key = segment_roi_json(roi_json_path, segments_name, segments_speaker,
                                                               segments_start, segments_end, total_frames_num)
    frame2segment_roi_bound = {}
    for i, segment_name in enumerate(segments_name):
        if segment_name not in delete_segments_key:
            segments_path = os.path.join(os.path.abspath(roi_store_dir), '{}.npz'.format(segment_name))
                ...
                for in_frame_idx in range(segments_end[i] - segments_start[i]):
                    if segments_start[i] + in_frame_idx in frame2segment_roi_bound:
                        frame2segment_roi_bound[segments_start[i] + in_frame_idx].append(
                            [segment_name, in_frame_idx, segments_end[i] - segments_start[i],
                ...
                if frames_idx in frame2segment_roi_bound:
                    ...
                    for frame_info in frame_info_list:
                        if frame_info[1] == 0:
                            ...
                            segments_roi_frames_buffer[frame_info[0]] = [crop_frame_roi(frame, frame_info[3], (96, 96))]
                        else:
                            segments_roi_frames_buffer[frame_info[0]].append(crop_frame_roi(frame, frame_info[3], (96, 96)))

                        if frame_info[1] == frame_info[2] - 1:
                            np.savez(os.path.join(roi_store_dir, '{}.npz'.format(frame_info[0])),
...
if __name__ == '__main__':
    ...
    parser.add_argument('data_root', type=str, default='data/test_far_video',
                        help='root directory of dataset')
    parser.add_argument('roi_json_dir', type=str, default='',
                        help='directory of roi json')
    parser.add_argument('roi_store_dir', type=str, default='data/test_far_video',
    ...
    all_input_params = input_interface(data_root=args.data_root, roi_json_dir=args.roi_json_dir)
    all_sentences = sorted([*all_input_params.keys()])
    nj = args.nj
    ji = args.ji if nj > 1 else 0
    start_time = time.time()
    handle = codecs.open(os.path.join(args.roi_store_dir, 'log', 'roi.{}.scp'.format(ji+1)), 'w')

    for sentence_idx, sentence_path in enumerate(all_sentences):
        if sentence_idx % nj == ji:
            print('#'*72)
            print('processing {}'.format(sentence_path))
            segment_video_roi_json(video_path=sentence_path, roi_store_dir=args.roi_store_dir, file_handle=handle, 
            ...
            print('{}/{} {}/{} min'.format(
                in_index, in_len, current_dur, round(current_dur * (in_len + 1) / (in_index + 1), 2)))
