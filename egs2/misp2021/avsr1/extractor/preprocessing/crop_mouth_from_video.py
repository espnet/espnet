...
def load_args(default_config=None):
    ...
    # -- mean face utils
    parser.add_argument(
        "--mean-face", default="./20words_mean_face.npy", help="mean face pathname"
    )
    # -- mouthROIs utils
    parser.add_argument(
...
lines = (
    list(filter(lambda x: "test" == x.split("/")[-2], lines))
...
for filename_idx, line in enumerate(lines):
    ...
    assert os.path.isfile(video_pathname), "File does not exist. Path input: {}".format(
        video_pathname
    )
    assert os.path.isfile(
        landmarks_pathname
    ), "File does not exist. Path input: {}".format(landmarks_pathname)
