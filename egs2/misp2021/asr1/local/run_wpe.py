...
def wpe_worker(wav_scp, data_root='MISP_121h', output_root='MISP_121h_WPE_', processing_id=None, processing_num=None):
    sampling_rate = 16000
    iterations = 5
    stft_options = dict(
        size=512,
    ...
    for wav_idx in range(len(wav_lines)):
        ...
        if processing_token:
            ...
            for wav_path in wav_list:
                file_exist = file_exist and os.path.exists(wav_path.replace(data_root, output_root))
                if not file_exist:
                    break
            if not file_exist:
