...
def get_parser():
    ...
    parser.add_argument(
        "--beamforming", default=False, help="Current version only supports False"
    )
    parser.add_argument("--include_wearer", type=str, default="False")

    # for LRS3 Dataset (additional Training data)
    parser.add_argument(
        "--LRS3", default=False, action="store_true", help="whether training with LRS3"
    )
    parser.add_argument(
        "--landmark", default=None, type=str, help="pre-extracted landmarks of LRS3"
    )
    return parser
