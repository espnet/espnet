...
def main():
    ...
    for split in splits:
        ...
        if not args.include_wearer and split != "train":
            file_lists = [
                file for file in file_lists if int(file.split("_")[-3]) != 2
            ]
        ...
        audio_scp = f"data/{split}_with_LRS3/wav.scp" if args.LRS3 else f"data/{split}/wav.scp"
        ...
        with open(text_scp, "w") as txtw:
            for line in file_lists:
                with open(os.path.join(args.data_dir, line.replace("Audio", "Text")) + ".txt", "r") as txt:
                    txtw.write(
                        f'{"_".join(line.split(os.sep))}\
        ...
        if args.LRS3 and split == "train":
            file_lists = sorted(
                glob.glob(os.path.join(args.data_dir, "LRS3",
                          "Audio", split, "*", "*.wav"))
            )
            file_lists = [
                f"{os.sep}".join(os.path.normpath(
                    file).split(os.sep)[-3:])[:-4]
                for file in file_lists
            ]
            ...
            with open(text_scp, "a") as txtw:
                for line in file_lists:
                    with open(os.path.join(args.data_dir, "LRS3", "Text", line + ".txt"), "r") as txt:
                        txtw.write(
                            f'{"_".join(line.split(os.sep))}\
                                \t{txt.readlines()[0].strip()}\n'
                        )
...
def get_parser():
    ...
    parser.add_argument("--data_dir", type=str,
                        required=True, help="preprocessed_data_dir")
    parser.add_argument("--include_wearer", type=str, required=True,
                        help="wheter include the eyeglasses wearer")
    parser.add_argument("--LRS3", type=str, default=False,
                        help="whether use LRS3 for training")
    return parser
