...
def generate_output(source_dir, output_dir, input_file):
    ...
    with open(os.path.join(output_dir, 'spk2utt'), 'w') as f_spk2utt, \
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Generate output files from input paths')
    parser.add_argument('--src', type=str, required=True, help='Path to the source directory containing WAV files')
    parser.add_argument('--dst', type=str, required=True, help='Path to the output directory')
