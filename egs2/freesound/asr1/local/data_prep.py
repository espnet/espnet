...
for split in ["train", "valid", "test"]:
    ...
    with open(Path(esc_root, f"manifest/balanced_speech_{mapping[split]}_manifest.json"), "r") as f:
        for line in f:
            speech_data.append(json.loads(line))
    with open(
        Path(esc_root, f"manifest/balanced_background_{mapping[split]}_manifest.json"), "r"
    ) as f:
        for line in f:
...
for x in dir_dict:
    with open(
        ...
        for line in dir_dict[x]:
            ...
            utt_id=wav_path.split("/")[-2]+"_"+wav_path.split("/")[-1]+"_"+str(offset)+"_"+str(duration)
            ...
            wav=f'"| sox {str(wav_path)} -r 16k -t wav'+ f' trim {str(offset)} {str(duration)}"'
