...
if __name__ == "__main__":
    ...
    myfile = open("SLUF1_token_labelF1_CRF.txt", "w")
    for gold_id in list(gold_examples):
        if gold_id in pred_examples:
            ...
            # if (results["overall"][3]+ results["overall"][5])==0:
            #     print(gold_id)
            #     myfile.write(str(gold_id)+" "+str(-1.0)+"\n")
            # else:
            myfile.write(str(gold_id)+" "+str(results["overall"][2])+"\n")
        bar.next()
    bar.finish()
