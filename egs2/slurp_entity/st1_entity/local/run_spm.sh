...
cut -d' ' -f2- data/train/text.asr.en > ${new_data}/input.txt
...
for split in train devel test; do 
    cut -d' ' -f-1 data/${split}/text.asr.en > ${new_data}/tmp_${split}_utt
    cut -d' ' -f2- data/${split}/text.asr.en > ${new_data}/tmp_${split}_transcript
    spm_encode --model=${bpemodel}.model --output_format=piece < ${new_data}/tmp_${split}_transcript > ${new_data}/new_${split}_transcript
    paste -d' ' ${new_data}/tmp_${split}_utt ${new_data}/new_${split}_transcript > ${new_data}/${split}/text.asr.en
