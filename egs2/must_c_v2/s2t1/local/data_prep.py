...
def prepare_single(utts: List[Dict], wav_id: str, max_sec: float=30, resolution: float=0.02) -> List[Dict]:
            ...
            for u in max_utts:
                src.extend(
                    [
                        ...
                        time2token(utts[u]['offset'] + utts[u]['duration'] - offset, resolution)
                    ]
                )
                tgt.extend(
                    [
...
def prepare_all(
    data_path: Path, output_path: Path, datasets: List[str], langs: List[str], max_sec: float, resolution: float
):
...
if __name__ == "__main__":
    ...
    timestamp_tokens = [
        "<notimestamps>",
        *[f"<{i * args.resolution:.2f}>" for i in range(round(args.max_sec / args.resolution) + 1)],
    ]
