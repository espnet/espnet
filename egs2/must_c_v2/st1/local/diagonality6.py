...
def diagonality(att):
    ...
    att = att.reshape(bs, nheads, -1).softmax(-1).reshape(bs, nheads, len_out, len_in)

    if isinstance(att, torch.Tensor):
        att = att.detach().cpu().numpy()
    lcm = compute_lcm(len_out, len_in)
    out_upsample = lcm // len_out
    in_upsample = lcm // len_in
    assert out_upsample * len_out == lcm
    assert in_upsample * len_in == lcm

    rel_distance = np.zeros((len_out, len_in))
    for i in range(len_out):
        for j in range(len_in):
            # scale the indices up to square
            i_scaled = (i * out_upsample) / lcm
            j_scaled = (j * in_upsample) / lcm
            rel_distance[i, j] = np.abs(i_scaled - j_scaled)

    result = (1. - (att * rel_distance).sum(-1) / rel_distance.max(-1)).mean(-1)    # (B, NHeads)
    result2 = (1. - (att * rel_distance).sum(-2) / rel_distance.max(-2)).mean(-1)    # (B, NHeads)
...
if __name__ == '__main__':
    ...
    for name, modu in st_model.named_modules():
        if "decoder" in name and "src_attn" in name:
    ...
    for keys, batch in tqdm(loader):
        ...
        if hasattr(st_model, "encoder_hier"):
            enc, enc_lens, _ = st_model.encoder_hier(enc, enc_lens)
        ...
        for name, output in outputs.items():
            ...
            diag, diag2 = diagonality(output)      # (B, NHeads)
            return_dict[name].append(diag)
            return_dict2[name].append(diag2)
    ...
    for name, diags in return_dict.items():
        ...
        log_fp.write(
            f"{name}: nsamples={diags.shape[0]}, mean={format_array(diags.mean(0))}, std={format_array(diags.std(0))}, mean2={format_array(diags.mean(0).mean(0))}\n"
        )
    for name, diags in return_dict2.items():
        diags = np.concatenate(diags, axis=0)     # (num_samples, num_heads)
        log_fp.write(
            f"{name}: nsamples={diags.shape[0]}, mean={format_array(diags.mean(0))}, std={format_array(diags.std(0))}, mean2={format_array(diags.mean(0).mean(0))}\n"
        )
