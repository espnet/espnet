...
if len(args.unk_word.split()) != 1:
    sys.exit("{0}: invalid unknown-word '{1}'".format(sys.argv[0], args.unk_word))

basename = os.path.basename(args.lexicon_file)
if basename != "lexiconp.txt" and basename != "lexiconp_silprob.txt":
    sys.exit(
        "{0}: expected the basename of the lexicon file to be either "
...
try:
    ...
    sys.exit(
        "{0}: failed to open lexicon file {1}".format(sys.argv[0], args.lexicon_file)
    )
...
while True:
    ...
    if this_split_line[0] == args.unk_word:
        if unk_index != -1:
            sys.exit(
                "{0}: expected there to be exactly one pronunciation of the "
    ...
    if len(this_split_line) <= num_fields_before_pron:
        sys.exit(
            "{0}: input file {1} had a bad line (too few fields): {2}".format(
...
if len(split_lines) == 0:
    sys.exit(
        "{0}: read no data from lexicon file {1}.".format(
            sys.argv[0], args.lexicon_file
        )
    )
...
try:
    ...
    sys.exit(
        "{0}: failed to open lexicon file {1} for writing (permissions probleM?)".format(
            sys.argv[0], args.lexicon_file
        )
    )
...
try:
    ...
    sys.exit(
        "{0}: failed to close lexicon file {1} after writing (disk full?)".format(
            sys.argv[0], args.lexicon_file
        )
    )
