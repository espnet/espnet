...
parser.add_argument(
    "arpa_in", type=str, help="The input ARPA file (must not be gzipped)"
)
parser.add_argument(
    "allowed_bigrams_in",
    type=str,
...
class ArpaModel(object):
    ...
    def Read(self, arpa_in):
        ...
        try:
            ...
            sys.exit("{0}: error opening ARPA file {1}".format(sys.argv[0], arpa_in))
        # first read till the \data\ marker.
        while True:
            line = f.readline()
            if line == "":
                sys.exit(
                    "{0}: reading {1}, got EOF looking for \\data\\ marker.".format(
                        sys.argv[0], arpa_in
                    )
                )
            if line[0:6] == "\\data\\":
        ...
        while True:
            ...
            if line[0:5] != "ngram":
                sys.exit(
                    "{0}: reading {1}, read something unexpected in header: {2}".format(
        ...
        while True:
            ...
            if line == "":
                sys.exit(
                    "{0}: reading {1}, found EOF while looking for \\end\\ marker.".format(
                        sys.argv[0], arpa_in
                    )
                )
            elif line[0:5] == "\\end\\":
                if len(self.orders) == 0:
                    sys.exit(
                        "{0}: reading {1}, read no n-grams.".format(
                            sys.argv[0], arpa_in
                        )
                    )
                break
                ...
                if (
                    not expected_line in line
                ):  # e.g. allow trailing whitespace and newline
                    sys.exit(
                        "{0}: reading {1}, expected line {1}, got {2}".format(
                            arpa_in, expected_line, line[:-1]
                        )
                    )
                if args.verbose >= 2:
                    print(
                        "{0}: reading {1}-grams".format(sys.argv[0], cur_order),
                        file=sys.stderr,
                    )
                ...
                while True:
                    ...
                    if l != cur_order + 1 and l != cur_order + 2:
                        sys.exit(
                            "{0}: reading {1}: in {2}-grams section, got bad line: {3}".format(
                    ...
                    try:
                        ...
                        backoff_prob = (
                            math.exp(float(a[cur_order + 1]) * log10)
                            if l == cur_order + 2
                            else None
                        )
                    except Exception as e:
                        sys.exit(
                            "{0}: reading {1}: in {2}-grams section, got bad "
                    ...
                    if backoff_prob != None:
                        self.orders[cur_order][
                            hist + (word,)
                        ].backoff_prob = backoff_prob

        if args.verbose >= 2:
            print(
                "{0}: read {1}-gram model from {2}".format(
                    sys.argv[0], cur_order, arpa_in
                ),
                file=sys.stderr,
            )
        if cur_order < 2:
            ...
            sys.exit(
                "{0}: this script does not work when the ARPA language model "
    ...
    def GetProb(self, hist, word):
        ...
        if len(hist) == 0:
            ...
            if not word in word_to_prob:
                sys.exit(
                    "{0}: no probability in unigram for word {1}".format(
                        sys.argv[0], word
                    )
                )
    ...
    def GetStateForHist(self, hist_to_state, hist):
        if hist in hist_to_state:
            ...
            if len(hist) <= 1:
                ...
                # in the ARPA file
                sys.exit(
                    "{0}: error processing histories: history-state {1} "
    ...
    def PrintAsFst(self, disambig_symbol, bigram_map):
        ...
        for state in range(len(state_to_hist)):
            ...
            if hist_len == 1:  # it's a bigram state...
                ...
                if not context_word in bigram_map:
                    print(
                        "{0}: warning: word {1} appears in ARPA but is not listed "
                        "as a left context in the bigram map".format(
                            sys.argv[0], context_word
                        ),
                        file=sys.stderr,
                    )
                    continue
                ...
                for word in word_list:
                    ...
                    if abs(cost) < 0.01 and args.verbose >= 3:
                        print(
                            "{0}: warning: very small cost {1} for {2}->{3}".format(
                                sys.argv[0], cost, context_word, word
                            ),
                            file=sys.stderr,
                        )
                    if word == "</s>":
                        ...
                        next_state = self.GetStateForHist(
                            hist_to_state, (context_word, word)
                        )
                        print(
                            "%d %d %s %s %.3f" % (state, next_state, word, word, cost)
                        )
                ...
                normalization_stats[hist_len][1] += sum(
                    [self.GetProb(hist, word) for word in bigram_map[most_recent_word]]
                )
                ...
                if args.verbose >= 3 and abs(cost) < 0.001:
                    print(
                        "{0}: very low backoff cost {1} for history {2}, state = {3}".format(
                            sys.argv[0], cost, str(hist), state
                        ),
                        file=sys.stderr,
                    )
        ...
        if args.verbose >= 1:
            for hist_len in range(1, len(self.orders)):
                ...
                avg_prob_sum = (
                    normalization_stats[hist_len][1] / num_states
                    if num_states > 0
                    else 0.0
                )
                print(
                    "{0}: for {1}-gram states, over {2} states the average sum of "
            ...
            if num_ngrams_disallowed != 0:
                print(
                    "{0}: for explicit n-grams higher than bigram from the ARPA model, {0} "
                    ...
                    "normally expect all or almost all of them to be allowed).".format(
                        num_ngrams_allowed, num_ngrams_disallowed
                    ),
                    file=sys.stderr,
                )
...
def ReadBigramMap(bigrams_file):
    ...
    try:
        ...
        sys.exit(
            "utils/lang/internal/arpa2fst_constrained.py: error opening "
    ...
    while True:
        ...
        if len(a) != 2:
            sys.exit(
                "utils/lang/internal/arpa2fst_constrained.py: bad line in "
        ...
        if word1 in ans and word2 in ans[word1]:
            sys.exit(
                "{0}: bigrams file contained duplicate entry: {1} {2}".format(
        ...
        if word2 == "<s>" or word1 == "</s>":
            sys.exit(
                "{0}: bad sequence of BOS/EOS symbols: {1} {2}".format(
                    sys.argv[0], word1, word2
                )
            )
        if word1 == "<s>":
    ...
    if len(ans) == 0:
        sys.exit(
            "{0}: no data found in bigrams file {1}".format(sys.argv[0], bigrams_file)
        )
    elif not (have_one_bos and have_one_eos and have_one_regular):
        sys.exit(
            "{0}: the bigrams file {1} does not look right "
...
if len(args.disambig_symbol.split()) != 1:
    sys.exit(
        "{0}: invalid option --disambig-symbol={1}".format(
