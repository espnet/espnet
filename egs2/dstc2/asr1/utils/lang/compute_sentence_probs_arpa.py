...
parser = argparse.ArgumentParser(
    description="This script evaluates the log probabilty (default log base is e) of each sentence "
...
parser.add_argument("arpa_lm", type=str, help="Input language model in arpa form.")
parser.add_argument("ngram_order", type=int, help="Order of ngram")
parser.add_argument(
    "text_in",
    type=str,
    help="Filename of input text file (each line will be interpreted as a sentence).",
)
parser.add_argument("prob_file", type=str, help="Filename of output probability file.")
parser.add_argument(
    "--log-base", type=float, default=math.exp(1), help="Log base for log porbability"
)
...
def check_args(args):
    ...
    args.prob_file_handle = (
        sys.stdout if args.prob_file == "-" else open(args.prob_file, "w")
    )
    if args.log_base <= 0:
        sys.exit(
            ...
            if line[0] == "-":
                ...
                if is_logprob(line_split[-1]):
                    ...
                    if ngram_key in ngram_dict:
                        sys.exit(
                            "compute_sentence_probs_arpa.py: Duplicated ngram in arpa language model: {}.".format(
                                ngram_key
                            )
                        )
...
def compute_sublist_prob(sub_list):
    ...
    if sub_string in ngram_dict:
        ...
        backoff_weight = (
            0.0
            if (
                backoff_substring not in ngram_dict
                or len(ngram_dict[backoff_substring]) < 2
            )
...
if __name__ == "__main__":
    ...
    if args.ngram_order <= 0 or args.ngram_order > max_ngram_order:
        sys.exit(
            ...
            + "Invalid ngram_order (either negative or greater than maximum ngram number ({}) allowed)".format(
                max_ngram_order
            )
        )
