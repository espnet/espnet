...
def utf8_visual_to_logical(text):
    ...
    bidi = icu_bidi.Bidi()
    bidi.inverse = True
    bidi.reordering_mode = (
        icu_bidi.UBiDiReorderingMode.UBIDI_REORDER_INVERSE_LIKE_DIRECT
    )
    bidi.reordering_options = (
        icu_bidi.UBiDiReorderingOption.UBIDI_OPTION_DEFAULT
    )  # icu_bidi.UBiDiReorderingOption.UBIDI_OPTION_INSERT_MARKS

    bidi.set_para(text, text_dir, None)

    res = bidi.get_reordered(
        0
        | icu_bidi.UBidiWriteReorderedOpt.UBIDI_DO_MIRRORING
