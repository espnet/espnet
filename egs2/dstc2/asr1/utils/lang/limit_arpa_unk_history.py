...
def get_ngram_stats(old_lm_lines):
    ...
    if len(ngram_counts) == 0:
        sys.exit(
            """Couldn't get counts per ngram section.
...
def find_and_replace_unks(old_lm_lines, max_ngrams, skip_rows):
    ...
    print(
        "Removed backoff probabilties from {} lines.".format(backoff_row_count),
        file=sys.stderr,
    )
...
def read_old_lm():
    ...
    with io.TextIOWrapper(sys.stdin.buffer, encoding="latin-1") as input_stream:
