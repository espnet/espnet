...
def get_args():
    parser = argparse.ArgumentParser(
        description="""This script augments a phones.txt
    ...
    with open(filename, "r", encoding="latin-1") as f:
        ...
        for line in lines:
            ...
            try:
                ...
                raise RuntimeError(
                    "Could not interpret line '{0}' in file '{1}'".format(
                        line, filename
                    )
                )
            if s[0] == "#nonterm_bos":
                raise RuntimeError(
                    "It looks like the symbol table {0} already has nonterminals "
