...
def read_kaldi_datadir(dir):
    ...
    for utt in wav_scp:
        if utt in text and utt in utt2dur and utt in utt2spk:
            utterances.append(
                Utterance(utt, wav_scp[utt], utt2spk[utt], text[utt], utt2dur[utt])
            )
        else:
            num_fail += 1
    ...
    if float(len(utterances)) / len(wav_scp) < 0.5:
        logger.info(
            "More than half your data is problematic. Try "
            "fixing using fix_data_dir.sh."
        )
        sys.exit(1)

    logger.info(
        "Successfully read {} utterances. Failed for {} "
...
def generate_kaldi_data_files(utterances, outdir):
    ...
    logger.info(
        "Successfully wrote {} utterances to data "
...
def find_allowed_durations(start_dur, end_dur, args):
    ...
    d = start_dur
    with open(
        os.path.join(args.dir, "allowed_durs.txt"), "w", encoding="latin-1"
    ) as durs_fp, open(
        os.path.join(args.dir, "allowed_lengths.txt"), "w", encoding="latin-1"
    ) as lengths_fp:
        while d < end_dur:
    ...
    for u in utterances:
        ...
        if i < len(allowed_durations):  # we have a larger allowed duration
            ...
            if args.speed_perturb:
                ...
                u2.wavefile = "{} sox -t wav - -t wav - speed {} | ".format(
                    u.wavefile, speed
                )
                u2.dur = allowed_dur2
            ...
            u3.wavefile = (
                "{} extend-wav-with-silence --extra-silence-length={} - - | ".format(
                    u.wavefile, delta
                )
            )
            u3.dur = allowed_dur2
...
def main():
    ...
    logger.info(
        "Durations in the range [{},{}] will be covered. "
        "Coverage rate: {}%".format(
            start_dur, end_dur, 100.0 - args.coverage_factor * 2
        )
    )
    logger.info(
