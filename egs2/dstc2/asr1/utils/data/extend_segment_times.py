...
parser.add_argument(
    ...
    type=float,
    default=0.1,
    help="Amount of padding, in seconds, for the end time of " "each segment.",
)
parser.add_argument(
    "--last-segment-end-padding",
    type=float,
    default=0.1,
...
while True:
    ...
    if not end_time > start_time:
        print(
            "extend_segment_times.py: bad segment (ignoring): " + line, file=sys.stderr
        )
...
for recording, utt_indexes in recording_to_utt_indexes.items():
    ...
    this_entries = sorted(
        [entries[x] for x in utt_indexes], key=lambda x: 0.5 * (x[2] + x[3])
    )
    min_time = 0
...
print(
    "extend_segment_times.py: extended {0} segments; fixed {1} "
