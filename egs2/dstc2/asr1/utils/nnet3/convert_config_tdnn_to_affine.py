        ...
        for line in f:
            if (
                ...
                for col in line.strip().split():
                    if (
                        col.startswith("input=") and offsets is not None
                    ):  # converted from Tdnn with input splices
                        inp = col.split("=")[1]
                        offsets_str = [
                            "Offset({}, {})".format(inp, o) if o is not "0" else inp
                            for o in offsets
                        ]
                        if len(offsets_str) > 1:
                            new_split_line.append(
                                "input=Append({})".format((", ").join(offsets_str))
                            )
                        else:
                ...
                if (
                    len(m) > 0 and m[-1] == "false"
                ):  # determine converting to Affine or Linear
                    use_bias = False
                new_split_line = []
                offsets = re.findall(r"time-offsets=(\S+)", line)
                if (
                    len(offsets) > 0
                ):  # extract time-offsets for determining input-dim below
                    offsets = offsets[-1].split(
                        ","
                    )  # -1 in case multiple fields of "time-offsets"
                else:
                    offsets = None
                for col in line.strip().split():
                    if col.startswith("name="):  # keep the name of Component
                        ...
                        type_str = "type={}".format(
                            ...
                            if use_bias
                            else "LinearComponent"
                        )
                        new_split_line.append(type_str)
                    elif col.startswith(
                        "input-dim="
                    ):  # change input-dim for Affine/Linear Component
                        ...
                        "time-offsets="
                    ):  # record time-offsets for component-node
                        offsets_dict[name] = offsets
                    elif not col.startswith(
                        "use-bias="
                    ):  # all the other fields: simply copy over
                        new_split_line.append(col)
                print(" ".join(new_split_line))
