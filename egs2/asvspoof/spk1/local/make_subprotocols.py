...
def process_trials(input_file, output_file, trial_type):
    with open(input_file, 'r') as infile, open(output_file, 'w') as outfile:
        for line in infile:
            ...
            if trial_type == "SV":
                label = ('1' if scenario == 'bonafide target' 
                         else '0' if scenario == 'bonafide nontarget' 
                         else None)
            else:
                match_scenario = f"{trial_type} spoof"
                label = ('1' if scenario == 'bonafide target' 
                         else '0' if scenario == match_scenario 
                         else None)
...
def main():
    parser = argparse.ArgumentParser(
        description='Make subprotocol files')
    parser.add_argument(
        '--in_file', required=True, help='Input file with trials')
    parser.add_argument(
        '--out_file', required=True, help='Output file name')
    parser.add_argument(
        '--trial_type', required=True, 
