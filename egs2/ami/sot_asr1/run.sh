...
if [ -n "${inference_model}" ]; then
    # ----- Mode 2: checkpoint-bundle inference + evaluation -----
    ...
    for tset in ${decode_test_sets}; do
        ...
        echo "[run.sh] cpWER on ${tset}"
        mkdir -p "${outdir}/eval"
        python local/evaluate_sot.py \
            --hyp_text "${outdir}/1best_recog/text" \
            --ref_text "data/${tset}/text" \
            --output_dir "${outdir}/eval" \
            --speaker_change_token "<sc>" \
            --text_norm whisper_nsf_keep_fillers

        echo "[run.sh] DER on ${tset}"
        python local/compute_der.py \
            --hyp_text_sot "${outdir}/1best_recog/text_sot" \
            --ref_text_sot "data/${tset}/text" \
            --output_dir "${outdir}/eval" \
            --separator "<sc>" \
            --collar 0.25
    done
