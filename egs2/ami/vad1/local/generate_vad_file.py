        ...
        if key in segment_dict:
            ...
            for segment in segment_dict[key]:
                if float(segment[0]) <= end and float(segment[1]) >= start:
                    overlap_segments.append(str(round(max(float(segment[0]), start) - start, 2)) + ' ' + str(round(min(float(segment[1]), end) - start, 2)))
            ...
            if len(overlap_segments) > 0:
                ...
                for i in range(len(overlap_segments)):
                    if i == 0:
                        ...
                        if float(overlap_segments[i].split(' ')[0]) == float(merged_segments[-1].split(' ')[1]):
                            merged_segments[-1] = merged_segments[-1].split(' ')[0] + ' ' + overlap_segments[i].split(' ')[1]
                        else:
                            merged_segments.append(overlap_segments[i])
                text_write_file.write(key + '_' + str(start).zfill(4) + ' ' + ' '.join(merged_segments) + '\n')
                segment_write_file.write(key + '_' + str(start).zfill(4) + ' ' + key + ' ' + str(start) + ' ' + str(end) + '\n')
                utt2spk_write_file.write(key + '_' + str(start).zfill(4) + ' ' + key + '\n')
