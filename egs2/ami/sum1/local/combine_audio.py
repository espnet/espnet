...
with open(os.path.join(data_dir, "wav.scp"), "r") as f:
    rec2wav = {
        line.strip().split(" ")[0]: line.strip().split(" ")[8] for line in f.readlines()
    }
...
with open(os.path.join(out_dir, "wav.scp"), "a") as f, open("cmd", "a") as g, open(
    os.path.join(out_dir, "segments"), "a"
) as f1, open(os.path.join(out_dir, "utt2spk"), "a") as f2, open(
    os.path.join(out_dir, "text"), "a"
) as f3:
    for meeting, segs in meeting2segs.items():
        ...
        f3.write("\n".join(text))
        g.write(
            "ffmpeg -y -safe 0 -hide_banner -loglevel error -f concat -i {} -c copy {}\n".format(
                os.path.join(meeting_audio_dir, f"{meeting}.list"), output_audio_path
            )
        )
