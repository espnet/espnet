        ...
        for n in ['train', 'dev', 'test']:
            for wav in wav_dict[n]:
                wav_abspath = os.path.abspath(
                    os.path.join(
                        args.data_path, spk, 'spchdatadir', cmd, wav
                    )
                )
                wav_id = f'{spk}_{cmd}_{wav.rstrip(".wav")}'
                task_str = frametotask(
                    os.path.join(
                        args.data_path, spk, 'framedir', cmd, wav.rstrip('.wav') + '.xml'
                    )
                )
...
for n in ['train', 'dev', 'test']:
    ...
    with open(
        os.path.join(parent_dir, 'text'), 'w'
    ) as text_f, open(
        os.path.join(parent_dir, 'wav.scp'), 'w'
    ) as wav_scp_f, open(
        os.path.join(parent_dir, 'utt2spk'), 'w'
    ) as utt2spk_f:
