...
if [ -z "${CMU_ARCTIC}" ]; then
   log "Fill in the value of 'CMU_ARCTIC' in db.sh"
   exit 1
fi
db_root=${CMU_ARCTIC}
...
if [ ${stage} -le 0 ] && [ ${stop_stage} -ge 0 ]; then
    ...
    local/data_prep.sh \
        ...
        "${db_root}/cmu_us_${srcspk}_arctic" "${srcspk}"
    local/data_prep.sh \
        --train_set "${trg_train_set}" \
        --dev_set "${trg_dev_set}" \
        --eval_set "${trg_eval_set}" \
        "${db_root}/cmu_us_${trgspk}_arctic" "${trgspk}"
fi
