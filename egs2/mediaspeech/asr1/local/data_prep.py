...
parser.add_argument("--validated_ratio", type=float, default=0.136, help="Ratio of validated set")
args = parser.parse_args()

file_names = [name[:-5] for name in os.listdir(args.data_path) if name.endswith('.flac')]
...
print ('samp_cnt, dev_samp_cnt, test_samp_cnt, validated_samp_cnt, train_samp_cnt: ', samp_cnt, dev_samp_cnt, test_samp_cnt, validated_samp_cnt, train_samp_cnt)
...
for file_idx in range(samp_cnt):
    ...
    processed_sample = {
        ...
        'text': text,
        'id': file_idx,
        'abs_path': 'ffmpeg -i %s -f wav -ar 16000 -ab 16 -ac 1 - |' % os.path.abspath(aud_file_path)
    }
...
for setname in ["train", "dev", "test", "validated"]:
    ...
    with open(os.path.join(dest_dir, "text"), "w") as text_f, \
        open(os.path.join(dest_dir, "wav.scp"), "w") as wav_scp_f, \
        open(os.path.join(dest_dir, "utt2spk"), "w") as utt2spk_f, \
        open(os.path.join(dest_dir, "utt2gender"), "w") as utt2gndr_f:
