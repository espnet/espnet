...
def get_chunk_dict(speaker, backchannel, word_lines, file_id, last_time, chunk_dict, local_start_time):
    backchannel_count=0
    activity_count=0
    ...
    for i in range(total_chunk):
        ...
        bc_label,backchannel_count=get_label(start_chunk,end_chunk,backchannel,backchannel_count)
        activity_label,activity_count=get_label(start_chunk,end_chunk,activity_arr,activity_count)
        if speaker=="A":
            if bc_label:
...
backchannel_dict={}
df=pd.read_csv("VAP/dataset_swb/backchannels.csv")
...
def resort_dialogue(sentence_dict):
    ...
    for k in sentence_dict:
        sentence_dict_clean[sentence_dict[k][-2]]=[sentence_dict[k][0],sentence_dict[k][-1],sentence_dict[k][2]]
...
csvfile = open('Val_Two_Channel_Label.csv', 'w')
writer = csv.DictWriter(csvfile, fieldnames = field_names)
duration_dict=get_duration() 
writer.writeheader()
for line in open("VAP/dataset_swb/splits_new/val.txt"):
    file_id=line.strip()
    print("----ok1---")
    print(file_id)
    print("----ok1---")
    ...
    word_file_A=open("VAP/dataset_swb/swb_ms98_transcriptions/"+dir_id+"/"+file_id+"/"+file_name_A+"-word.text")
    word_file_B=open("VAP/dataset_swb/swb_ms98_transcriptions/"+dir_id+"/"+file_id+"/"+file_name_B+"-word.text")
    sent_file_A=open("VAP/dataset_swb/swb_ms98_transcriptions/"+dir_id+"/"+file_id+"/"+file_name_A+"-trans.text")
    sent_file_B=open("VAP/dataset_swb/swb_ms98_transcriptions/"+dir_id+"/"+file_id+"/"+file_name_B+"-trans.text")
    sentence_dict_sort = create_dialog_sort(sent_file_A, sent_file_B)
    line_file_filtered_A, line_backchannel_A = remove_backchannel(file_name_A, backchannel_dict, word_file_A)
    line_file_filtered_B, line_backchannel_B = remove_backchannel(file_name_B, backchannel_dict, word_file_B)
    sentence_dict_sort_clean = clean_dialogue(line_file_filtered_A,line_file_filtered_B, sentence_dict_sort)
    ...
    # for k in sentence_dict_sort_final:
    #     if k>local_start_time:
    #         local_start_time=k
    #     break
    chunk_dict={}
    # import pdb;pdb.set_trace()
    get_chunk_dict("A", backchannel_A, line_file_filtered_A, file_id, last_time, chunk_dict, local_start_time)
    get_chunk_dict("B", backchannel_B, line_file_filtered_B, file_id, last_time, chunk_dict, local_start_time)
