...
dir_dict = {
    ...
    "valid": "Val_Two_Channel_Label_Mono_subsample.csv",
    "test": "Test_Two_Channel_Label_Mono.csv",
}
...
for x in dir_dict:
    ...
    for line in open("data/train_nodup/wav.scp"):
        ...
        line1 = (
            ...
            .replace("-c 1 ", "")
            .replace("-c 2 ", "")
            .replace("rate 16000", "rate 16000 channels 1")
        )
        if id1 in word_dict:
            ...
            for j in word_dict[id1]:
                for j_end in word_dict[id1][j]:
                    ...
                    segment_id = (
                        id1
                        + "_"
                        + "{:.6f}".format(float(j) / 10**4).split(".")[-1]
                        + "_"
                        + "{:.6f}".format(float(end_time) / 10**4).split(".")[-1]
                    )
                    file_write.write(
                        segment_id
                        + " "
