...
if __name__ == "__main__":
    ...
    with open(output_txt_path, 'w') as text_f, open(output_wav_scp_path,'w') as wav_scp_f, open(output_utt2spk_path,'w') as utt2spk_f:
        for train_id in all_train_ids:
            ...
            if audio_length > AUDIO_LENGTH_THRESHOLD:
                N_VERY_LONG_AUDIO+=1
                continue
            with open(os.path.join(ROOT_DATA_DIR, DATA_DIR, train_id, "metadata.json")) as f:
                meta = json.load(f)
    ...
    print(f"Processed {N_PROCESSED} audio files. Skipped {N_VERY_LONG_AUDIO} long audio, {N_ZERO_LENGTH_AUDIO} empty audio and {N_ZERO_LENGTH_CAPTIONS} empty caption items.")