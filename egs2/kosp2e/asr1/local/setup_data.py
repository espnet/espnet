...
assert os.path.exists(
    wavs_path
), "WAVS path does not exist. See https://github.com/warnikchow/kosp2e/tree/main"
assert os.path.exists(
    data_path
), "DATA path does not exist. See https://github.com/warnikchow/kosp2e/tree/main"
assert os.path.exists(
    metadata_path
), "METADATA path does not exist. See https://github.com/warnikchow/kosp2e/tree/main"
...
for data_type in tqdm(data_types, desc="Loading metadata"):
    meta_data_train[data_type] = pd.read_excel(
        f"{metadata_path}/{data_type}_train.xlsx"
    )
...
for data_type in tqdm(data_types, desc="Creating 'text' file"):
    ...
    for wav_file in tqdm(
        sorted(os.listdir(f"{wavs_path}/{data_type}")), desc=f"Processing {data_type}"
    ):
        ...
        if train_val_test is None:
            ...
            transcription = meta_data_train[data_type][
                meta_data_train[data_type]["id"] == wav_name
            ]["sentence"].values[0]
            with open(f"{data_path}/train/text", "a") as f:
                f.write(f"{wav_name} {transcription}\n")
        elif train_val_test == "val":
            transcription = meta_data_val[data_type][
                meta_data_val[data_type]["id"] == wav_name
            ]["sentence"].values[0]
            with open(f"{data_path}/val/text", "a") as f:
                f.write(f"{wav_name} {transcription}\n")
        elif train_val_test == "test":
            transcription = meta_data_test[data_type][
                meta_data_test[data_type]["id"] == wav_name
            ]["sentence"].values[0]
