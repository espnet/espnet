...
def prepare_data():
    ...
    for hf_split, espnet_split in split_map.items():
        ...
        with open(data_dir / "text", "w", encoding="utf-8") as f_text, \
            ...
            for i, example in enumerate(ds[hf_split]):
                ...
                try:
                    ...
                    if i < 5: # Print first few errors to debug
                        print(f"Failed to process example {i}: {e}. Data type: {type(audio_data)}")
                    continue
        
        # Generate spk2utt
        os.system(f"utils/utt2spk_to_spk2utt.pl data/{espnet_split}/utt2spk > data/{espnet_split}/spk2utt")
