...
if ! "${skip_data_prep}"; then
    ...
    if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
        if [ "${feats_type}" = raw ]; then
            ...
            for dset in "${valid_set}" ${test_sets}; do
                ...
                if [ -f "data/${dset}/wav.scp.${tgt_lang}" ]; then
                ...
                # NOTE(Ziang, Nov 1st, skip below for now)
    ...
    if [ ${stage} -le 3 ] && [ ${stop_stage} -ge 3 ]; then
        ...
        for dset in "${train_set}" "${valid_set}"; do
            ...
            if [ -f "${data_feats}/org/${dset}/feats_type" ]; then
                cp "${data_feats}/org/${dset}/feats_type" "${data_feats}/${dset}/feats_type"
            else
                echo "feats_type does not exist."
            fi

            # NOTE(Ziang), don't have wav.scp.en for dev/test set
            for utt_extra_file in ${utt_extra_files}; do
                if [ -f "${data_feats}/org/${dset}/${utt_extra_file}" ]; then
                    cp "${data_feats}/org/${dset}/${utt_extra_file}" "${data_feats}/${dset}"
                else
                    echo "${utt_extra_file} does not exist."
                fi
            done
            ...
            if [ "${_feats_type}" = raw ]; then
                ...
                for lang in "${src_lang}" "${tgt_lang}"; do
                    ...
                    if [ -f "${data_feats}/org/${dset}/utt2num_samples.${lang}" ] && [ -f "${data_feats}/org/${dset}/wav.scp.${lang}" ]; then
                        ...
                        echo "utt2num_samples.${lang} or wav.scp.${lang} does not exist."
                    fi
                done
            else
            ...
            # NOTE(jiatong): some extra treatment for extra files, including sorting and duplication remove
            for utt_extra_file in ${utt_extra_files}; do
                if [ -f "${data_feats}/${dset}/${utt_extra_file}" ]; then
