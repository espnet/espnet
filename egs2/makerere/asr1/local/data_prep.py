...
df = pd.read_csv('downloads/makerere_radio_dataset/transcribed/cleaned.csv')
    ...
    for x in ["train", "test"]:
        ...
        with open(os.path.join("data", x, "text"), "w") as text_f, open(
            ...
            for audio_path in audio_list:
                ...
                if len(list(df.loc[df['wav_filename'] == filename]['transcript'])) == 0:
                    continue
                transcript = list(df.loc[df['wav_filename'] == filename]['transcript'])[0] #" ".join(list(filename[:-5]))  # "o73" -> "o 7 3"
