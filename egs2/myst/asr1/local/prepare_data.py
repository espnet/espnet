...
for original_partition_name, data_partition_name in zip(original_partition_list, data_partition_list):
    ...
    for audio_file in tqdm(audio_files):
        ...
        count += 1
        if count == cache_size:
            with open(text_file, 'a') as tf, open(utt2spk_file, 'a') as uf, open(wav_scp_file, 'a') as wf:
                tf.write(text_data)
