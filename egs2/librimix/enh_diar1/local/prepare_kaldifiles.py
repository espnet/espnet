...
def process_metadata(metadata, target_dir, source_rttm, fs, num_spk):
    ...
    spk_1 = open(os.path.join(target_dir, "spk1.scp"), "w", encoding="utf-8")
    spk_2 = open(os.path.join(target_dir, "spk2.scp"), "w", encoding="utf-8")
    if num_spk == 3:
        spk_3 = open(os.path.join(target_dir, "spk3.scp"), "w", encoding="utf-8")
    noise_1 = open(os.path.join(target_dir, "noise1.scp"), "w", encoding="utf-8")
    ...
    with open(metadata, "r", encoding="utf-8") as f:
        ...
        if num_spk == 2:
            ...
            for linenum, line in enumerate(f, 1):
                ...
                spk_1.write("{} {}\n".format(mix_id, spk1_path))
                spk_2.write("{} {}\n".format(mix_id, spk2_path))
                noise_1.write("{} {}\n".format(mix_id, noise1_path))
                spk1_segs, spk2_segs = source_rttm[reco1], source_rttm[reco2]
                length_sec = float(length) / fs
                ...
                for spk_id, start, end in spk1_segs:
                    ...
                    if end <= length_sec:
                        ...
                        rttm.write(
                            "SPEAKER\t{}\t1\t{}\t{}\t<NA>\t<NA>\t{}\t<NA>\n".format(
                                mix_id, start, length_sec - start, spk_id
                            )
                        )
                        spk2utt_cache[spk_id] = spk2utt_cache.get(spk_id, []) + [mix_id]
