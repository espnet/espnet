...
if ! "${skip_train}"; then
    if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
        ...
        ${train_cmd} JOB=1:"${_nj}" "${_logdir}"/stats.JOB.log \
            ${python} -m espnet2.bin.diar_enh_train \
                --collect_stats true \
...
if ! "${skip_eval}"; then
    if [ ${stage} -le 6 ] && [ ${stop_stage} -ge 6 ]; then
        ...
        for dset in "${valid_set}" ${test_sets}; do
            ...
            ${_cmd} --gpu "${_ngpu}" JOB=1:"${_nj}" "${_logdir}"/diar_inference.JOB.log \
                ${python} -m espnet2.bin.diar_enh_inference \
                    --ngpu "${_ngpu}" \
                    --fs "${fs}" \
    ...
    if [ ${stage} -le 7 ] && [ ${stop_stage} -ge 7 ]; then
        ...
        for score_obs in true false; do
            ...
            local/show_enh_score.sh "${_dir}/../.." > "${_dir}/../../RESULTS.md"
        done
        log "Evaluation result for observation: ${data_feats}/RESULTS.md"
        log "Evaluation result for enhancement: ${diar_exp}/enhanced/ENH_RESULTS.md"
