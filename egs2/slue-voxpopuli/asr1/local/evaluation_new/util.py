...
def release2prediction(example: Dict[str, Any], load_gold: bool = False):
    ...
    """
    result = {}
    res = {"text": " ".join([t["surface"] for t in example["tokens"]]),
           "scenario": example["scenario"],
           "action": example["action"],
           "entities": [{"type": entity["type"], "filler": " ".join(
            [example["tokens"][i]["surface"].lower() for i in entity["span"]])} for entity in example["entities"]]}
    if load_gold:
...
def format_results(results: Dict[str, Tuple[float]],
                   label: str,
                   full: bool = True,
                   errors: bool = False,
    ...
    if table_layout in {"csv", "tsv"}:
        for i, row in enumerate(table):
            for j, item in enumerate(row):
                table[i][j] = str(item)
        return SEPARATORS[table_layout].join(header) + "\n" + "\n".join(
            [SEPARATORS[table_layout].join(row) for row in table])
