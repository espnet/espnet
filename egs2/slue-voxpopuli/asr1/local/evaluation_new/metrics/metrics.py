...
class ErrorMetric:
    ...
    def get_metric(self) -> Dict[str, Tuple[float, ...]]:
        ...
        for tag in all_tags:
            precision, recall, f1_measure = compute_metrics(self._true_positives[tag],
                                                            self._false_positives[tag],
                                                            self._false_negatives[tag])
            all_metrics[tag] = (precision,
                                recall,
                                f1_measure,
        ...
        if self._average == "macro":
            ...
            precision, recall, f1_measure = compute_metrics(sum(self._true_positives.values()),
                                                            sum(self._false_positives.values()),
                                                            sum(self._false_negatives.values()))
        all_metrics["overall"] = (precision,
                                  recall,
                                  f1_measure,
    ...
    @staticmethod
    def get_instance(metric: str,
                     average: str = "micro",
                     distance: str = "word") -> 'ErrorMetric':
        """
...
class SpanFMeasure(ErrorMetric):
    ...
    def __call__(self,
                 ...
                 prediction: List[Dict[str, str]]) -> None:
        """
...
class SLUF1(ErrorMetric):
    ...
    """
    def __call__(self,
                 results: Dict[str, Tuple[float, ...]]) -> None:
        """
...
class SpanDistanceFMeasure(ErrorMetric):
    ...
    def _get_lowest_distance(self,
                             ...
                             gold_spans: List[str]) -> Tuple[int, float]:
        """
...
def compute_metrics(true_positives: float, false_positives: float, false_negatives: float) -> Tuple[float, float, float]:
    """
