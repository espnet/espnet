...
for x in dir_dict:
    with open(os.path.join("data", x, "text.asr.en"), "w") as asr_text, open(
        os.path.join("data", x, "text.ner.en"), "w"
    ) as ner_text, open(
        ...
        for row in transcript_df.values:
            ...
            entities = sorted(entities, key=lambda x: x["filler_start"])
            new_entity_transcript=""
            for entity in entities:
                new_entity_transcript += (entity["type"]
                    + " FILL "
                    + entity["filler"].lower()
                    + " SEP "
                )
            asr_words = "{}".format(new_transcript).replace("<unk>", "unknown")
            asr_words = re.sub(r"[\.;?!]", "", asr_words)
            asr_words = re.sub(r"\s+", " ", asr_words)

            if new_entity_transcript =="":
                new_entity_transcript="<na>"
            ner_words = "{}".format(new_entity_transcript).replace("<unk>", "unknown")
            ner_words = re.sub(r"[\.;?!]", "", ner_words)
            ner_words = re.sub(r"\s+", " ", ner_words)
