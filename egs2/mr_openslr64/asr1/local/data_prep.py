...
# Copyright 2021 Carnegie Mellon University (Nikhil Gupta)
...
if __name__ == "__main__":
    ...
    for line in tsv_lines:
        l_list = line.split("\t")
        fid = l_list[0]
        # Speaker information is absent. Using the file ID as speaker ID.
        # Change if speaker information is available.
        spk = l_list[0]
        text = l_list[1]
    ...
    for phase in spks_by_phase:
        ...
        for spk in spks:
            fids = sorted(list(set(spk2utt[spk])))
            num_fids += len(fids)
            if phase == "test" and num_fids > 200:
                curr_num_fids = num_fids - 200
