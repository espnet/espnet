...
with open(train_file) as f:
    for line in f:
        if not line:
            continue
        train_lines.append(line.split(maxsplit=1)[1])
...
for test_name in ("test", "dev"):
    ...
    with open(test_file) as f:
        for line in f:
            if not line:
                continue
            test_uttids.append(line.split(maxsplit=1)[0])
            test_lines.append(line.split(maxsplit=1)[1])
        ...
        with open(f"data/{test_name}/{name}", "r") as f:
            ...
            for line in f:
                if not line.split()[0] in duplicate_uttids:
                    ...
                    print("removing wav file ", line.split()[0])
