...
def get_parser():
    parser = ArgumentParser(
        description="Convert text to s2t format."
    )
    parser.add_argument(
        "--data_dir",
        type=Path,
...
if __name__ == "__main__":
    ...
    with (args.data_dir / 'text.ctc').open('r') as f_text_ctc, (
        ...
        args.data_dir / 'text.prev'
    ).open('w') as f_text_prev:
        for line in f_text_ctc:
            uttid, raw_text = line.strip().split(maxsplit=1)
            f_text.write(f"{uttid} <eng><asr><0.00> {raw_text.strip()}{time2token(utt2dur[uttid])}\n")
