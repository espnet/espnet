...
lm_test_text_asr=dummy    # Text file path of asr evaluation set.
lm_test_text_tts=dummy    # Text file path of tts evaluation set.
...
use_speech=true
use_text=true
learn_kmeans=false  # Boolean flag for denoting whether to learn kmeans model
...
Options:
   ...
   # [Task dependent] Set the datadir name created by local/data.sh
    ...
    --learn_kmeans # Boolean flag for denoting whether to learn kmeans model
            ...
            for _task in "data/speech/"*; do
                for dset in ${_dsets}; do
                    ...
                    if [ -d "${_dir}" ]; then
                        ...
                        utils/copy_data_dir.sh --validate_opts --non-print "${_dir}" "${data_audio}/$(basename ${_task})/${dset}/"

                        scripts/audio/format_wav_scp.sh --nj "${nj}" --cmd "${train_cmd}" \
                            --audio-format "${audio_format}" --fs "${fs}" \
                            "${_dir}/wav.scp" "${data_audio}/$(basename ${_task})/${dset}"

                        echo "${feats_type}" > "${data_audio}/$(basename ${_task})/${dset}/feats_type"
                        echo "${audio_format}" > "${data_audio}/$(basename ${_task})/${dset}/audio_format"
                    fi
                done
            done
        else
...
if [ ${stage} -le 3 ] && [ ${stop_stage} -ge 3 ] && ! [[ " ${skip_stages} " =~ [[:space:]]3[[:space:]] ]]; then
    ...
    if "${use_speech}"; then
        ...
        # If learn_kmeans is false, skip kmeans learning stage
        if ! "${learn_kmeans}"; then
            kmeans_opts=${kmeans_opts}+" --skip_stages 2"
        fi
        ...
        for _task in "data/speech/"*; do
            ...
            if [ -d "${_task}" ]; then
                    scripts/feats/perform_kmeans.sh \
                        ...
                        --datadir "${data_audio}/$(basename ${_task})" \
                        --featdir "${data_extract}/$(basename ${_task})" \
        ...
        for _task in "data/speech/"*; do
            for dset in ${_dsets}; do
                _dir=${_task}/${dset}
                if [ -d "${_dir}" ]; then
                    utils/copy_data_dir.sh "${data_audio}/$(basename ${_task})/${dset}" "${data_feats}/${dset}/speech/$(basename ${_task})"
                    cat "${data_extract}/$(basename ${_task})/${kmeans_feature_type}/${_suf}${dset}/pseudo_labels_km${nclusters}.txt" \
                        > "${data_feats}/${dset}/speech/$(basename ${_task})/token"
                fi
            done
        done
    fi
    ...
    if "${use_text}"; then
        for _task in "data/text/"*; do
            for dset in ${_dsets}; do
                _dir=${_task}/${dset}
                if [ -d "${_dir}" ]; then
                    echo "${data_feats}/text/$(basename ${_task})/${dset}"
                    mkdir -p "${data_feats}/${dset})/text/$(basename ${_task})"
                    cp "${_dir}/text" "${data_feats}/${dset})/text/$(basename ${_task})"
                fi
            done
        done
    fi
...
if [ ${stage} -le 8 ] && [ ${stop_stage} -ge 8 ] && ! [[ " ${skip_stages} " =~ [[:space:]]8[[:space:]] ]]; then
    if [ -f ${lm_test_text_textlm} ]; then
        ...
        echo "PPL: ${lm_test_text_textlm}: $(cat ${_output_dir}/ppl)">${lm_exp}/"results.txt"
    fi
    ...
    if [ -f ${lm_test_text_speechlm} ]; then
        ...
        echo "PPL: ${lm_test_text_speechlm}: $(cat ${_output_dir}/ppl)">${lm_exp}/"results.txt"
    fi
fi
