...
dir_dict = split_df
for x in dir_dict:
    os.makedirs(os.path.join(data_prep_root, x), exist_ok=True)
    with open(os.path.join(data_prep_root, x, "wav.scp"), "w") as wav_scp_f, open(
        os.path.join(data_prep_root, x, "utt2spk"), "w"
    ) as utt2spk_f, open(os.path.join(data_prep_root, x, "text"), "w") as text_f:
