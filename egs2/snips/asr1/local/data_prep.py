...
parser.add_argument('--text_f', type=str, default = 'data/text.trans',help='file path for text')
parser.add_argument('--semantics', type=str, default = 'data/semantics',help='file path for semantics')
parser.add_argument('--utt2spk_f', type=str, default = 'data/utt2spk',help='file path for utt2spk')
parser.add_argument('--wavscp_f', type=str, default = 'data/wav.scp',help='file path for wav.scp')
parser.add_argument('--non_linguistic_symbols', type=str, default = 'data/non_linguistic_symbols.txt',help='non_linguistic_symbols')
...
with open(meta,'r') as meta_f, open(args.text_f,"w") as text_f, open(
    dataset,'r') as dataset, open(args.non_linguistic_symbols,"w") as non_linguistic_symbols, open(
        args.wavscp_f,"w") as wavscp_f, open(args.utt2spk_f,"w") as utt2spk_f,open(args.semantics,"w") as semantics:
        ...
        for intent,utts in intents.items():
            ...
            for utt in utts["utterances"]:
                ...
                utt_semantic = [utt_semantic[0]] #Currently, focus on intent classification task only
                utt_semantic = " ".join(utt_semantic)
                if utt_text != "Turn the lights up": # 1310.wav missed in the original dataset
