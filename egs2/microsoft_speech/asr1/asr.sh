...
if ! "${skip_train}"; then
    ...
    if [ ${stage} -le 9 ] && [ ${stop_stage} -ge 9 ]; then
        if "${use_ngram}"; then
            ...
            cut -f 2- -d " " ${data_feats}/lm_train.txt | /projects/ASR/espnet/tools/installers/kenlm/build/bin/lmplz -S "20%" --discount_fallback -o ${ngram_num} - >${ngram_exp}/${ngram_num}gram.arpa
            /projects/ASR/espnet/tools/installers/kenlm/build/bin/build_binary -s ${ngram_exp}/${ngram_num}gram.arpa ${ngram_exp}/${ngram_num}gram.bin 
        else
