...
This section lists the command for each stage and what needs to be set up before it will work. `--lm_config conf/train_transformer_opt350.yaml` reproduces the paper's OPT-350M-initialized main results; drop it (or point it at `conf/train_transformer_size768_e12.yaml`) for the from-scratch ablation instead.
