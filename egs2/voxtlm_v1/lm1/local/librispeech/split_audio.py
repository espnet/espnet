...
def split_sentence(wav_scps, texts, utt2spks, spk2utts, split_audio_path, id_prefix):
    ...
    new_wav_scps.append(f"{id_prefix}_1 {os.path.abspath(os.path.join(split_audio_path, f'{id_prefix}_1.flac'))}\n")
    new_wav_scps.append(f"{id_prefix}_2 {os.path.abspath(os.path.join(split_audio_path, f'{id_prefix}_2.flac'))}\n")
    ...
    return sorted(filtered_wav_scps), sorted(filtered_utt2spks), sorted(new_spk2utts), sorted(new_texts)
    
    
def split_audio_files(asr_eval_path, th=20):
    split_path = os.path.abspath(os.path.join(os.path.dirname(asr_eval_path), os.path.basename(asr_eval_path)+'_split'))
    split_audio_path = os.path.abspath(os.path.join(os.path.dirname(asr_eval_path), 'tmp_split'))
    ...
    for wav_scp in tqdm(wav_scps):
        ...
        if y_len > (th * sr):
            ...
            new_wav_scps, new_utt2spks, new_spk2utts, new_texts = split_sentence(new_wav_scps, new_texts, new_utt2spks, new_spk2utts, split_audio_path, basename[:-5])
