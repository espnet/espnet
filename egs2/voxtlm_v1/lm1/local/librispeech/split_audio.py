...
def split_sentence(wav_scps, texts, utt2spks, spk2utts, split_audio_path, id_prefix):
    ...
    for i in range(1, 3):
        audio_file_name = f"{id_prefix}_{i}.flac"
        full_path = os.path.abspath(os.path.join(split_audio_path, audio_file_name))
        new_wav_scps.append(f"{id_prefix}_{i} {full_path}\n")
