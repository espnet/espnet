...
def main_gen_single_process(params, proc_idx, st_idx, ed_idx, total_procs):
    ...
    while file_num < ed_idx:
        if (file_num - st_idx) % 10 == 9:
            print ('[INFO] (noisyspeech_synthesizer.py, process {}) generating wav mixture no. {} / {}'.format(
        ...
        # add reverb with selected RIR (Shih-Lun: deemed deprecated by Issue #80: https://github.com/microsoft/DNS-Challenge/issues/80)
        # rir_index = random.randint(0,len(params['myrir'])-1)
        
        # my_rir = os.path.normpath(os.path.join(params['data_root_dir'], 'impulse_responses', params['myrir'][rir_index]))
...
def main_gen(params):
    ...
    assert params['fileindex_start'] + round(file_num / params['n_jobs'] * params['n_jobs']) == params['fileindex_end'] + 1
    ...
    for j in range(params['n_jobs']):
        mp_args.append((
            ...
            params['fileindex_start'] + round(file_num / params['n_jobs'] * (j + 1)),
            params['n_jobs']
        ))
...
#   add mandarin data to clean speech
    ...
    # rir (Shih-Lun: deemed deprecated by Issue #80: https://github.com/microsoft/DNS-Challenge/issues/80)
