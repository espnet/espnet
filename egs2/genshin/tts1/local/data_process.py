...
def safe_rename(old_path: str, new_path: str) -> bool:
    try:
        ...
        print(
            f"Renamed successfully: {os.path.basename(old_path)} -> {os.path.basename(new_path)}"
        )
        return True
        ...
        if file_path.lower().endswith(".wav"):
            try:
                ...
                if sample_rate != resample_audio:
                    needs_conversion = True
                    print(
                        f"File needs resampling: {os.path.basename(file_path)} (current: {sample_rate}Hz)"
                    )
                ...
                if dry_run:
                    ...
                    print(
                        f"         (current: channels={data.shape[1] if len(data.shape)>1 else 1}, "
                ...
                if match:
                    ...
                    if dry_run:
                        ...
                        if os.path.exists(new_path):
                            ...
                            print(
                                f"Merged and removed folder: {old_path} -> {new_path}"
                            )
                        else:
        ...
        for root, dirs, _ in os.walk(target_folder, topdown=False):
            for dir_name in dirs:
                if has_chinese(dir_name):
                    ...
                    if not new_name:
                        print(
                            f"Skipping {dir_name}: name would be empty after cleaning"
                        )
                        continue
