...
def make_segment(file_id, labels, threshold=30, sil=["pau", "br", "sil"]):
    ...
    for i in range(len(labels)):
        ...
        if label.label_id in sil:
            if "itako25" in file_id and i < len(labels) - 1 and labels[i + 1].label_id == "N":
                inter = float(label.start) + (float(label.end) - float(label.start)) / 3
                segment.add(label.start, inter, "j")
                segment.add(inter, label.end, "a")
                continue
            if i < len(labels) - 1 and (("itako09" in file_id and labels[i + 4].label_id == "r") or ("itako48" in file_id and labels[i + 4].label_id == "k")):
            ...
            if len(segment.segs) > 0:
                ...
                segment = SegInfo()
            continue
        if "itako21" in file_id and ((label.label_id == "a" and labels[i - 1].label_id == 'u') or (label.label_id == "o" and labels[i - 1].label_id in ["i", "e"] and labels[i + 1].label_id == "o") or (label.label_id == "i" and labels[i - 1].label_id == "o" and labels[i - 2].label_id == "w")):
            segments.extend(segment.split(threshold=threshold))
            segment = SegInfo()
        if "itako27" in file_id and label.label_id == "i" and labels[i - 1].label_id == "s":
