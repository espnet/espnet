...
for idx = 1 : 2
 ...
 mkdir(strcat(out_folder,'/observation/',wsj_name,'_',mic_sel));
 ...
 create_folder_str(wsj_folder_clean,strcat(out_folder,'/observation/',wsj_name,'_',mic_sel));
 create_folder_str(wsj_folder_clean,strcat(out_folder,'/source/',wsj_name,'_',mic_sel));
 create_folder_str(wsj_folder_clean,strcat(out_folder,'/early/',wsj_name,'_',mic_sel));

 % copy of the transcription files
 if idx == 1
  % wsj1
  out_dir = strcat(out_folder,'/observation/',wsj_name,'_',mic_sel,'/13-32.1/wsj1/doc')
  mkdir out_dir
  copyfile(strcat(wsj_folder,'/13-32.1/wsj1/doc'), out_dir);
  out_dir = strcat(out_folder,'/observation/',wsj_name,'_',mic_sel,'/13-34.1/wsj1/doc')
  mkdir out_dir
  copyfile(strcat(wsj_folder,'/13-34.1/wsj1/doc'), out_dir);
  out_dir = strcat(out_folder,'/observation/',wsj_name,'_',mic_sel,'/13-34.1/wsj1/trans')
  mkdir out_dir
  copyfile(strcat(wsj_folder,'/13-34.1/wsj1/trans'), out_dir);
 else
  % wsj0
  out_dir = strcat(out_folder,'/observation/',wsj_name,'_',mic_sel,'/11-13.1/wsj0/doc')
  mkdir out_dir
  copyfile(strcat(wsj_folder,'/11-13.1/wsj0/doc'), out_dir);
  out_dir = strcat(out_folder,'/observation/',wsj_name,'_',mic_sel,'/11-4.1/wsj0/transcrp')
  mkdir out_dir
 ...
 for i=1:length(list)
  ...
  signal_48k=resample(signal,3,1);
  signal_48k=signal_48k./max(abs(signal_48k));

  % Convolution
  signal_rev=fftfilt(risp_imp,signal_48k);
  ...
  risp_imp_early = risp_imp;
  risp_imp_early(p+after_impulse:end) = 0;
  %%% direct signal + early reflections
  signal_early=fftfilt(risp_imp_early,signal_48k);
  signal_early=signal_early/Norm_factor;
  signal_early=linear_shift(signal_early',-p);

  % 48-16 kHz conversion
  signal_rev=resample(signal_rev,1,3);
  denominator=max(abs(signal_rev));
  signal_rev=signal_rev./denominator;
  %%% direct signal + early reflections
  signal_early=resample(signal_early,1,3);
  signal_early=signal_early./denominator;
  %%% source signal
  %%% NOTE (Wangyou):
  %%% The RIRs in DIRHA_WSJ seem to result in large values in `signal_rev`,
  %%% like tens of thousands, while `signal_48k` is within [-1.0, 1.0].
  %%% So it is fine to divide `signal_rev` by a large `Norm_factor` to make it
  %%% range from -1.0 to 1.0, but applying such normalization to `signal_48k`
  %%% would result in too small values (?e-6) in the source signal `signal_48k`.
  %%% I am not sure what is an appropriate scale for the source signal.
  %%% Here I simply normalize the source signal to range from -1.0 to 1.0.
%   signal_48k=signal_48k/Norm_factor;
  signal_16k=resample(signal_48k,1,3);
  signal_16k=signal_16k./max(abs(signal_16k));

  % saving the output wavfile
  name_wav=strrep(list{i},wsj_folder_clean,strcat(out_folder,'/observation/',wsj_name,'_',mic_sel));
  ...
  % saving additional signals
  %%% source signal
  name_wav2=strrep(list{i},wsj_folder_clean,strcat(out_folder,'/source/',wsj_name,'_',mic_sel));
  name_wav2=strrep(name_wav2,'.wv1','.wav');
  if vers>=2014
   audiowrite(name_wav2,0.95.*signal_16k,16000)
  else
   wavwrite(0.95.*signal_16k,16000,name_wav2)
  end
  %%% direct signal + early reflections
  name_wav3=strrep(list{i},wsj_folder_clean,strcat(out_folder,'/early/',wsj_name,'_',mic_sel));
  name_wav3=strrep(name_wav3,'.wv1','.wav');
  if vers>=2014
   audiowrite(name_wav3,0.95.*signal_early,16000)
  else
   wavwrite(0.95.*signal_early,16000,name_wav3)
  end
