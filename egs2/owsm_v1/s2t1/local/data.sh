...
# Note (jinchuan):
# (1) please work progressively from v1 to v3: this means you need to
...
# liscence issue. You should take care of it by yourself.
# (3) Due to the large volume of data, we are not confident the scripts
...
# (5) Even though we provide this centralized data preparation script
# and combine all datasets in it, we strongly recommend users
# to not use the merged train_v* and valid_v* for feature extractions.
# Instead, users may run stage 2-4 for each data separately and combine
# all datasets together under dump/raw directory. This will allow you to
# handle all datasets simultaneously; inspection and debuging will also be
# easier. This is exactly what we did in our experiments.
...
# Note(jinchuan):
# AMI & SWBD: the original egs2/ami/asr1/local/data.sh is not smooth
